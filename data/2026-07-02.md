...
TL;DR: 提出多尺度层注意力机制(MSLA)，通过显式建模多尺度和跨层特征交互，提升甲骨文识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法（包括最先进的层注意力技术）在捕获甲骨文细粒度细节和微妙变化方面表现有限，仅能带来边际改进。

Method: 提出多尺度层注意力(MSLA)，显式建模多尺度和跨层特征交互，在多个空间尺度上丰富细粒度细节表示。

Result: 在大型甲骨文数据集上，MSLA优于现有注意力机制，同时保持计算效率。

Conclusion: MSLA通过多尺度跨层特征交互，实现了更准确鲁棒的甲骨文识别。
...
TL;DR: 提出DiSIINet，基于扩散模型的共生信息交互网络，将医学图像增强和分割统一在一个框架中，两者相互促进提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法将图像增强作为独立预处理步骤，未能充分利用其与分割任务的协同潜力。

Method: 基于DDIM，集成增强分支和分割分支，通过共生信息交互（SII）模块利用交叉注意力在逆向扩散过程中实现动态特征级信息交换。

Result: 在多模态医学数据集（MRI、CT、超声）上，相比顺序或独立方法取得显著性能提升。

Conclusion: 增强与分割任务可在统一框架中相互促进，DDIM保证了高质量输出和高效推理。
...
TL;DR: 提出一种无需训练的证据注入框架，通过双端证据注入（ROI引导的视觉激活调节 + 解剖坐标映射为语义令牌）来缓解多模态大模型在医学视觉问答和报告生成中的幻觉问题，在多个模型和数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在临床VQA和放射学报告生成中表现良好，但推理时的幻觉问题（生成与影像证据矛盾的结论）损害了可信度。现有缓解策略依赖额外训练、外部检索或多阶段事后验证，成本高、流程复杂且泛化性差。

Method: 提出一个无需训练的证据注入框架：(1) 利用MedSAM获取ROI先验，通过ROI引导的激活调节重新校准视觉感知轨迹；(2) 将解剖坐标映射为离散语义令牌作为可验证的外部记忆，锚定文本推理轨迹；(3) 引入任务感知动态路由器，根据任务语义选择特定模态的干预策略，平衡感知基础和语言流畅性。

Result: 在2个任务、5个数据集上评估LLaVA-1.5-7B、LLaVA-Med-1.5-7B、Qwen3-VL-8B/32B、InternVL-3.5-8B/38B，闭端准确率提升最高约6%，开放端幻觉降低约35%。

Conclusion: 该训练免费框架通过双端证据注入有效缓解MLLM在医学场景中的幻觉问题，且具有良好的跨模型和跨任务泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有城市级VPR方法忽视了数据集中严重的长尾问题，导致模型偏向频繁出现的地点，在稀疏覆盖区域表现不佳。

Method: 提出DAPR框架，包含两个核心模块：(1) 模型无关的插件式框架，重平衡头类和尾类之间的梯度贡献；(2) 在分类-检索流程中应用多尺度距离搜索机制，计算每类分布紧凑度，在检索阶段提供互补增益。

Result: 在SF-XL基准上，相比分类-检索基线，测试集v1提升18.3%，v2提升6.7%。在SF-XL、MSLS、Pitts30k等多个基准上取得一致改进，泛化性良好。

Conclusion: DAPR作为插件式模块能有效解决VPR中的长尾问题，在多种方法和基准上均表现出稳定的性能提升。
...
TL;DR: 像素眼(PixelEyes)通过解耦推理与感知，引入掩码引导视觉搜索和语义区域广度优先搜索，解决了多轮视觉推理中重复定位失败和轨迹冗余问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在多轮视觉推理中，推理与感知纠缠在一起，导致定位失败后产生大量冗余轨迹，效率低下。

Method: 1) 掩码引导视觉搜索：用分割模型提供像素级精确定位；2) 语义区域广度优先搜索(BFS)：按语义区域组织探索避免重复裁剪错误子区域；3) 构建PixelEyes-6K数据集内化上述能力。

Result: 在Pinpoint-Bench基准上，最新MLLM和视觉推理代理均表现不佳，验证了该基准的难度和质量，PixelEyes显著优于现有方法。

Conclusion: 解耦推理与感知、结合精确视觉定位工具和语义BFS搜索策略，能有效提升多轮视觉推理的效率和准确性。
...
TL;DR: 提出SegFS双流快慢框架，通过将实例传播从目标解码转移到特征空间条件化，实现在移动设备上高效实时的开放词汇视频实例分割，延迟相比MOBIUS降低14倍且保持竞争性能。


<details>
  <summary>Details</summary>
Motivation: 当前开放词汇视频实例分割模型虽已降低计算成本，但实时高帧率设备端推理仍是开放挑战，需要更高效的方案。

Method: 提出双流快慢框架SegFS：慢流在稀疏关键帧上用开放词汇目标模型预测实例级表示，将其投影回骨干特征空间；快流通过轻量网络在后续帧中高效重定位和分割实例，将实例传播从目标解码转移到特征空间条件化。

Result: 快分支延迟比面向移动设备的MOBIUS低14倍，同时在标准OV-VIS基准上保持竞争性能。

Conclusion: 解耦多模态语义理解与密集掩码预测，实现高效时序传播，验证了特征空间条件化在移动端实时推理中的有效性。
...
TL;DR: 提出DeCoDe方法，通过将少样本图像分类分解为成对比较，利用现成MLLM的logit作为相似度分数，无需额外训练即可实现强少样本分类性能。


<details>
  <summary>Details</summary>
Motivation: 尽管MLLM在图像分析方面表现出色，但将其能力迁移到少样本图像分类仍具挑战性，现有方法通常需要额外训练或设计复杂流程。

Method: 将少样本分类分解为一系列二值比较判断：对查询图像与每个支持类别的图像进行配对，提示MLLM判断两者是否属于同一类别，将肯定响应的logit作为相似度分数，结合数据域高级信息进一步提升性能。

Result: 在12个数据集（6个标准+6个新构建）上显著优于当前最先进的少样本方法。

Conclusion: 简单的分解技术能将现成MLLM转化为强大的少样本学习器，无需额外训练即可在标准和新领域均取得领先性能。
...
TL;DR: 提出一种利用合成数据自动生成工业装配序列并训练实时质检模型的系统，无需真实数据标注，在真实装配任务上达到92.4%准确率。


<details>
  <summary>Details</summary>
Motivation: 工业装配质检依赖真实数据，但采集和标注成本高昂，本文旨在用合成数据替代真实数据以降低成本。

Method: 基于物理的运动生成模块模拟人类装配差异；域随机化渲染应对环境变化；基于目标检测的步骤识别模块实现仿真到真实的迁移。

Result: 真实装配案例准确率92.4%，三项改进分别带来46.7%、15.8%和61.2%的性能提升。

Conclusion: 该系统为工业装配质检提供了无需昂贵真实数据采集的实用解决方案，并在实际工业任务中验证了有效性。
...
TL;DR: MG-SpaIR是一个无需训练数据的图像恢复框架，通过多级粗到细残差层次结构和显式稀疏近端正则化，有效处理混合退化（模糊、下采样、噪声、缺失像素），优于Deep Image Prior等无数据基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的方法依赖大量训练数据且泛化性有限，而隐式神经表示在图像恢复中存在光谱局限性和伪影问题，需要一种无需训练数据、可解释且稳定的混合退化图像恢复方案。

Method: 基于隐式神经表示，提出多级粗到细残差层次结构逐级细化重建，并在高分辨率图像域引入显式稀疏近端正则化（如ℓ₀类型）抑制伪影，通过多级近端交替优化方案求解。

Result: 在混合退化基准测试中，MG-SpaIR一致优于Deep Image Prior等强无数据基线方法，具有更好的稳定性、可解释性和数据效率。

Conclusion: MG-SpaIR提供了一种稳定、可解释且数据高效的替代方案，建立了标准正则条件下的收敛保证，适合无需训练数据的图像恢复场景。
...
TL;DR: 本文重新定义了主动学习的预算阶段，提出三阶段分类（数据驱动、过渡、模型驱动），并证明策略有效性取决于归纳偏差与当前瓶颈的对齐。


<details>
  <summary>Details</summary>
Motivation: 现有AL方法依赖启发式标签数量划分预算阶段，无法跨数据集或架构泛化，缺乏统一理论框架。

Method: 重新解释PAC风格风险组件为动态交互项，证明主导机制转移的结构必然性；使用可测量代理和分段回归识别三阶段分类。

Result: 实验表明代表性、覆盖率和不确定性策略在不同阶段表现各异；自监督表示可更早进入过渡阶段。

Conclusion: AL效率取决于策略归纳偏差与当前瓶颈的匹配，为过渡感知的AL算法提供统一框架。
...
TL;DR: 提出了一种基于3D高斯溅射的渐进式测试时优化框架，从单目视频中实现高保真4D动物重建，通过对称感知时间编码和部件条件变形机制，在多种物种上取得了优于现有方法的几何精度、时间一致性和视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖严格的类别先验限制了泛化性，要么使用无约束生成模型牺牲了输入保真度，缺乏一种既能跨物种泛化又能保持高保真度的4D动物重建方法。

Method: 提出渐进式测试时优化框架，基于3D高斯溅射，采用对称感知时间编码（利用双边线索并吸收相机估计漂移）和部件条件变形机制（由可学习部件锚点和可学习蒙皮场引导），将铰接姿态与非刚性变形解耦。

Result: 在多种物种上实现了鲁棒的泛化，在几何精度、时间一致性和视觉保真度方面均优于现有基线方法，即使在先验严重不匹配的情况下也能表现良好。

Conclusion: 粗粒度的形状先验结合渐进式解耦策略足以实现高保真4D动物重建，方法在跨物种泛化性和重建质量上具有显著优势。
...
TL;DR: 提出一种黑盒模型窃取攻击，通过ViT补丁化导致的周期性精度下降，逆向恢复视觉语言模型中的视觉分词器配置（如补丁大小、预处理分辨率）。


<details>
  <summary>Details</summary>
Motivation: 现有VLM部署时视觉分词器参数（如补丁大小、预处理策略）被视为敏感信息，但缺乏针对此类信息的攻击方法，作者希望揭示这一隐私泄露风险。

Method: 利用ViT风格补丁化引入的任务级侧信道：构造对齐隐藏补丁网格的合成网格图像，观察精度周期性下降；通过遍历网格单元尺寸推断补丁大小；引入填充和一致性检查测试，区分动态/固定分辨率预处理并恢复目标分辨率。

Result: 在开源Qwen-VL系列及闭源模型（GPT、Claude）上成功恢复分词器相关参数。

Conclusion: 证明此类参数泄漏可用于预处理感知的迁移攻击和模型目标的对抗操作，揭示VLM部署的新安全威胁。
...
TL;DR: PRISM-VO是一种纯优化的稀疏光度视觉里程计框架，专为聚焦型全光相机设计，通过光度全光光束法平差在滑动窗口中联合优化相机位姿和逆深度，实现精确的度量尺度运动估计。


<details>
  <summary>Details</summary>
Motivation: 解决单目SLAM算法的尺度模糊性问题，同时避免复杂的初始化过程，仅依赖单个全光传感器实现可靠度量尺度的运动估计。

Method: 提出PRISM-VO框架，核心是新颖的光度全光光束法平差，在滑动窗口中联合优化相机位姿和点的逆深度，结合单个全光图像的几何深度与时间多视图约束。

Result: 在室内和室外场景中优于当前最先进的全光视觉里程计方法，与优化和基于学习的方法相比，PRISM-VO能够准确可靠地恢复场景的度量尺度。

Conclusion: PRISM-VO是一种有效的纯优化全光视觉里程计方法，克服了尺度模糊问题，无需复杂初始化，性能优越。
...
TL;DR: 论文揭示了预训练文本到图像扩散模型在适应（概念定制或概念擦除）过程中，会对语义无关的概念造成系统性损害，而传统聚合指标（如FID、KID）无法有效检测这种隐藏漂移。作者提出了DriftScope工具，通过软提示优化在token级别归因漂移，实现可解释的概念级审计。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅评估适应的预期效果，忽略了适应过程对无关概念造成的隐性损害，而聚合指标（FID/KID）无法在模型仍可用时检测到特定类别的性能下降。

Method: 使用稀疏自编码器分析和零样本分类来揭示概念漂移；提出DriftScope，通过优化软提示在token级别归因模型检查点之间的视觉概念变化，无需访问真实数据或模型内部。

Result: 适应过程会导致特定类别零样本准确率下降最多18.9个百分点，概念级分布发生显著偏移，而FID/KID在模型仍功能正常时保持平稳。

Conclusion: 适应导致的隐藏概念漂移是权重级修改的系统性后果，需要DriftScope这类可解释概念级审计工具来弥补聚合指标的不足。
...
TL;DR: 提出VOCA方法，利用视频编解码信息提升压缩视频流上的立体视觉里程计性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉里程计和SLAM系统主要针对原始无压缩视频，而实际系统常使用有损压缩视频，压缩带来的视觉伪影会降低传统跟踪方法的性能。

Method: VOCA：一种因果立体视觉里程计方法，利用编解码器信息来提升压缩流上的跟踪性能。

Result: 在压缩视频流的因果VO任务上，在相对轨迹误差、效率和绝对轨迹误差方面达到最先进水平。

Conclusion: 展示了广泛可用的视频编解码信息在视觉任务中的潜力。
...
TL;DR: 提出了一种结合患者特异性强度归一化和不确定性门控解剖注意力的框架，用于腹主动脉瘤腔内血栓的鲁棒分割，提升了多中心CTA数据的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 腹主动脉瘤腔内血栓分割因异质性特征和低对比度而困难，且不同CTA协议导致的域偏移限制了深度学习模型的多中心泛化。

Method: (1) 基于局部解剖高斯混合模型的患者特异性强度归一化；(2) 不确定性门控解剖注意力模块，结合空间先验并根据体素级置信度自适应调节其影响。

Result: 在分布内测试数据上达到SOTA，在外部多中心CTA数据上的泛化性能显著优于现有方法。

Conclusion: 通过显式分离视觉和解剖证据，方法保持可解释性，有效解决了域偏移问题。
...
TL;DR: 本文提出EgoSafetyBench，一个细粒度的以自我为中心视频安全基准，用于评估VLM在流式安全防护中的能力，揭示了现有模型在区分真正危险与表面可疑场景以及对抗场景内误导性文本方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有二值安全基准无法区分真正危险与表面可疑但常规的活动，导致安全防护模型要么过度干预要么漏检真实危险。

Method: 构建EgoSafetyBench，包含1,200个机器人视角视频场景，以0.5秒粒度标注。分为情景追踪（800场景，涵盖四类安全场景）和视觉通道追踪（400场景，测试场景内误导性文本对安全判断的影响）。采用对比阶梯设计，仅改变单个决定性线索来测试模型。

Result: 现有VLM能识别包含危险的视频，但常错过具体危险时刻（尤其是上下文危险）；误导性文本会降低所有测试模型的表现，脆弱模型漏检高达三分之一的危险，鲁棒模型则过度干预安全内容。

Conclusion: 当前VLM作为流式安全防护器存在显著局限性，表面上的安全鲁棒性往往反映的是不加区分的报警而非真正的物理推理能力。
...
TL;DR: 本文探讨了在现代ViT骨干网络下，医学图像分割是否仍需U-Net风格的解码器，并提出了一种基于查询的仅编码器分割框架EoSeg，实验证明其有效性。


<details>
  <summary>Details</summary>
Motivation: 随着大规模预训练提升ViT表示能力，传统U-Net风格解码器的必要性受到质疑，作者希望探究仅编码器架构能否胜任医学图像分割任务。

Method: 提出EoSeg（Encoder-only Segmentation），采用基于查询的仅编码器设计，包含多级查询建模和可学习的块融合策略。

Result: 在七个基准数据集上验证有效性，Synapse上mDice达85.50%，ACDC上91.73%，GlaS上93.27%。

Conclusion: 现代ViT骨干网络下U-Net风格解码器不再是医学图像分割的必要组件，EoSeg提供了一种有效的仅编码器设计方案。
...
TL;DR: 提出基于幅相分解的图像去模糊方法UPADNet，将传统LMMSE估计与深度学习展开相结合，在多个基准数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 认识到相位估计在恢复清晰图像细节中的重要性，现有去模糊方法大多直接在空间域恢复图像，忽视了相位信息的关键作用。

Method: 首先对模糊噪声图像进行幅值和相位的LMMSE估计，然后通过迭代优化算法恢复清晰图像，最后将迭代算法中统计确定的矩阵参数替换为可学习的参数，构建端到端训练的网络（UPADNet）。

Result: 在GoPro、RealBlur和COCO等基准数据集上，UPADNet优于包括基于算法展开的深度网络在内的现有方法，在高噪声和训练数据有限的情况下优势更明显。

Conclusion: 幅相分解结合算法展开是一种有效的图像去模糊策略，尤其适用于噪声较大或训练数据匮乏的场景。
...
TL;DR: 本文提出一种基于粒子多样化的测试时自适应框架，通过多假设推理解决熵最小化中的欠规范问题，提升模型在分布偏移下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 标准TTA在无监督下基于熵最小化进行自适应时，存在欠规范问题：多个不同的参数更新都能达到低熵，但决策边界截然不同，导致模型脆弱且容易崩塌到虚假模态。

Method: 将TTA重新解释为后验启发式视角，引入基于粒子的多样化框架，在输出、参数、优化器和输入四个层面进行结构化探索，允许多条合理的自适应轨迹同时进行。

Result: 在混合分布偏移下提升3-4%，批大小为1时提升2-3%，标签偏移下提升1-2.5%，优于现有SOTA方法。

Conclusion: 将TTA视为多假设推理问题而非单点优化任务，是缓解欠规范问题并实现可靠部署的关键。
...
TL;DR: 提出MVDGC框架，通过3D圆柱查询统一BEV和图像视图的行人检测，消除投影失真并联合优化BEV定位和2D框预测。


<details>
  <summary>Details</summary>
Motivation: 现有MVPD方法将图像特征投影到BEV存在严重畸变，导致空间结构破坏和特征模糊，且未充分利用BEV点与2D框之间的强关联关系。

Method: 提出MVDGC框架，采用3D圆柱查询（稀疏集），将行人建模为垂直圆柱体，同时约束BEV平面中心点和图像视图中的矩形投影，通过相机投影直接提取完整图像特征，消除投影畸变，统一BEV和图像视图定位为单一任务。

Result: 在密集场景下定位更精确，消除了投影失真带来的特征模糊问题，实现了BEV和图像视图的联合优化。

Conclusion: 3D圆柱查询有效统一了多视角行人检测中的BEV和图像视图任务，避免了投影畸变，提升了密集场景下的检测精度。
...
TL;DR: 本文提出Aegis，一种基于联合嵌入预测架构(JEPA)的乳腺癌检测与密度评估系统，在数据集上达到0.949 AUC的乳腺癌分诊性能和0.953 AUC的乳腺密度分类性能。


<details>
  <summary>Details</summary>
Motivation: 提升乳腺癌检测和乳腺密度评估的准确性，解决传统方法在跨机构、跨人群泛化能力不足的问题。

Method: 采用自监督JEPA预训练三种Vision Transformer变体(Small/Base/Large)，在71,103个研究(14个临床站点)上预训练，再进行监督微调，并结合渐进式分辨率缩放(最高2048x1536)。

Result: 最大模型在785例测试集上AUC 0.949(93%敏感度,75%特异度)；集成FDA批准基线模型后AUC提升至0.952；乳腺密度二分类AUC 0.953，四分类精确准确率62.6%，邻近准确率98.8%。

Conclusion: Aegis在乳腺癌检测和密度评估上达到先进水平，且在零样本跨人群迁移(外部VinDr-Mammo数据集)中展现出良好泛化能力。
...
TL;DR: 本文提出点监督在线时序动作定位（POTAL）方法OnPoint，通过离线到在线的多级蒸馏框架，仅用单点标注即可在流式视频中定位动作，在五个数据集上优于强基线。


<details>
  <summary>Details</summary>
Motivation: 传统时序动作定位依赖密集的片段标注或离线访问完整视频，限制了可扩展性和在线应用场景。

Method: 提出OnPoint框架，包含(1)伪片段实例蒸馏、(2)类激活序列蒸馏、(3)预测窗口级蒸馏的三级离线到在线蒸馏；同时引入原始点标签训练和动作性引导的注意力校准。

Result: 在五个数据集上，OnPoint持续优于强基线方法，建立了POTAL任务的坚实基础。

Conclusion: 提出的点监督在线时序动作定位范式(POTAL)及OnPoint方法有效解决了传统TAL的可扩展性和在线使用问题。
...
TL;DR: Rosetta 是一个可组合的原生多模态预训练框架，通过动量锚定正交投影（MAOP）实现模态扩展而不遗忘先前知识，解决了标准MoE和MoT架构在连续生成与离散理解任务间的梯度冲突与灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型在扩展新模态时容易发生表示覆写和灾难性遗忘，标准MoE和MoT架构在多模态扩展中严重受限。

Method: 采用模块化范式：全局共享专家保留核心基础知识，模态特定能力分布于即插即用专家；提出MAOP（动量锚定正交投影），利用优化器动量状态作为隐式语义锚点，选择性地中和新模态的冲突梯度分量，同时保留协同更新。

Result: 与标准MoE和MoT相比，Rosetta能稳健保留已建立的语言和视觉理解能力，同时展现优异的图像生成能力并解锁跨模态协同。

Conclusion: Rosetta为可组合的统一多模态基础模型铺平了道路，实现了非破坏性的模态扩展。
...
TL;DR: 本文研究了在无约束真实世界视频中，面部情感如何影响人体运动预测。提出门控情感变换器（GAT）动态调节跨模态信息流，发现面部情感仅能在短中期窗口（如30帧）提供有限预测线索，长期轨迹仍主要由运动学连续性决定。


<details>
  <summary>Details</summary>
Motivation: 真实世界视频中人体运动预测面临未来行为模糊性和多模态观测噪声的挑战，面部情感作为潜在行为线索的实际效用和机制边界尚不明确。

Method: 建立结合MediaPipe身体姿态轨迹和HSEmotion面部情感表示的多模态流水线，提出门控情感变换器（GAT）动态调节跨模态信息流，并进行多时间跨度评估和反事实实验。

Result: 早期跨模态简单拼接会降低预测精度；GAT门控机制能稳定跨模态融合；面部情感仅在短中期窗口（如30帧）内提供有效预测线索，长期轨迹仍由运动学连续性主导。
...
TL;DR: 提出Splash框架，通过掩码隔离方法在MLLM中扩展触觉感知能力，避免灾难性遗忘，在不增加推理开销的情况下实现触觉推理。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在集成触觉感知时存在零和权衡：紧凑模型的参数预算有限，获取新触觉模态与保留已有的视觉-语言推理能力之间存在冲突。

Method: 提出Splash框架，量化预训练参数的重要性，将参数空间划分为休眠子空间和关键子空间。冻结关键子空间作为稳定锚点保护视觉知识，仅更新休眠子空间来内化触觉对齐。

Result: 在SSVTP、TVL、TacQuad等视觉-触觉基准上达到SOTA性能，同时保持原有的通用能力，且不增加LLM部分的推理开销。

Conclusion: Splash实现了非破坏性的模态扩展，有效防止灾难性遗忘，为MLLM赋予触觉推理能力提供了一种高效、无损的解决方案。
...
TL;DR: 本文研究了在零售场景下，使用同步的自我中心和外部中心视频数据，通过LoRA微调基础视频扩散模型的效果，发现仅使用外部中心视图训练即可达到甚至优于多视图联合训练的效果。


<details>
  <summary>Details</summary>
Motivation: 基础视频扩散模型在互联网通用视频上预训练后，与真实世界的部署场景（如零售场景）存在分布偏移，需要有效的领域适配方法。

Method: 构建RetailSMV数据集（32,105个零售视频片段，含同步自我中心/外部中心双视角），基于Cosmos3-Nano使用LoRA进行三种配置（仅自我中心、仅外部中心、联合训练）的微调，并通过7个指标进行严格统计评估。

Result: 仅外部中心视图训练的模型在6/7个指标上达到或超过联合训练，且在LPIPS、PSNR、DreamSim上显著更优；在自我中心训练中加入外部中心数据有帮助，反之则有害。

Conclusion: 在零售场景的视频世界模型适配中，外部中心视图比自我中心视图更具信息量，且近水平预测窗口是适配收益最大的区域。
...
TL;DR: 本文提出一种基于车尾牌照字符尺寸的单目视觉距离估计方法，利用牌照字符标准高度作为尺度参照，结合字符高度、笔画宽度、字符间距等多重测量及单目深度网络，实现低成本、高精度的前车距离估计（误差<0.13m），并同时获取车辆身份信息。


<details>
  <summary>Details</summary>
Motivation: 现有前车距离估计依赖雷达、激光扫描仪或立体相机，成本高、功耗大、封装受限。本文探索能否仅用单个普通相机，利用道路上每辆车都有的标准尺寸车尾牌照来恢复距离，以低成本替代或补充现有传感器。

Method: 检测车尾牌照，测量其印刷字符高度、笔画宽度、字符间距及安装孔间距，识别发牌州以获取标准字符物理高度，结合相机投影几何恢复距离。多重测量与单目深度网络加权融合，帧间平滑计算距离变化率和碰撞时间。

Result: 实现了低成本、高精度的距离估计，误差小于0.13米，同时输出距离、方位和车辆身份信息。

Conclusion: 该方法利用印刷标准而非飞行时间或视差来获取尺度信息，是一种廉价、低维护的传感器补充方案，可用于容错感知堆栈中的碰撞预警等应用。
...
TL;DR: CORGI提出了一种无需3D监督的单张野生图像高质量3D狗模型重建框架，通过一致性感知的轨道生成、可变形3DGS和自监督生成修复三个核心模块，实现了跨品种的几何精确、视觉连贯且可动画化的3D资产重建。


<details>
  <summary>Details</summary>
Motivation: 从单张非约束图像中重建高度可活动的动物（如狗）的高保真3D模型仍然是一个艰巨的挑战，现有方法面临生成不一致性和缺乏多视图观测的问题。

Method: 提出CORGI框架，包含三个核心模块：1) 一致性感知轨道生成(CDOG)，利用专用LoRA规范任意输入姿态并合成360度视频；2) 一致性感知可变形3DGS(CA-3DGS)，基于D-SMAL先验，通过神经变形场显式建模每视图生成误差；3) 自监督变形条件生成修复(DCGR)，消除结构畸变并恢复高频细节。

Result: CORGI在多种狗品种上实现了最先进的性能，能够生成几何精确、视觉连贯且可动画化的3D资产。

Conclusion: CORGI完全消除了3D监督需求，能够无缝泛化到不同狗品种，生成高质量3D模型。
...
TL;DR: 提出DroneFINE，一种针对无人机场景下VLM检测的域感知PEFT方法，包含HyperAdapter（前景感知多路径适应）和SemanticGate（背景语义抑制），在无人机检测数据集上取得接近全微调的性能且参数量大幅减少。


<details>
  <summary>Details</summary>
Motivation: 无人机目标检测面临领域鸿沟：VLM预训练数据是"自然场景、前景主导"，而无人机图像是"鸟瞰视角、背景主导、小目标"。现有PEFT方法因静态结构约束无法有效弥合这一差距。

Method: 提出DroneFINE框架，包含两个域感知模块：1) HyperAdapter——数据依赖、前景感知的多路径适应机制，克服PEFT的静态结构约束；2) SemanticGate——基于文本条件的背景语义抑制算法，利用背景词汇主动引导模型抑制无关区域响应。

Result: 在VisDrone和UAVDT数据集上，DroneFINE显著优于现有PEFT方法，达到与全微调可比的性能，同时大幅减少可训练参数量。

Conclusion: 提出的域感知PEFT范式DroneFINE有效弥合了VLM与无人机图像之间的领域鸿沟，为VLM在无人机场景的应用提供了高效解决方案。
...
TL;DR: 本文提出POIL任务（个性化目标识别与定位），在POL基础上增加负样本拒绝能力，并设计IPLoc-ID方法，通过自提问机制连接候选框预测与目标验证，在抑制误检的同时保持定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有POL方法（如IPLoc）假设查询图像始终包含目标物体，这严重限制了其在真实场景中的应用。实际场景中大量查询图像并不包含目标物体，需要具备拒绝能力。

Method: 提出POIL任务框架，将POL置于更广泛的少样本目标检测中。设计IPLoc-ID算法，先预测候选边界框，再通过自提问机制（self-posed query）在统一自回归生成框架中判断该框是否对应目标物体。

Result: IPLoc-ID在负样本查询图像上显著抑制了误检，同时保持了与IPLoc相当的定位性能。

Conclusion: IPLoc-ID有效解决了实际场景中的实例级POIL任务，这是传统目标检测、少样本目标检测或仅定位的IPLoc方法无法充分解决的。
...
TL;DR: UltraFlux是一种基于Flux的扩散Transformer模型，通过数据-模型协同设计，原生支持4K分辨率下多种宽高比的文本到图像生成，在多项指标上超越开源和部分闭源模型。


<details>
  <summary>Details</summary>
Motivation: 现有扩散Transformer在1K分辨率表现良好，但扩展到4K原生分辨率时，位置编码、VAE压缩和优化等多个因素会耦合失效，单独解决任何一个因素都无法取得满意效果。

Method: 提出数据-模型协同设计：构建MultiAspect-4K-1M数据集；模型侧引入Resonance 2D RoPE+YaRN位置编码、非对抗VAE后训练、SNR-Aware Huber Wavelet损失函数、Stage-wise Aesthetic Curriculum Learning。

Result: 在Aesthetic-Eval 4096基准和多宽高比4K设置下，UltraFlux在保真度、美学质量和图文对齐指标上持续优于强开源基线，结合LLM提示优化器可媲美或超越Seedream 4.0。

Conclusion: 通过系统性解决高分辨率下的多因素耦合问题，实现了稳定、细节保留的4K DiT，且对多种宽高比具有良好泛化能力。
...
TL;DR: 提出SFDA-Track，一种无需源数据的域自适应目标跟踪方法，通过Mamba架构和超球面原型投影应对恶劣天气下的跟踪挑战。
...
Method: 1) 采用均值教师架构+双交互Mamba模块提取抗天气变化的候选目标token；2) 超球面原型投影模块将token投影到多域原型上，同时强制域特定和域不变属性。

Result: 在多个基准测试上超越现有最先进方法。

Conclusion: 提出的无源域自适应跟踪器可有效适应多种恶劣天气条件，具有良好的泛化能力。
...
TL;DR: 本文提出了一种尺度感知的令牌路由混合专家（MoE）架构，并引入外部自监督特征残差聚合方案，用于改进视觉自回归模型（VAR）在多尺度表示学习中的优化冲突和错误传播问题，在ImageNet 256×256上以更少训练轮次和参数实现了更优的FID。


<details>
  <summary>Details</summary>
Motivation: VAR模型存在多尺度表示学习缺陷：低尺度捕获全局语义、高尺度关注细粒度细节，共享架构导致优化冲突；且因果自回归过程下早期尺度的不准确语义会传播并严重降低最终输出质量。

Method: 1) 引入尺度感知的令牌路由混合专家（MoE）架构，实现跨尺度的解耦表示学习；2) 融入外部自监督特征增强早期尺度的语义建模，设计了针对VAR范式的残差特征聚合方案。

Result: 在ImageNet 256×256上，相比密集基线模型取得更优FID，仅需一半默认训练轮次和更小参数量，训练成本增幅极小，且随着训练轮次增加性能差距进一步扩大。

Conclusion: 提出的方法显著提升了VAR的训练效率和生成质量，有效解决了多尺度表示学习中的优化冲突和错误传播问题。
...
TL;DR: 本文提出FoCo，通过两个代理任务（文本锚定的视觉聚合和上下文条件语义补全）联合训练，在不依赖昂贵三元组的情况下实现零样本组合图像检索，在四个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有零样本组合图像检索方法仅增强视觉和文本表示，但组合函数本身未被学习，限制了模型表达细粒度语义修改的能力。

Method: 提出FoCo，将组合建模为两个协调阶段：1) 文本锚定的视觉聚合，选择性收集与修改相关的视觉内容；2) 上下文条件语义补全，将聚合的视觉内容与剩余场景上下文转换为连贯的组合表示。两个任务通过跨实例对比目标联合训练。

Result: 在四个零样本CIR基准上达到最先进性能，展示了更好的泛化能力。

Conclusion: 通过显式学习组合函数而非仅调整表示，FoCo能更好地表达多样化和细粒度的语义修改，在零样本组合图像检索中表现优异。
...
TL;DR: LIST3R是一个基于实例锚点的长序列3D重建框架，通过将长视频分割为重叠子序列并构建结构化实例库，利用跨子序列的实例锚点匹配实现全局一致的3D重建。


<details>
  <summary>Details</summary>
Motivation: 长序列3D重建中，随着序列增长，累积误差和片段对齐问题日益严重，现有方法缺乏有效的长期空间记忆机制来组织多片段重建。

Method: 将长视频分割为重叠子序列，为每个部分重建构建带有语义和几何证据的结构化局部实例库，通过跨子序列匹配实例锚点来恢复重复访问区域并提供目标感知约束，逐步将局部实例库整合为全局3D实例库。

Result: 在长序列基准测试上，LIST3R生成了更精确的轨迹和更高质量的3D重建结果，证明了持久实例锚点在组织长时序3D重建中的有效性。

Conclusion: 基于实例锚点的框架能有效组织长序列3D重建，模拟人类空间记忆组织方式，在长时序场景中优于现有方法。
...
TL;DR: 本文提出了一套计算机视觉逆问题方法，从微生物群体扩张的终点图像中恢复边界流动历史，并检验其是否与传递性适应度层级兼容，包括理论可观测性证明、循环性检测及主动设计控制层。


<details>
  <summary>Details</summary>
Motivation: 微生物群体扩张中有色扇区编码的信息远超谱系存活计数，需要从终点图像中恢复历史动态并检验适应度层级结构。

Method: 将问题形式化为计算机视觉逆问题，利用对数-极坐标提取扇区边界曲线的几何信号，提出可观测性、稳定性、传递性/循环性分解、接触完备圆形设计、高斯循环性检验及Bonferroni区间扫描等方法。

Result: 在确定性基准（解析终点图像、模糊/噪声往返测试、标量零假设检验、公开图像追踪、多分辨率机制性终点、非学习冻结前锋模拟器）上验证了方法有效性。

Conclusion: 实现从终点图像恢复边缘流动历史，检测四类型膨胀中的循环残差，并利用残差作为驱动力构建主动设计控制层，涵盖反应-扩散控制、表型前沿优化、协议合成、蒙特卡洛鲁棒性及群体状态桥接。
...
TL;DR: 提出首个针对图像到3D形状扩散Transformer（DiT）的压缩方法，通过剪枝、量化和微调实现高达66%模型尺寸缩减，同时保持几何保真度。


<details>
  <summary>Details</summary>
Motivation: 大型DiT模型计算成本高，现有扩散模型压缩策略难以直接迁移到3D生成领域，且以往3D效率方法仅关注推理速度而非主干网络压缩。

Method: 提出几何感知压缩框架，基于3D DiT层对几何合成的重要性非均匀分布，引入生命力引导的结构化剪枝、自适应量化和目标微调。

Result: 在多个最先进图像到3D模型上实现高达66%模型尺寸缩减，且合成保真度与完整模型相当。

Conclusion: 该框架可作为即插即用解决方案，用于高效3D形状生成。
...
TL;DR: DriveVer是一个轻量级、即插即用的测试时验证器，通过测试时缩放范式让自动驾驶系统在推理阶段验证和修正轨迹，无需昂贵训练。它在NAVSIM基准上显著提升基础规划模型性能，仅34M参数即可实现实时高效推理。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶模型存在性能瓶颈，训练时缩放导致高计算成本和边际收益递减，且现有规划器采用一次生成范式缺乏二次验证和主动修正机制。

Method: 构建基于NAVSIM基准的专用轨迹数据集（通过条件驱动聚类和平衡采样），采用双头架构融合候选轨迹、多视图视觉表征和车辆运动学特征，同时预测安全置信度分数和绝对几何修正向量。

Result: 在NAVSIM基准上显著提升基础规划模型性能，仅34M参数即可实现竞争性结果并保持实时推理效率。

Conclusion: 测试时缩放范式可有效提升自动驾驶规划性能，轻量化验证器是解决训练成本高和边际收益递减问题的可行方案。
...
TL;DR: 本文指出文本到图像扩散模型的安全对齐方法在粗粒度指标上表现良好，但在细粒度语义保真度上存在显著下降。作者发现这是由于文本编码器嵌入空间出现"语义坍缩"，并提出结构感知几何正则化（SAGE）方法来恢复结构化效用，同时保持强安全性。


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法依赖粗粒度的全局指标（如FID、CLIPScore）评估效用，忽略了对细粒度语义正确性的影响，导致对模型真实效用的高估。

Method: 提出结构感知几何正则化（SAGE），在安全对齐过程中显式保留文本编码器嵌入的扩散程度和跨提示相似性结构。

Result: 在TIFA指标上相比先前最优方法提升5.0%，同时保持强的安全性能和粗粒度效用分数。

Conclusion: 安全对齐方法需要关注细粒度语义保真度，所提出的SAGE方法能有效缓解语义坍缩问题，实现安全性与结构化效用之间的更好平衡。
...
TL;DR: 本文提出一种上下文感知门控解码器（MedCAGD），通过多尺度通道重校准、门控跳跃融合和全局上下文聚合，提升医学图像分割中解码器对特征的有效利用，在11个基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法多关注编码器的改进，但解码器在跨尺度对齐、上下文整合和边界保持方面仍有限制，分割精度受解码器设计瓶颈制约。

Method: 提出上下文感知门控解码器，包括：轻量级多尺度通道重校准、带空间竞争的门控跳跃融合、全局上下文聚合机制，将编码器全局信息注入解码中间阶段。

Result: 在11个医学图像分割基准上，该方法一致超越强基线方法，同时保持计算效率。

Conclusion: 从解码器中心视角重新审视医学图像分割，证明精心设计的解码器能有效转化强预训练编码器表示，产生空间一致的分割预测。
...
TL;DR: 本文提出MindAU框架，利用EEG信号进行细粒度面部动作单元编辑，通过噪声鲁棒表示学习、跨模态对齐和多模态扩散编辑实现高保真身份保持的面部编辑，并构建了E-CAFE基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有脑解码研究主要聚焦于从神经信号重建视觉内容，但利用EEG信号指导面部表情编辑尚未被充分探索。其核心挑战在于从噪声EEG中识别与面部动作相关的模式，并将其映射到局部、保持身份的表情编辑中，尤其对于面部神经肌肉障碍患者的辅助表达技术具有重要意义。

Method: MindAU包含三个关键模块：(1)通过时间掩码重建和AU分类监督学习噪声鲁棒且AU判别性的EEG表示；(2)利用双流流形对齐将EEG特征与AU级文本语义和身份约简的视觉位移轨迹在Qwen2.5-VL多模态空间中对齐；(3)在基于多模态扩散的编辑器中，结合EEG感知的多模态旋转位置编码、地标引导参考掩蔽和AU感知区域监督，实现高保真身份保持编辑。

Result: 在E-CAFE基准数据集上的大量实验验证了MindAU的有效性，表明其在EEG条件下面部AU编辑任务中取得了优越性能。

Conclusion: MindAU为EEG引导的细粒度面部表情编辑提供了统一框架，展示了作为未来面部神经肌肉障碍患者辅助表达技术的潜力。
...
TL;DR: 本文提出DroneIQA-VLE方法，融合SigLIP2视觉编码器与多任务回归头，以及LoRA微调的Qwen3.5-9B多模态大模型，通过两条互补管线集成预测无人机图像质量，在ICME 2026 Drone-IQA挑战赛中获得第二名。


<details>
  <summary>Details</summary>
Motivation: 针对低空无人机图像中目标感知的图像质量评估任务，现有方法难以同时捕捉全局、目标和背景质量信息，需要更有效的多任务预测框架。

Method: 构建两套互补管线：(1) SigLIP2视觉编码器+多任务回归头，分别预测全局、目标和背景质量分数；(2) LoRA适配的Qwen3.5-9B多模态大模型进行质量分数回归。最终将两条管线的全局质量预测结果取算术平均作为最终输出。

Result: 在ICME 2026 Drone-IQA Grand Challenge中取得第二名，代码已开源。

Conclusion: 融合传统视觉编码器与多模态大模型的集成方法能有效提升无人机图像质量评估性能，验证了多管线互补策略的有效性。
...
TL;DR: 提出EO-VGGT框架，通过显式物理几何嵌入将预训练的透视驱动3D基础模型适配到卫星推扫式成像几何，解决多视角卫星影像的DSM重建问题。


<details>
  <summary>Details</summary>
Motivation: 前馈3D基础模型在卫星遥感部署中受限于隐式透视假设与显式轨道推扫几何之间的结构差异，以及视角集的异质性。

Method: 1) GCCS策略优化视角选择；2) 传感器射线编码器(SRE)参数化像素级推扫视线为高维空间几何token；3) 轻量级RPAA适配器通过门控残差块将几何token注入冻结的Transformer骨干。

Result: 整合显式物理几何与优化视角选择对鲁棒的前馈卫星3D重建至关重要。

Conclusion: 通过显式物理几何嵌入可有效将透视驱动模型适配到轨道观测几何，提升卫星多视角3D重建质量。
...
TL;DR: 提出HyFL-CLIP，通过在双曲空间微调CLIP，利用双曲几何的层次结构能力，解决CLIP在长文本描述（>77 tokens）下对齐退化的问题，在跨模态检索等任务上显著提升。


<details>
  <summary>Details</summary>
Motivation: CLIP在长上下文描述（超过77个token）中表现不佳，因为绝对位置编码限制和预训练数据多为短描述，导致句子被重排、摘要或部分省略时图像-文本对齐质量下降。现有扩展位置编码的方法在文本扰动下仍存在对齐退化问题，根本原因在于欧几里得对比目标强制一一匹配，缺乏建模全局上下文与组成部分之间层次关系的机制。

Method: 提出HyFL-CLIP双曲微调框架，通过跨流形相似度蒸馏，将欧几里得CLIP学到的文本-图像对齐知识迁移到双曲空间。利用双曲几何的层次蕴含关系，通过爱因斯坦中点聚合连接摘要级token特征、长上下文描述、短文本组件和图像，建模部分-整体层次关系。

Result: 在长上下文跨模态检索、带扰动描述的跨模态检索、模态内检索和短文本跨模态检索等多个基准上取得改进，尤其在文本扰动下的长文本跨模态检索中相比最佳先前方法提升高达19.5%。HyFL-CLIP还能无缝集成到其他模型框架（如SDXL）中。

Conclusion: HyFL-CLIP通过双曲空间微调有效增强了CLIP对长上下文的理解鲁棒性，证明了双曲几何在层次化多模态对齐中的价值，且具有良好的可迁移性。
...
TL;DR: 针对视觉语言模型在视觉表征学习中缺乏显式控制导致的不稳定问题，提出IRA随机注意力机制，通过正则化视觉信息注入量来稳定模型，实验表明该方法能产生更平滑的曲率轨迹并抑制注意力下沉。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在标准下一词预测目标下，视觉嵌入被被动优化，容易注入冗余或虚假信号，导致物体幻觉、视觉接地弱和灾难性遗忘等问题。

Method: 提出Information-Regularized Attention (IRA)，一种随机注意力机制，显式调节注入中间Transformer层隐藏状态的视觉信息量，通过局部重参数化将视觉表征的不确定性转化为独立于数据点的局部噪声。

Result: IRA能产生更平滑的曲率轨迹，并抑制所有层的注意力下沉现象，表明视觉信号变换更稳定。

Conclusion: 随机注意力不仅是一种正则化手段，更是生成式架构中表征学习的关键贡献者，为构建更可靠的VLM提供了新方向。
...
TL;DR: 提出VideoSearch-R1，一种通过多轮交互与视频搜索引擎实现迭代式视频检索与推理的智能体框架，引入软查询精炼（SQR）在连续潜在空间中优化搜索查询，并使用GRPO进行训练，在VCMR任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将视频检索视为预处理步骤，检索失败后无法修正，导致后续细粒度推理失效；而现有智能体框架假设相关视频已给定，仅关注视频内推理。缺乏一个能迭代检索并进行细粒度推理的统一框架。

Method: 提出VideoSearch-R1智能体框架，包含多轮视频检索与推理；引入软查询精炼（SQR）在连续潜在空间优化检索词；使用GRPO算法结合检索和下游任务奖励信号进行训练。

Result: 在三个数据集上的视频语料库时刻检索（VCMR）任务取得SOTA性能；SQR相比显式文本级查询精炼显著减少生成token数量。

Conclusion: VideoSearch-R1通过迭代检索和软查询精炼有效解决了视频检索与推理的耦合问题，性能优越且效率更高。
...
TL;DR: 提出非对称互惠变分学习（AMVL），通过双向KL散度校准机制解决多模态大语言模型在连续潜在推理中的训练-推理不匹配问题，在BLINK基准上平均提升+10.83分。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM受限于语言空间瓶颈，而连续潜在推理虽能保留感知细节，但存在严重的训练-推理不匹配：训练时后验分布可访问真实答案导致捷径学习，推理时先验分布无法获取这些信息，从而性能下降。

Method: 提出非对称互惠变分学习（AMVL）：前向KL散度训练目标无关的先验分布匹配后验分布；反向KL散度同时正则化后验分布，防止其塌缩到推理不兼容的区域，缓解"答案泄露"问题。通过理论分析将泄露形式化为先验污染，并证明双KL目标可减少污染。

Result: 在BLINK复杂基准上平均提升+10.83分，个别推理任务提升高达+32.00分，优于强离散和潜在推理基线，并验证了潜在空间稳定性提升。

Conclusion: AMVL框架有效解决了连续潜在推理中的训练-推理不匹配问题，双KL目标能同时优化先验和后验分布，提升MLLM的复杂视觉推理能力。
...
TL;DR: 提出StochasT方法，通过随机分组语言任务为不同深度的对话轮次，解决LVLM在多轮训练与单轮评测之间的不匹配问题，提升模型在两种场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: LVLM的视觉指令微调(VIT)通常将同一图像的多个语言任务打包成多轮对话训练，但现有评测仅在单轮场景下进行，导致训练与评测不匹配，模型在多轮训练中易出现视觉注意力衰减和上下文过拟合。

Method: 提出StochasT，随机将同一图像的语言任务分组为不同大小的簇（即不同轮次深度），同时保留其原始顺序。该方法借鉴Dropout和随机深度思想，但实际不丢弃任何数据以最大化训练数据利用率。同时引入基于平衡拉丁方设计的评测机制。

Result: 大量实验表明StochasT能有效赋予LVLM在单轮和多轮场景下强大且协调的能力。

Conclusion: StochasT通过随机化轮次深度，弥合了多轮训练与单轮评测之间的差距，提升了LVLM在不同上下文依赖场景下的鲁棒性。
...
TL;DR: 提出MindEdit-Bench基准，测试VLM在物体级反事实空间推理（如移动/旋转物体）上的能力，发现VLM表现远逊于人类（差距53个百分点）。


<details>
  <summary>Details</summary>
Motivation: 现有VLM基准主要测试观测性空间推理和固定场景下的视角变换，缺乏对假设性物体操作（移动/旋转）后果的反事实推理能力的评估。

Method: 从智能手机三连拍室内场景构建3D场景图，设计6个空间推理任务（包括2个新型反事实推理任务），提供8-24个结构化选项，覆盖120个私有室内场景，共1003个人类验证问题。

Result: 15个VLM在6个任务上的平均准确率仅8%-31%，人类多数投票准确率为81%-97%，人机差距达53个百分点，每项任务至少差39个百分点。

Conclusion: 当前VLM在物体级反事实空间推理上存在显著缺陷，尤其是在深度轴推理和复杂可见性编辑场景中表现薄弱。
...
TL;DR: GenSP是一个数据驱动框架，通过学习神经生成模型来预测从单位球面到形状集合的连续映射，从而获得一致且低失真的球面参数化。


<details>
  <summary>Details</summary>
Motivation: 现有的球面参数化方法通常独立优化每个形状，导致不同形状间参数化不一致，缺乏跨形状的对应关系。

Method: 提出三个关键策略：(1) 连续神经变形模型预测表面点，避免离散化伪影；(2) 用中间形状扩充训练空间，学习有意义的变形；(3) 沿潜在空间生成树传播映射，计算可靠初始对应。

Result: 在ShapeNet数据集上，该方法在降低几何失真和提升跨形状一致性方面显著优于现有最先进方法。

Conclusion: 通过生成模型实现球面参数化的一致性是一种有效范式，能够在保持低失真的同时建立可靠的跨形状对应关系。
...
TL;DR: 提出HieDG框架，将几何动态建模为离散表征，解决多动物跟踪中连续几何嵌入不稳定的问题，在多个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于查询的跟踪器依赖连续几何嵌入，坐标微小扰动会不成比例地改变跨帧注意力权重，导致身份关联性能下降。

Method: 提出HieDG框架，采用两阶段残差码本将位置、尺度和速度线索离散化为结构化离散token，与视觉嵌入对齐并集成到跟踪查询中。

Result: 在AnimalTrack、BFT、BuckTales动物跟踪基准上达到SOTA，HOTA、AssA、IDF1显著提升；在DanceTrack、SportsMOT上表现有竞争力。

Conclusion: 离散化几何建模能有效提升多动物跟踪的身份一致性，且可推广到通用多目标跟踪场景。
...
TL;DR: 本文提出了一种无需训练、可解释的几何对齐框架，通过Sim(3)变换将生成式3D先验与部分单目观测对齐，解决了尺度模糊和几何幻觉问题，在提出的GenPMOAlign基准上显著优于传统方法和现有学习基线。


<details>
  <summary>Details</summary>
Motivation: 将生成式3D重建与部分单目观测对齐是一个关键但未被充分探索的挑战，由于稀疏单目输入与密集生成先验之间存在严重不对称性（尺度模糊、几何幻觉、缺乏初始重叠），传统配准方法效果不佳。

Method: 提出无需训练的可解释几何对齐框架，基于3D相似变换(Sim(3))恢复度量尺度与位姿；引入显式尺度因子解决度量模糊；采用粗到细对齐策略，利用几何感知描述子进行鲁棒初始化，使用解耦闭式求解器进行精确优化；引入幻觉过滤(Hallucination Filtering)操作抑制由几何幻觉产生的异常点。

Result: 在提出的GenPMOAlign基准上，该方法实现了稳定且准确的配准，显著优于经典几何管线与最先进的基于学习的基线方法。

Conclusion: 该框架有效解决了生成式3D先验与部分单目观测对齐中的尺度模糊和几何幻觉问题，为相关领域提供了新的解决思路。
...
TL;DR: 提出了一种基于先验锚定的多器官病理报告生成框架（PriOrGen），通过视觉原型锚定瓶颈模块和元报告锚定库模块，缓解长尾分布下的视觉表征偏差和文本解码偏差，在头尾器官类别上均取得优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 现有病理报告生成方法多针对单一器官场景，忽略了临床实践中多器官场景（器官类型呈长尾分布）的需求，存在视觉表征偏差（编码器偏向头部类别）和文本解码偏差（解码器过拟合头部类别叙事模式）两大问题。

Method: 提出Prior-anchored multi-Organ pathology report Generation (PriOrGen)框架，包含两个核心模块：(1) Visual-Prototype Anchored Bottleneck模块，利用信息瓶颈原理和可学习锚定表示，选择性保留诊断相关视觉信息，过滤头部偏向冗余；(2) Meta-Report Anchored Bank模块，构建器官特定元报告锚定库，检索器官忠实文本先验，引导解码器摆脱头部类别叙事模式。

Result: 在多器官病理数据集上的大量实验表明，该方法有效缓解了长尾偏差，在头部和尾部器官类别上均优于现有最先进方法。

Conclusion: 所提PriOrGen框架成功解决了多器官病理报告生成中的长尾分布问题，通过视觉和文本层面的双重偏差校正，实现了更可靠的病理报告自动生成。
...
TL;DR: 提出AnF-DiffPET扩散框架，利用解剖和频率引导，从低剂量PET恢复高剂量PET图像，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 低剂量PET存在噪声大、定量不准确的问题，现有去噪方法缺乏足够的解剖引导、多尺度特征传播不稳定以及频域恢复不确定。

Method: 提出AnF-DiffPET框架，包含解剖-频率引导（AFG）、多尺度交叉Transformer重建（MSCTR）和频率对比难例挖掘（FCHM），以CT为条件进行低剂量到高剂量PET去噪。

Result: 在四个PET/CT数据集上，该方法在图像保真度、解剖一致性和定量准确性上优于CNN、GAN、Transformer和扩散基线方法。

Conclusion: 所提出的解剖和频率引导扩散框架有效提升了低剂量PET去噪性能。
...
TL;DR: 提出Cross4D-JEPA方法，通过从冻结的2D/视频基础模型中蒸馏稠密语义特征到4D点云编码器，实现了优于现有方法的自监督预训练效果。


<details>
  <summary>Details</summary>
Motivation: 现有4D点云自监督预训练方法几乎全是模态内方法，少数跨模态方法使用单全局嵌入丢失了细粒度语义信息。

Method: 教师-学生框架，建立3D点到教师patch特征的稠密跨模态对应，逐点匹配潜在空间特征，无需掩码、负样本或解码器。

Result: 在MSR-Action3D等四个基准上优于模态内和全局跨模态基线，与更重的4D方法竞争，且稠密表示具有更好的迁移性和标签效率。

Conclusion: 稠密跨模态对应是关键增益来源而非教师模态本身，结合更小编码器即可匹配重型方法性能。
...
TL;DR: Restore3D是一个同时恢复破损3D物体形状和纹理的框架，利用多视图图像和掩码自感知模块实现高质量修复。


<details>
  <summary>Details</summary>
Motivation: 现有3D修复方法主要关注几何形状补全，忽视了纹理恢复，且难以处理复杂多样的物体。作者旨在同时恢复形状和纹理。

Method: 提出自动化数据生成流水线合成成对的不完整-完整样本；设计带深度感知掩码修正器的掩码自感知模块（Mask Self-Perceiver）；采用由粗到精的重建策略从多视图图像恢复带纹理的3D网格。

Result: 在合成和真实破损物体基准测试上，Restore3D在多视图修复质量和纹理网格重建方面优于代表性的修补、补全和重建基线方法。

Conclusion: Restore3D有效实现了破损3D物体形状和纹理的同时恢复，生成了高分辨率、语义连贯且视图一致的多视图图像及高质量纹理网格。
...
TL;DR: 提出SPECSIA-15K数据集和DraViE模块，用于从单张2D绘画生成3D动画时去除新颖视角伪影，提高时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于绘画的3D动画流程中，样本级2D精细化容易过拟合到观察视角，无法修正新颖视角下的投影伪影。

Method: 构建了SPECSIA-15K配对风格化数据集（14,980对伪影/修正目标），并提出DraViE轻量级即插即用模块，利用数据级先验去除新颖视角伪影。

Result: 在新颖视角保真度和时间一致性上取得一致提升，且每个角色的适配成本低于样本级微调。

Conclusion: DraViE模块能有效解决单图3D动画中的新颖视角伪影问题，兼具高效性和泛化能力。
...
TL;DR: NoPA提出用非参数分布替代单高斯分布来表示3D场景中的物体，在保持实时推理速度的同时更好地保留3D几何细节，并设计了基于最大均值差异的合并策略和关系传播机制，显著提升了3D场景图生成的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景图生成方法要么计算量大无法实时运行，要么使用单高斯近似物体导致几何细节丢失和错误合并，作者希望解决这一精度与速度的权衡问题。

Method: 1）用非参数分布（固定粒子集）表示每个物体，保留3D几何信息；2）基于最大均值差异（MMD）和核密度估计设计合并策略，实现鲁棒的在线物体合并；3）通过高亲和力物体之间的关系传播来纠正错误分类导致的关系损失。

Result: NoPA在保持实时推理速度的同时，大幅优于现有3D场景图生成方法。

Conclusion: 非参数分布表示能有效保留3D几何细节，结合MMD合并策略和关系传播，可以显著提升实时3D场景图生成的精度。
...
TL;DR: GEAR-Seg 提出了一种解耦的智能体框架，将推理分割任务拆分为类别无关分割、语义描述和LLM推理三个显式步骤，实现了可解释的零样本推理分割，并构建了大规模基准GEAR-131K。


<details>
  <summary>Details</summary>
Motivation: 现有端到端推理分割模型将感知和推理混杂在黑箱中，缺乏可解释性和可扩展性，亟需一种解耦、可追踪的方案。

Method: 将视觉像素转化为密集属性文本，通过解耦类别无关分割、语义描述和LLM推理三个模块，形成显式、可追踪的逻辑链，实现零样本推理分割。

Result: 在多种推理分割和细粒度指代分割基准上取得了极具竞争力的零样本性能；构建的GEAR-131K数据集包含超过3.8万张图像、65.6万对QA-mask，并引入面向复杂现实操作推理的多面分类体系。

Conclusion: GEAR-Seg不仅实现了高性能零样本推理分割，还可作为可扩展的数据引擎；蒸馏实验表明，仅通过自动化流程监督的轻量模型就能接近昂贵人工标注基线的上界性能。
...
TL;DR: 提出轻量级控制适配框架EcoSim，通过FiLM层调制中间特征，为预训练扩散/自回归交通模型添加多模态控制能力（草图、潜在行为编码、文本），仅需不到1%的配对数据即可实现可控交通仿真。


<details>
  <summary>Details</summary>
Motivation: 现有可控交通仿真方法需要大量标注数据重新训练生成模型，缺乏轻量级的多模态控制适配方案。

Method: 基于身份初始化的FiLM（特征线性调制）层，在预训练扩散模型和自回归模型的中间特征上进行调节，实现草图、潜在行为编码和文本三种控制模态的轻量级适配。

Result: 在Waymo Open Sim Agents Challenge上，仅使用不到1%的配对控制数据即实现强可控性，支持反事实场景生成和长尾合成，保持稳定的闭环驾驶真实性和安全性。

Conclusion: 轻量级适配框架为预训练生成模型解锁了可控交通仿真的新可能，实现目标场景的定向生成。
...
TL;DR: 本文提出EgoGapBench基准，用于评估多智能体场景中的自我中心动作选择能力，发现MLLMs在此任务上远逊于人类，且仅靠第一人称数据训练难以弥补这一差距。


<details>
  <summary>Details</summary>
Motivation: 现有自我中心基准主要依赖第一人称视角数据，难以单独评估"自我中心视角理解"能力，因为理解第一人称输入和采取自我中心视角是可分离的能力，尤其在缺少身体线索或有其他智能体在场时。

Method: 构建EgoGapBench诊断基准，定义自我中心动作选择（EAS）任务，要求模型从智能体自身视角在多智能体场景中选择合适动作。

Result: 人类在该基准上表现可靠，而开源和闭源MLLMs表现显著更差，系统性地选择了其他可见智能体的动作；微调现有自我中心数据无法弥补差距甚至有害，微调EgoGapBench训练数据可提升准确率但仍未达到人类水平。

Conclusion: EAS难以仅从第一人称视角数据中习得，MLLMs不仅需要场景理解评估，还需要评估和训练自我中心动作选择能力。
...
TL;DR: 提出BrainFIBRE，首个脑微观结构基础模型，通过自监督部分信息分解（SPID）和反事实候选构建（CCC）从NODDI图谱中学习可迁移的微观结构表征，在多种预测任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: NODDI生成的三种微观结构图谱（NDI、ODI、FWF）富含信息，但标准表征学习难以解耦各图谱的独特信息及其共享与协同交互，亟需一种能有效整合这些多模态微观结构信息的基础模型。

Method: 提出BrainFIBRE基础模型，包含两大创新：1) SPID（自监督部分信息分解）首次将PID引导的多模态学习扩展到自监督范式；2) CCC（反事实候选构建）通过模态丢弃和交换扰动模态间对齐，为混合专家架构提供对比信号，实现无需下游标签即可解耦独特、协同和冗余信息。

Result: 在55,592名UK Biobank参与者上预训练，在高加索和亚洲队列中，在预测年龄、性别、脑血管/神经退行性标志物及认知等任务上达到SOTA性能，且表征具有神经生物学可解释性。

Conclusion: BrainFIBRE建立了微观结构层面神经影像分析的通用基础，能揭示任务和队列特异性的交互模式。
...
TL;DR: 提出Caption Bottleneck Models (CaBM)，用LMM生成的自由文本描述替代传统概念瓶颈层，实现无泄漏、无需预定义概念集的可解释分类。


<details>
  <summary>Details</summary>
Motivation: 传统概念瓶颈模型依赖预定义概念集（专家标注或LLM生成），存在概念集固定、标签偏差和信息泄漏问题，亟需更灵活、无泄漏的可解释框架。

Method: 用多模态大模型（LMM）生成图像的文本描述（caption），然后基于这些自由文本训练分类器，从而用自然语言层替代预定义概念瓶颈层。

Result: 在细粒度和粗粒度基准上，CaBM在保持可解释性的同时达到了有竞争力的准确率，并能够自动发现高质量、数据集特有的概念。

Conclusion: CaBM通过自由文本瓶颈成功规避了预定义概念集的限制和信息泄漏问题，是一种兼具精度与可解释性的有效框架。
...
TL;DR: 提出基于边缘的位姿优化框架EPO，通过可微边缘对齐替代传统BA优化，在保持速度优势的同时提升3D重建几何精度，且内存消耗低至可在消费级硬件运行。


<details>
  <summary>Details</summary>
Motivation: 3D基础模型虽推理速度快，但几何精度不及传统SfM管线。现有后处理BA优化需提取特征track，牺牲了速度优势。

Method: 提出EPO框架，利用可微边缘图对齐作为几何优化的代理，完全避免特征提取和track构建，实现端到端优化。

Result: 在多个数据集和任务上，EPO达到或超过BA类方法的精度，同时显著降低运行时间和内存消耗，能在消费级硬件上运行。

Conclusion: EPO是一种高效、低内存的3D几何优化替代方案，适合在资源受限设备上部署。
...
TL;DR: 本文提出Active Spatial Guidance (Guidance)，一种无需显式注入位置编码的ViT训练方法，通过在训练时对最后一层patch tokens施加2D坐标回归损失来学习空间结构，在分类、分割和深度估计任务上优于传统位置编码方法。


<details>
  <summary>Details</summary>
Motivation: 针对ViT依赖显式位置编码来打破自注意力置换不变性的问题，探索能否从数据中自然学习空间组织结构，而非人为注入。

Method: 提出Active Spatial Guidance (Guidance)，训练时在ViT编码器最后一层patch tokens上添加辅助2D坐标回归损失，指导头仅在训练阶段使用，推理时移除，模型完全不含位置注入模块。

Result: 在ImageNet-100分类、ADE20K语义分割和Hypersim单目深度估计上一致优于学习绝对位置编码和旋转位置编码等基线；在分辨率迁移和多分辨率训练下鲁棒性更强。

Conclusion: ViT的空间归纳偏置不需要通过架构注入位置编码，可以通过训练时监督来塑造，证明了数据驱动空间学习的可行性。
...
TL;DR: 提出几何感知可变形聚合（GADA）方法，通过可变形偏移和隐式置信度加权机制解决高斯泼溅中由不确定几何导致的像素级错位问题，实现更优的高频细节保留和2.13倍速度提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于warping的高斯泼溅方法因几何不确定性导致像素级空间错位，限制了对薄结构和高频细节的校正效果。

Method: 提出GADA，包含：（1）基于可变形偏移的迭代校正模块，主动修正空间错位；（2）隐式置信度加权机制，选择性抑制不可靠的多视图融合证据。

Result: 优于先前基于warping的高斯泼溅方法，保持高频质量的同时实现2.13倍FPS加速。

Conclusion: 通过主动校正空间错位和选择性融合多视图信息，有效提升了高斯泼溅在复杂几何场景下的表现和效率。
...
TL;DR: 本文提出一种混合方法（语义区域检测+生成式LLM），在历史亚美尼亚报纸的阅读顺序重建任务中，比最强几何基线减少76%的排序错误，并发布了66页标注数据集和专用OCR模型。


<details>
  <summary>Details</summary>
Motivation: 历史亚美尼亚报纸布局复杂且语言资源稀缺，现有方法难以准确重建阅读顺序，需要一种高效的数据引导策略。

Method: 对比了几何启发式、YOLO布局解析、端到端文档模型ECLAIR，以及混合方法（语义区域检测+生成式LLM）。

Result: 混合方法错误率最低，排序错误减少76%，在多页和噪声OCR场景下保持鲁棒性。

Conclusion: 该方法并非面向生产环境，而是作为低资源场景下的快速数据标注引导策略。
...
TL;DR: 提出ReVisIT框架，通过检索图像-标签对作为视觉思维单元，实现无训练的动态视觉推理，在多个基准上达到或接近SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有"用图像思考"的方法依赖生成（调用外部工具、合成代码、生成新图像）或需要昂贵训练，而检索式方法虽无需训练但未被充分探索。本文旨在填补这一空白。

Method: ReVisIT框架包含：结构化类别定义、每查询多模态检索示例、交替用户/助手注入示例、联合多属性解码。组件可灵活组合，退化优雅。

Result: VL-ICL Bench上Qwen3-VL-30B-A3B达98.5%（接近72B LLaVA-OneVision的98.7%）；Bongard-OpenWorld上GPT-4.1提升26.1点；MAAC-Bench上跨三个主干网络获得4-6点宏观增益。

Conclusion: 检索+轮次注入是通用杠杆，结构化定义按需适配；83%的检索增益来自检索质量而非示例存在；发布了MAAC-Bench基准及基于规则验证的LLM审核协议。
...
TL;DR: 本文提出一种基于扩散模型的多类常态框架，用于无监督异常检测，特别是在复制检测图案（CDP）认证中，通过类条件ControlNet和双模板掩码技术，仅使用真实样本训练即可检测伪造品。


<details>
  <summary>Details</summary>
Motivation: 传统的重建式生成模型在多类常态建模中难以同时捕捉多个内部分布并产生可比较的异常分数，而在CDP认证中真假样本视觉相似但存在细微差异，需要更精细的检测方法。

Method: 1) 使用类条件ControlNet仅在真实CDP样本上训练；2) 通过重建误差检测伪造品；3) 提出双模板掩码技术，隐藏输入模板的互补区域，仅对 withheld 像素进行评分，减少对可见二进制结构的依赖。

Result: 在Indigo 1 x 1 Base数据集上，该方法在多类真实vs伪造评估中优于传统和改编的生成基线，且无需使用伪造样本进行训练或阈值校准。

Conclusion: 扩散模型结合条件控制和掩码策略能有效解决多类常态异常检测问题，在CDP认证中展现出优越性能。
...
TL;DR: 提出CPF-GCD框架，通过可学习的视觉基元构建低秩组合特征空间，提升开放世界广义类别发现性能。


<details>
  <summary>Details</summary>
Motivation: 现有GCD方法主要关注聚类目标设计，忽视了标准视觉主干网络产生的高秩、纠缠的token表示不适合无监督发现潜在概念和结构这一关键瓶颈。

Method: 提出Compositional Primitive Fields (CPF)，在骨干网络和头部之间插入空间场机制，通过低秩基元混合重写噪声patch tokens，将图像分解为可复用的原子部件及其空间布局。

Result: CPF作为通用即插即用模块，在多种GCD基线上持续提升性能，验证了低秩组合结构归纳偏置的有效性。

Conclusion: 识别和利用低秩组合结构是开放世界识别中关键的归纳偏置，使新类别能自然地作为共享词汇表上的新激活模式出现。
...
TL;DR: 提出Anom-π闭环框架，将视频异常理解重构为主动序贯决策过程，通过iDPO和AEI实现策略对齐，仅2B参数即超越大规模SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 现有被动式视频异常理解方法存在观测别名问题（静态采样无法区分语义不同的事件），需要更主动的解耦策略。

Method: 提出Anom-π闭环框架：(1) 将视频理解重构为动态环境中的主动序贯决策过程；(2) 结合内部认知推理与策略性证据获取；(3) 设计时序原子操作（局部回退、时间扩展、细粒度采样）；(4) 提出Interactive Direct Preference Optimization (iDPO)实现轨迹级策略对齐；(5) 设计Active Evidence Inquiry (AEI)效用平衡任务成功、信息获取和交互成本。

Result: 仅2B参数即达到高竞争力性能，在复杂场景下显著超越大规模SOTA VAU模型。

Conclusion: 主动式、闭环的决策范式能有效解决视频异常理解中的观测别名问题，且小模型通过策略学习可超越大模型。
...
TL;DR: 本文提出了一个用于连续冠层高度变化估计的CHC数据集，并引入了不确定性感知变化回归任务，评估了多种地理空间基础模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法将冠层高度变化视为二元变化检测问题，忽略了变化的连续性和标签的不确定性。

Method: 构建了CHC数据集（3米分辨率，覆盖西班牙10598平方公里），包含连续冠层高度差异和空间分辨率不确定性；提出不确定性感知变化回归任务及微调GFMs的策略。

Result: 评估了多种最先进的地理空间基础模型，指出了连续冠层高度变化估计的前进方向和剩余挑战。

Conclusion: 连续变化回归和不确定性建模对于遥感监测冠层高度变化至关重要，现有GFMs仍面临挑战。
...
TL;DR: 本文探讨了在无训练引导（TFG）中，不同预测目标（$ε$、$v$、$x$预测）对扩散模型生成样本流形保真度的影响，提出$x$预测在保持样本在流形上最可靠，并引入Child FID指标来评估流形损伤。


<details>
  <summary>Details</summary>
Motivation: 无训练引导需要在早期高噪声步骤中估计干净图像，而$ε$和$v$预测的估计误差容易导致引导偏离数据流形；$x$预测直接输出干净图像，可避免此误差来源，但缺乏系统分析。

Method: 理论分析不同预测目标对估计准确度的影响；提出guided-class FID (Child FID)指标；在细粒度鸟类基准和风格迁移任务上实验验证。

Result: $x$预测在保持引导样本在流形上最可靠，优于$ε$和$v$预测，是TFG最强的基础。

Conclusion: $x$预测是无训练引导的最佳预测目标选择，Child FID能有效暴露标准评估忽略的流形损伤。
...
TL;DR: 提出LRPO方法，通过多推理轨迹蒸馏语义优势为语言化的异常经验先验，无需参数更新即可实现视频异常检测，在三个基准数据集上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有VAD方法依赖大规模标注或专家先验，希望以最少人工干预获取异常知识。

Method: 提出Linguistic Relative Policy Optimization (LRPO)，从多个推理轨迹中蒸馏组相对语义优势，转化为语言表达的异常经验先验，通过注入上下文来调整模型输出分布（无需参数更新）。构建通用经验（跨场景可迁移）和场景经验（上下文相关）两种互补表示，并引入异常对齐奖励引导轨迹优化。

Result: 在XD-Violence、UCF-Crime和UBnormal数据集上，在无调参设置下显著优于现有SOTA方法。

Conclusion: LRPO通过语言化经验先验实现了高效的无调参视频异常检测，减少了人工干预需求。
...
TL;DR: 提出DART方法，通过难度自适应路由和结构化推理，弥合零样本视频时序定位中的推理差距，在零样本和分布外场景下达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有零样本VTG方法依赖帧-查询特征匹配，处理简单事件有效，但难以理解复杂多阶段查询中的时序顺序和因果结构，存在推理差距。

Method: 提出DART：1) 查询条件DPP选择多样化关键帧并计算谱熵作为难度指标；2) 简单查询走Fast路径直接预测；3) 复杂查询走Slow路径，使用Temporal Markup Prompting进行全局事件分析、逐帧时序角色标注和边界提取。

Result: 在Charades-STA和ActivityNet Captions上，零样本和多种分布外场景均达到SOTA，mIoU最高提升3.5个点，使用帧数减少7倍以上。

Conclusion: DART通过难度自适应路由结合结构化推理有效弥补了零样本VTG中的推理差距，实现了高效且准确的时序定位。
...
TL;DR: ABot-M0.5提出一种新的世界动作模型，通过三级对齐（时间粒度、动作空间、训练-测试一致性）提升移动操作机器人的长程任务成功率与精细控制精度。


<details>
  <summary>Details</summary>
Motivation: 现有VLA策略缺乏世界建模，世界动作模型与移动操作的时空结构对齐差，存在接触动力学缺失、动作分布冲突和长程误差累积问题。

Method: 1) 引入中间隐式动作对齐时间粒度；2) 设计双级Mixture-of-Transformers解耦导航与操作动作空间；3) 提出dream-forcing训练策略提升推理一致性。

Result: 在移动操作与精细操控基准上达到SOTA，长程任务成功率和精细控制精度均领先。

Conclusion: 粒度对齐、动作解耦、推理一致的世界动作建模对移动操作机器人至关重要。
...
TL;DR: LUMA是一个轻量级、主干无关的掩码Transformer头部，可公平比较不同主干网络在图像分割中的表现，发现普通ViT在效率上占优，预训练目标比架构本身更影响分割质量。


<details>
  <summary>Details</summary>
Motivation: 不同Transformer主干网络在图像分割中的比较存在混淆，因为每个主干都搭配了不同的解码器、训练配方和预训练方式，导致报告的性能差异很少真正反映主干网络本身。

Method: 提出LUMA（Lightweight Universal Mask Adapter），一种轻量级、主干无关的掩码Transformer头部，将任何主干网络视为黑盒特征提取器，通过廉价交叉注意力让查询读取其特征。固定该头部后，在统一配方下对20种主干网络、11种预训练方案和多种分辨率进行基准测试。

Result: LUMA在更低成本下匹配了最先进的高效ViT分割器EoMT的精度；发现"高效"令牌混合器在高分辨率下并未实现效率提升，普通ViT在所有分辨率下保持吞吐量帕累托前沿；预训练目标（而非架构）是分割质量的主要决定因素。

Conclusion: 主干网络比较需要公平且统一的评估框架；预训练目标比架构选择对分割质量影响更大；普通ViT在效率上优于所谓的"高效"令牌混合器。
...
TL;DR: 提出Imprint，一种以交互为中心的记忆框架，通过在线记忆压缩而非摘要来构建长期自我中心问答的记忆，显著提升QA准确率和检索效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法将记忆构建为层级文本摘要，优化的是描述性压缩而非检索性能，重复交互被吸收为粗略描述，难以进行长期证据聚合。

Method: 将观测数据表示为结构化交互记录，利用人类记忆巩固信号（重复性、近期性、独特性）选择性保留和压缩交互，形成紧凑的检索导向记忆。

Result: 在EgoLifeQA基准上，QA准确率从31.0%提升至35.8%，证据导向回答提升6倍，内存占用减少2.3倍，检索延迟降低11.8倍。

Conclusion: 记忆压缩为长期自我中心问答提供了可扩展且检索有效的解决方案。
...
TL;DR: 本文提出了一种面向自动驾驶数据集设计的战略框架，强调从诊断问题出发，选择最小数据操作符来填补差距，并以KITScenes数据集家族为案例验证。


<details>
  <summary>Details</summary>
Motivation: 现有文献仅描述数据集包含什么，而非如何战略性地设计 impactful 数据集，这限制了资源有限的中小实验室和初创企业。

Method: 提出一个战略框架：诊断研究问题是被数据问题还是评估问题所阻塞 → 选择最小数据操作符填补差距 → 仅在无更廉价方案时记录新数据。涵盖差距识别、操作符选择、传感器套件设计、标注策略。

Result: 以KITScenes数据集家族作为运行案例，验证该框架的有效性，并公开数据集。

Conclusion: 数据集设计应遵循诊断驱动、最小成本原则，该框架可帮助资源有限的团队更高效地创建 impactful 数据集。
...
TL;DR: 本文提出ISPA框架，通过将部分注意力层从全局注意力转为局部注意力，并将历史上下文蒸馏到模型权重中，实现对KV缓存的压缩（最多去除50%），同时保持近无损的视觉质量。


<details>
  <summary>Details</summary>
Motivation: 自回归流式模型在长视频生成中KV缓存线性增长导致内存过载和推理吞吐下降，现有丢弃冗余token的压缩方法会破坏长程依赖，造成时间闪烁和身份丢失。

Method: ISPA框架：在预热阶段监控全局与局部注意力的输出差异，在转换点通过求解闭式最小二乘问题计算实例特定的权重调制，将部分层从全局注意力（F-Layer）转为局部注意力（L-Layer），从而将历史上下文"吸收"到模型权重中。

Result: 在1.3B到14B参数规模的架构上，ISPA可以去除多达50%的KV缓存，且视觉质量接近无损。

Conclusion: 本文提出的参数化记忆压缩视角（从丢弃到蒸馏）为流式生成模型的KV缓存管理提供了新方向，鼓励未来研究超越外部token级缓存管理，探索参数化记忆整合。
...
TL;DR: 提出PartialVisGraph，一种基于超图框架的骨架动作识别方法，专门解决视野受限场景下关节可见性丢失问题，通过可学习的虚拟超边和单头样本自适应Transformer，在处理部分可见骨架数据时取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有骨架动作识别方法几乎都假设输入数据完整且干净，但在实际部署（如第一人称视觉、密集监控、可穿戴设备等）中，有限的视野经常导致大量关节可见性丢失，造成严重的性能下降，这一关键问题尚未被充分探索。

Method: 1) 构建可表达的超图，引入可学习虚拟超边形成软关联矩阵，捕获高阶依赖关系；2) 提出单头样本自适应Transformer（Single-Head Sample-Adaptive Transformer），自适应地将关节特征聚合到超边上，并显式融入可见性先验来选择性门控信息流，防止遮挡关节干扰可靠特征传播；3) 建立带真实视野模拟基准的评估协议。

Result: 在NTU RGB+D 60和120数据集上，PartialVisGraph在部分可见性条件下持续达到最优准确率，在严重视野受限子集上相比强基线提升高达68.8%，在全可见性设置下也保持优越性能。

Conclusion: 该方法为无约束环境下的可部署骨架动作理解提供了一条有原则且实用的路径。
...
TL;DR: 提出了AV-SyncBench基准，首个完全分离音视频同步中时序与语义评估的基准，包含3269个视频和38390个样本，覆盖10个场景和5个挑战任务。


<details>
  <summary>Details</summary>
Motivation: 现有音视频特征提取评估协议存在维度偏差，且数据构建耦合，无法独立评估时序一致性和语义一致性。

Method: 从野外视频构建基准，覆盖语音、音乐、声音三大类别及10个场景、5个挑战任务；数据经自动过滤和人工验证确保屏幕内声源。

Result: 构建了3269个视频和38390个样本的数据集，评估了5个代表性模型。

Conclusion: AV-SyncBench能够解耦时序和语义维度，为音视频同步评估提供更全面的基准。
...
TL;DR: 提出了一种边缘约束的细粒度定位损失（EFL），通过编码表格结构先验来提升表格结构识别中行列边界的定位精度，在实时检测器上实现高效且数据高效的表格结构识别。


<details>
  <summary>Details</summary>
Motivation: 现有检测方法将表格元素视为通用物体，忽略了行列边界在表格结构中的不对称重要性，导致微小边界误差会传播为结构不一致。

Method: 提出Edge-constrained Fine-grained Localization loss (EFL)，对行元素强调水平边界、列元素强调垂直边界；基于D-FINE实时检测器实现，仅训练时生效，推理不变。

Result: 在PubTables-1M和两个私有数据集上，相比RT-DETRv2、YOLOv10-11等基线提升高达+1.6 GriTS点，同等推理速度下更优；仅需2k-3k标注数据即可保持鲁棒精度。

Conclusion: 编码表格结构先验的边界损失能有效提升TSR性能，且方法数据高效、无推理开销。
...
TL;DR: FlexDepth提出了一种自监督单目深度估计方法，通过两阶段静态-动态解耦训练策略和尺度驱动解码器，在复杂驾驶场景中以极低计算量实现高精度深度估计，最小模型仅需0.7 GFLOPs。


<details>
  <summary>Details</summary>
Motivation: 现有自监督单目深度估计模型局限于单一尺度，在复杂驾驶环境中性能下降严重，且处理动态目标的网络过于复杂，难以部署在资源受限的车载边缘设备上。

Method: 提出FlexDepth系列模型，采用两阶段静态-动态解耦训练策略独立评估静态背景和动态目标的置信度，并设计尺度驱动解码器(SDD)根据尺度大小动态选择组件以实现高效特征融合。

Result: 在标准驾驶基准上达到SOTA性能，最小模型Flex-Nano仅需0.7 GFLOPs，在移动平台上达到37.6 FPS，且具有优异的零样本泛化能力。

Conclusion: FlexDepth在不依赖辅助信息的情况下，以最小计算开销实现了任意尺度下的最先进性能，适用于资源受限的汽车边缘设备实时感知。
...
TL;DR: 提出一种无需训练的多类产前超声异常分类与定位框架，仅需每类少量参考图像即可工作。


<details>
  <summary>Details</summary>
Motivation: 产前异常分类定位对胎儿健康至关重要，但超声诊断因异常低发和高异质性而困难。现有深度学习方法依赖大规模标注数据难以获取，少样本学习需要微调，实用性受限。

Method: 提出训练无关框架，包含三个组件：(1)多粒度原型记忆库；(2)原型驱动软合并机制；(3)类感知精细化策略。

Result: 在包含1149例、2357张图像、9个类别的多中心产前超声数据集上验证，方法优于对比方法。

Conclusion: 提出首个无需训练的多类产前超声异常分类与定位框架，仅需少量参考图像即可有效工作。
...
TL;DR: 本文首次提出利用盲扫超声视频估计胎儿出生体重，通过视觉语言基础模型驱动的关键解剖帧选择和冗余感知特征压缩模块，在839名患者数据上实现MAE 161.3克，优于传统Hadlock方法。


<details>
  <summary>Details</summary>
Motivation: 传统胎儿体重估计依赖操作者经验，在资源匮乏场景下准确度受限。本文旨在通过盲扫超声视频实现操作者无关的自动体重估计。

Method: 提出基础模型驱动的关键解剖帧选择框架，包含：（1）基于视觉语言基础模型的解剖引导帧选择模块；（2）冗余感知特征压缩模块，在保留任务相关信息的同时减少时间冗余。

Result: 在839名患者的前瞻性数据上，MAE为161.3克，90.23%和100%的病例分别落在10%和15%绝对百分比误差内，优于Hadlock估计和强基线方法。

Conclusion: 盲扫超声视频结合基础模型可实现准确且操作者独立的胎儿体重估计，具有在低资源场景临床应用的潜力。
...
TL;DR: 本文提出GaussianFusion，一种基于3D高斯表示的多模态融合框架，替代传统BEV范式，在连续3D高斯空间中统一多模态特征，显著提升3D感知性能与效率。


<details>
  <summary>Details</summary>
Motivation: 传统BEV表示采用离散网格，导致细节丢失严重，限制了特征对齐和跨模态信息交互。

Method: 提出基于3D高斯表示的多模态融合框架，设计前向投影多模态高斯初始化模块和共享跨模态高斯编码器，通过注意力机制迭代更新高斯属性。

Result: 在nuScenes数据集上，3D目标检测超越BEVFusion 2.6 NDS；3D语义占用任务以30%高斯数量实现1.55 mIoU提升，并达到450%加速。

Conclusion: GaussianFusion作为任务无关模型，其统一高斯表示自然支持多种3D感知任务，展现出良好的通用性和鲁棒性。
...
TL;DR: 本文提出将不确定性放牧（UHerding）主动学习策略首次扩展到级联表格提取流水线（TD→TSR），并设计了两种流水线感知变体（RankFusion和CAPA），实验表明该方法能有效减少标注成本且性能优于基线。


<details>
  <summary>Details</summary>
Motivation: 表格提取依赖级联流水线（TD→TSR），但TSR需要细粒度结构标注，成本高昂。现有主动学习策略多为单模型设计，未考虑级联架构中的跨阶段依赖关系。

Method: 将UHerding从图像分类首次适配到级联目标检测流水线；提出两种流水线感知扩展：RankFusion（双流形覆盖检测和结构表示空间）和CAPA（阶段门控+逐任务不确定性校准）。

Result: 在4个数据集、多种标注预算下，UHerding优于所有基线；RankFusion期望增益更高但方差大，CAPA最稳定，在3/4数据集上超越标准UHerding。

Conclusion: 主动学习策略可有效降低表格提取流水线的标注成本，考虑流水线内部依赖关系的变体（特别是CAPA）能带来更一致且稳定的性能提升。
...
TL;DR: LeVLJEPA是一种全新的非对比式端到端视觉语言预训练方法，通过跨模态预测和分布正则化学习，无需负样本、温度参数等，在密集语义特征上优于对比式方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言预训练主要采用对比式目标函数，但视觉编码器的角色已从零样本分类器转变为冻结的视觉骨干网络，需要输出完整的网格令牌而非单个池化嵌入，对比式方法在密集语义特征上表现不足。

Method: 提出LeVLJEPA，这是首个完全非对比式的端到端视觉语言预训练方法。通过跨模态预测学习，使用停止梯度目标和每模态分布正则化，无需负样本、温度参数、动量编码器或教师-学生调度。

Result: 作为冻结的视觉语言模型骨干，LeVLJEPA在GQA、VQAv2和POPE上使用两种不同语言模型时均表现最佳；在语义分割任务上优于对比式基线；在线性探测等全局读任务上与对比式方法持平。

Conclusion: 非对比式预训练是生成密集语义视觉特征的有效方法，建立了非对比式视觉语言预训练的新范式。
...
TL;DR: 提出FrameONE，一个统一的多视角超声心动图关键帧检测框架，通过层次化运动建模实现跨视角泛化。


<details>
  <summary>Details</summary>
Motivation: 现有超声心动图关键帧检测方法依赖视图特定建模、辅助标注或密集视觉建模，泛化能力差。多视角建模中，关键帧检测由共享心脏运动驱动，但不同视图的外观差异和运动模式使统一建模困难。

Method: 提出FrameONE框架，包含层次化运动建模策略：(1) 视图内多任务学习减少外观偏差，促进运动聚焦表示；(2) 视图间通用运动学习模块分离视图无关动态与视图特定模式，实现共享且灵活的运动表示学习。

Result: 在涵盖四个标准视角的25,872个视频上，FrameONE达到了最先进的关键帧检测精度，并展现出强大的跨视图泛化能力。

Conclusion: FrameONE通过层次化运动建模有效解决了多视角超声心动图关键帧检测中的外观差异和运动模式多样性问题，验证了跨视图通用建模的可行性。
...
TL;DR: Pano2World提出从单张室内全景图直接生成可自由探索的3D高斯场景，通过全景扩散模型和潜特征适配器实现多视图一致性，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么存在错误累积和多步骤流水线问题，要么受限于视频生成模型的连续轨迹约束，无法灵活覆盖多方向场景。

Method: 1) 从源全景图重建粗3D高斯代理并渲染引导全景图；2) 全景扩散模型通过视图感知注意力路由联合去噪所有目标视图；3) 引入潜特征适配器直接蒸馏多视图隐特征为场景潜码，避免VAE解码信息损失。

Result: 在多位置全景新颖视角合成基准上显著优于现有方法。

Conclusion: Pano2World能有效将单张室内全景图转换为可持久化、可探索的3D高斯场景，支持自由视点导航。
...
TL;DR: 本文提出了大规模统一关键点数据集MegaKPT和基于DINOv3的通用关键点检测模型GKDT，支持视觉和文本提示，在22个测试集上展现出强大的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 随着预训练视觉和语言模型的发展，计算机视觉正从窄领域向开放领域识别转变，构建更强大且通用的关键点检测模型以支持多样化任务变得越来越重要。

Method: 1) 构建大规模统一关键点数据集MegaKPT（130万+实例，29个现有数据集）；2) 提出基于DINOv3的Transformer模型GKDT，支持视觉提示、文本提示或两者结合；3) 提出混合模态提示训练和动态重要性采样等训练策略。

Result: 在22个测试集上，单个GKDT模型在广泛类别上展现出强大的性能和泛化能力，大多数类别PCK@0.1准确率超过90%。

Conclusion: GKDT模型在通用关键点检测任务上表现优异，具有高实际应用价值，数据集和模型将开源。
...
TL;DR: 提出解耦引导(DeGu)框架，通过双独立引导流分别处理主体身份和场景上下文，解决文生图个性化中的条件纠缠问题，提升保真度与可编辑性平衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过同一条件路径编码主体身份和场景上下文，导致两者竞争注意力资源，产生保真度-可编辑性权衡。作者希望解耦这两个条件，消除相互干扰。

Method: 提出Decoupled Guidance (DeGu)框架：1) 双独立引导流分别处理主体身份和场景上下文；2) 空间混合机制动态融合两流，确保各自在语义相关区域操作不干扰；3) 即插即用，无需修改基础模型。

Result: DeGu可应用于多种现有个性化方法和骨干网络（包括流匹配扩散变换器DiT），持续提升个性化性能，并支持推理阶段控制保真度-可编辑性平衡。

Conclusion: 通过解耦引导流有效解决条件纠缠问题，打破保真度与可编辑性之间的固有权衡，是一种通用且高效的改进方案。
...
TL;DR: 本文提出Soft Mixture-of-Recursions (SoftMoR)方法，通过可学习的token级混合权重灵活组合递归步骤的中间表示，构建更深更强的视觉Transformer，在ImageNet-1K上用少量参数显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有递归Transformer方法未充分利用递归过程中产生的中间表示，导致增加递归深度无法可靠提升视觉Transformer性能。

Method: 提出SoftMoR，学习token级的混合权重，将各递归步骤的输出进行软组合；实例化为SR-ViT（Soft Recursive Vision Transformer）。

Result: SR-ViT-S在ImageNet-1K上递归深度从1增加到4时，top-1准确率从79.83%提升至82.48%，仅增加1.7M参数，性能超越DeiT-B且仅使用其约27%的参数。
...
TL;DR: 本文提出了一种基于平坦最小值（Flat Minima）优化的轻量级训练框架，通过受控高斯扰动和周期性重初始化来缓解3D高斯泼溅在稀疏视角监督下的过拟合问题，提升新视角合成的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 3DGS在稀疏输入视角监督下容易过拟合观测图像，对未见视角泛化能力差。

Method: 将平坦最小值优化适配到3DGS，引入各向异性感知的高斯扰动和周期性重初始化策略，稳定优化过程。

Result: 在LLFF和Mip-NeRF360数据集上，稀疏视角监督下定量指标和感知质量均得到提升，重建更清晰、稳定、泛化更好。

Conclusion: 该方法无需修改架构即可无缝集成现有3DGS流水线，有效缓解稀疏视角过拟合问题。
...
TL;DR: SpiralFovea是一种无需参数、输入自适应的分词器，通过内容驱动的热点锚点和多尺度螺旋环，以更少的token（≤78个）替代标准ViT的196个网格token，在细粒度基准上提升1.7-2.1个百分点的准确率，同时减少60%的输入token和84%的自注意力FLOPs。


<details>
  <summary>Details</summary>
Motivation: 现有自适应推理技术（如早退、MoE路由等）都改变了模型行为，但输入到backbone的tokenization仍然是固定的网格化方式，忽略了图像内容的自适应性。作者认为这是一个被忽视的杠杆。

Method: 提出SpiralFovea，一种参数无关、输入自适应的tokenizer。token的身份、位置、尺度和数量均由局部视觉熵决定，且在查询任何backbone参数之前完成选择。围绕内容驱动的热点锚点，使用多尺度螺旋环生成≤78个patch，替代标准ViT的196个网格patch。

Result: 在四个细粒度基准上，准确率提升+1.7-2.1个百分点，输入token减少60%，每层自注意力FLOPs减少84%，吞吐量提升18-29%。消融实验表明，增益大小与backbone的全图位置先验强度呈反比，自监督基础模型是输入自适应tokenization最有价值的场景。

Conclusion: 输入自适应的tokenization是一个有效的优化方向，尤其在自监督基础模型上价值最大。
...
TL;DR: DeWorldSG是一个从RGB-D序列生成时空鲁棒3D语义场景图的新框架，通过概率3D节点表示和时空证据聚合，在3DSSG和ReplicaSSG数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法因3D物体表示不稳定和逐帧推理导致的缺失关系，难以构建可靠的3D场景图。

Method: 通过深度引导滤波估计实例级几何3D高斯分布，将每个物体表示为概率3D节点；跨物体对聚合时空证据，并利用世界模型(V-JEPA 2)的上下文先验细化关系。

Result: 在3DSSG和ReplicaSSG数据集上取得SOTA性能，triplet召回率提升77.4%，predicate召回率提升23.2%。

Conclusion: 该方法生成时空一致的场景结构，适用于机器人操作和AR应用，代码和模型已开源。
...
TL;DR: 本文提出ClinRAG-GRAPH框架，利用临床知识图谱、域对抗学习和LLM驱动的检索增强生成，实现乳腺癌新辅助化疗前pCR预测，在多中心数据上验证了效果。


<details>
  <summary>Details</summary>
Motivation: 现有pCR预测方法存在跨模态建模不足、多中心影像异质性大、可解释性弱等问题，亟需更鲁棒的预测方法。

Method: 构建患者内临床先验图谱，使用先验引导的关系感知图卷积网络进行多模态表示学习；引入双支域对抗学习抑制MRI协议偏差；加入LLM驱动的子图RAG模块检索相似历史病例以增强可解释性。

Result: 内部测试集AUC 0.815，两个外部测试集AUC分别为0.774和0.712，展现了跨中心的鲁棒性。

Conclusion: ClinRAG-GRAPH通过图结构多模态融合、域对抗学习和RAG增强可解释性，有效提升了乳腺癌pCR预测的准确性和泛化能力。
...
TL;DR: 本文提出Spotted框架，结合视觉相似性与时空可行性先验，并引入人机交互策略，在动物重识别任务上显著提升准确率并减少人工审核量。


<details>
  <summary>Details</summary>
Motivation: 现有动物重识别方法仅依赖视觉线索，忽略了野外研究中易获取的时空辅助信息（如时间戳、相机位置），导致检索准确率不足且需要大量专家审核。

Method: (i) 基于最小移动速度计算时空可行性得分；(ii) 利用可行性线索作为伪监督训练轻量头；(iii) 融合视觉相似性与时空可行性得到匹配分数；(iv) 采用主动对采样策略优先处理不确定预测。

Result: 在三个数据集上，top-5识别准确率分别提升9pp、2pp和9pp；人机交互策略减少高达69%的查询比较量。

Conclusion: 融合时空信息的动物重识别框架可以有效提升性能并降低人工成本。
...
TL;DR: 提出HEE，一种无需训练、模型无关的框架，将静态图像理解转化为动态、查询引导的实体探索，通过双评分机制、目标检测、语义层次组织和回溯机制，在高质量图像理解任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 高分辨率图像中细粒度细节容易丢失，现有方法需要训练或启发式分割，难以泛化到复杂场景。

Method: HEE框架包含双评分机制（判断区域是否足够回答问题）、目标检测提取细粒度实体、聚类形成子区域、多级语义层次组织，以及置信度引导的回溯机制。

Result: 在Visual Probe和HR-Bench基准上，HEE在准确率和效率上均优于ZoomEye和RAP等训练无关方法，并在不同MLLM上表现出泛化能力。

Conclusion: HEE是一种无需训练、模型无关的高效框架，能够自适应地探索高分辨率图像中的细粒度实体，提升MLLM在复杂场景下的感知能力。
...
TL;DR: 本文提出DiT-Pruning方法，针对Diffusion Transformers（DiTs）设计了能量感知的显著性度量和聚类感知的剪枝粒度，在保持图像质量的同时大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: DiTs在图像生成中表现优异但计算开销大，传统剪枝方法因DiTs独特的架构和参数分布而失效，LLM剪枝方法会放大权重贡献，且现有剪枝粒度忽略模型结构差异。

Method: 提出能量视角的显著性度量来平衡权重和激活的贡献，并基于二维权重空间的聚类模式设计聚类感知的剪枝粒度。

Result: 在FLUX.1-dev上50%稀疏度时CLIP score仅下降0.001，显著优于近期剪枝方法。

Conclusion: DiT-Pruning能有效保持高稀疏度下的图像生成质量，为DiT模型的高效部署提供了可行方案。
...
TL;DR: 提出了一种无需训练的文本嵌入引导（TES）框架，通过优化条件文本嵌入来缓解文本到图像扩散模型中的人口统计偏见，在保持图像质量的同时提升公平性。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型存在人口统计偏见（性别、种族），现有方法要么需要昂贵重训练，要么推理时干预会降低图像质量和语义对齐。

Method: TES是一种无需训练的框架，通过两阶段策略直接优化扩散过程中的条件文本嵌入：1）早期全局对齐；2）迭代去噪时使用CLIP反馈进行细粒度调整，无需修改模型参数。

Result: 在Stable Diffusion上的实验表明，TES在公平性上优于现有无训练基线方法，同时保持了有竞争力的图像质量。

Conclusion: 推理时文本嵌入优化是一种实用且可扩展的公平性生成方案，可用于扩散模型。
...
TL;DR: 本文比较了四种学习心脏运动先验的策略（种群先验、共识先验、自动解码器、元学习），用于隐式神经表示的心脏运动估计，发现所有学习先验均优于随机初始化，其中自动解码器在早期恢复大变形更快，元学习在50次迭代内保持最佳适应轨迹。


<details>
  <summary>Details</summary>
Motivation: 隐式神经表示（INR）适合心脏运动估计，但逐序列拟合耗时且对优化轨迹敏感；学习先验可引导优化并加速适应，但心脏运动INR的先验学习尚缺乏探索。

Method: 比较四种学习心脏运动先验的策略：种群先验（联合优化）、共识先验（权重平均）、自动解码器、元学习，使用UK Biobank的短轴标记心脏磁共振图像进行评估。

Result: 所有学习先验均显著提升早期适应性能；自动解码器在早期恢复大变形更快；元学习在早期表现强劲且在50次迭代内保持最佳适应轨迹。

Conclusion: 学习先验可有效提升心脏运动INR的适应速度和效果，元学习整体表现最优。
...
TL;DR: 提出了Stitched Embeddings框架，首个无需物理模拟即可统一3D服装重建与缝纫模式推断的双向潜空间框架。


<details>
  <summary>Details</summary>
Motivation: 服装因拓扑多样性难以建模，传统方法依赖物理模拟将2D缝纫模式转换为3D几何，计算成本高。

Method: 利用预训练3D基础模型的几何先验，提出BoxMesh作为中间表示来对齐2D面板到3D配置，无需模拟器。

Result: 在模式重建上达到SOTA精度，同时显著提升效率。

Conclusion: 该可微框架支持从网格恢复模式、从2D模式编辑3D等新应用，为神经3D视觉与物理服装生产搭建了桥梁。
...
TL;DR: 针对胸部X光多标签分类中的极端长尾分布问题，提出TRCGL-Net，结合文本引导扩散模型、通道重加权、类别注意力机制和图卷积网络，有效提升罕见病识别性能。


<details>
  <summary>Details</summary>
Motivation: 真实临床数据存在极端长尾分布，尾部类别罕见病识别性能下降，原因不仅是数据稀缺，还有尾部病变特征在复杂解剖背景下衰减以及头部类别主导标签共现关系建模。

Method: 1) 可学习的文本引导条件扩散模型生成高质量尾部类别样本；2) 通道重加权机制进行特征重校准；3) 类别感知注意力机制生成类别特定注意力图；4) 基于标签共现的图卷积网络进行类别间信息传播。

Result: 在PadChest数据集上，尾部类别mAP 0.4904，总体mAP 0.4408，mAUC 0.8989，优于现有方法。
...
TL;DR: 本文全面综述了基于深度学习的多标签图像分类（MLIC）方法，将现有工作分为六大类，并总结了关键挑战与未来方向。


<details>
  <summary>Details</summary>
Motivation: 多标签图像分类是计算机视觉的基础任务，广泛应用于自动驾驶、疾病诊断等场景，但缺乏系统性的综述来梳理深度学习在该领域的发展。

Method: 构建了六分类法：区域导向、标签导向、架构导向、表征导向、学习导向和数据导向方法。

Result: 系统梳理了MLIC的背景、数据集、骨干网络和评估指标，深入分析了各类方法的优劣。

Conclusion: 该综述为MLIC领域提供了全面的视角，并指出了未来研究方向。
...
TL;DR: MFASSL是一种轻量级的视觉Transformer框架，通过软反射先验改进自监督学习，在医学图像和人脸等近似对称数据上超越MoCo-v3、DINO和MAE，仅增加约2.7%参数。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法强制翻转不变性，会抑制近似对称数据（如医学图像、人脸）中有用的左右对应关系，需要一种能保留不对称线索的软反射先验。

Method: 构建镜像配对视图，估计对称轴对齐；设计轻量级Mirror-Fusion Attention模块实现自适应token级交互；耦合反射一致性损失和中间层token对齐损失到基础SSL目标中。

Result: 在CheXpert、BraTS、CelebA-HQ、WFLW四个数据集上，MFASSL在下游性能、校准性和反射鲁棒性上均优于MoCo-v3、DINO和MAE基线，且优于现有等变SSL方法。
...
TL;DR: 本文提出MoVA框架，通过双不对称投影解决视频-文本对齐中的时间错位和语义不对称问题，在多个任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有对比预训练模型（如CLIP）在视频-文本对齐中面临两个核心问题：时间错位（文本只对应特定时间窗口）和语义不对称（帧级细节与字幕概念之间稀疏、双向、非等价的相关性），导致表征纠缠。

Method: 提出了MoVA（Modular Long Video-Text Alignment）框架，包含双不对称投影：文本侧投影自适应选择字幕的帧感知子空间，视频侧投影解缠与文本相关的视觉概念。

Result: 在多个视频-文本对齐任务上，MoVA优于现有方法，能够保留全局跨模态语义同时解缠演化的帧特定概念，并自然扩展到长字幕和长视频。

Conclusion: 建立了支持灵活视频-文本对齐的理论条件，MoVA通过双不对称投影有效解决了视频域特有的时间错位和语义不对称问题。
...
TL;DR: 提出TrajLoc方法，通过为每个物体分配独立的高斯热力图交叉注意力约束，实现多物体图像到视频生成中精确的轨迹控制，在6个数据集上平均PSNR提升4.3dB，轨迹端点误差降低51%。


<details>
  <summary>Details</summary>
Motivation: 现有方法将多个物体的轨迹纠缠在共享的密集条件信号中，难以在拥挤场景中保持物体级对应关系，尤其当物体数量增加、轨迹相交或遮挡时问题更突出。

Method: 在注意力层中为每个物体token的交叉注意力权重替换为以目标位置为中心的高斯热力图，通过每物体token接口传递轨迹和深度嵌入，并用第一帧外观编码替代物体token来保持身份。

Result: 在6个数据集上（最多20个物体同时控制），平均PSNR提升4.3dB，轨迹端点误差降低51%，适用于CogVideoX 5B和WaN 2.1 14B两种架构骨干。

Conclusion: 提出的每物体独立空间约束范式有效解决了多物体轨迹控制中的身份保持和轨迹遵循问题，显著优于现有方法。
...
TL;DR: EchoRisk是一个多中心、纵向超声心动图数据集，用于乳腺癌治疗相关心脏毒性的早期风险评估，包含422名患者的2159个视频，定义了三个临床任务，并提供了基线性能基准。


<details>
  <summary>Details</summary>
Motivation: 治疗引起的心脏毒性是乳腺癌患者治疗中断的主要非肿瘤原因，但目前缺乏从常规心脏影像中自动进行早期风险分层的方法。

Method: 构建包含422名患者、2159个超声心动图视频的多中心纵向数据集；定义三个任务：LVEF估计、LV功能障碍分类、早期心脏毒性预测；使用R(2+1)D视频骨干网络+LSTM聚合作为基线方法。

Result: 基线模型在心脏功能评估和LV功能障碍分类上表现良好，但仅凭单一治疗前视频预测早期心脏毒性仍是一个挑战。

Conclusion: EchoRisk数据集和基准为心脏肿瘤学领域的进一步研究提供了公开资源，促进了任务特定架构的开发。
...
TL;DR: 提出EFlow框架，通过证据优先的策略（将时间定位与逻辑推理解耦）解决长视频推理中的过早语义承诺问题，在多个基准上取得提升。


<details>
  <summary>Details</summary>
Motivation: 现有工具增强的视频框架常将时间定位与推理交织在同一轨迹中，导致早期语义假设（过早语义承诺）使证据定位产生偏差，进而强化错误推理。

Method: 基于Qwen3-VL，提出EFlow框架：(1) 通过CoT for Temporal Grounding和CoT for Reasoning显式分离时间定位与逻辑推理；(2) 引入置信度感知的反思机制，在证据不足时重新评估完整视频；(3) 构建专用轨迹数据集，通过SFT、RL和RFT训练。

Result: 在五个视频理解基准上，EFlow持续提升了长视频推理性能。

Conclusion: 证据优先的解耦策略能有效缓解过早语义承诺，提升长视频推理的准确性和鲁棒性。
...
TL;DR: 提出OmniView-Space框架，通过多视角空间映射、工具引导的自我中心推理和认知地图蒸馏，解决MLLM在多步空间推理中的空间一致性问题，达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在复杂多步空间推理中难以动态切换不同参考系（如摄像机中心、物体中心、方向中心），导致空间一致性不足。

Method: (1) 多视角空间映射（MPSM）：将重建几何重新锚定到查询对齐的视觉认知地图和文本空间图；(2) 工具引导的自我中心推理：训练模型主动选择所需自我锚点并请求对应MPSM证据；(3) 认知地图蒸馏：利用MPSM轨迹和自我框架奖励训练模型使用自生成认知地图进行推理。

Result: 在单图像和多图像空间推理基准上达到SOTA性能；蒸馏模型在减少对外部几何管线依赖的同时保持性能。

Conclusion: OmniView-Space通过多模态自我中心证据有效维护空间一致性，为MLLM空间智能提供了新范式。
...
TL;DR: 系统比较了五种特征提取器、七种分类头和三种分割策略在CT肺癌表型分析中的表现，发现任务最优设计取决于具体任务，推荐Curia+肿瘤分割+CatBoost作为默认方案。


<details>
  <summary>Details</summary>
Motivation: 现有研究在比较影像组学与基础模型时，未隔离特征提取器、分类头和分割选择的各自贡献，也缺乏跨队列鲁棒性测试。

Method: 在LUNG1（n=338）上训练，内测集（n=84）和外部LUNG2队列（n=211）上评估，采用两阶段流水线避免过拟合，以最差跨队列性能为主要指标。

Result: 分割选择主导体积和分期分类任务；分类器选择主导生存、组织学和年龄预测；影像组学在部分任务有竞争力；Curia变体在生存预测达到可比峰值；DINOv3略逊；补丁和切片聚合影响可忽略。

Conclusion: 推荐Curia+肿瘤分割+CatBoost作为跨任务默认方案；无肿瘤分割时Curia-2+肺分割+逻辑回归是竞争替代；任务特定选择始终优于任何跨任务默认方案。
...
TL;DR: 提出了LongVQUBench基准，用于评估大视觉语言模型对长视频质量的理解，包含三级评估和针孔失真问答范式，实验发现模型在长视频和深度推理上性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有视频质量基准主要关注短视频和孤立失真，忽视了长视频的时间连续性、累积退化和推理复杂性，缺乏对LVLM长视频质量理解能力的系统评估。

Method: 构建包含1200+视频和1500道题目的LongVQUBench基准，设计三级评估（LQU局部、CQR跨事件、GQU全局）和NDQA针孔失真问答范式。

Result: 14个LVLM的实验表明，随着视频长度增加和推理深度加深，模型性能显著下降，暴露了其在长时域时间整合和感知归因方面的局限性。

Conclusion: LongVQUBench为LVLM长视频质量理解提供了系统化、层次化、可解释的评估基础。
...
TL;DR: 该论文提出了一个五阶段框架，用于解构和分析二值目标分割中的评估指标，揭示了各指标的设计假设和局限性。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标被当作孤立公式使用，缺乏统一的比较视角，导致指标的设计假设和局限性不透明。

Method: 将每个评估指标分解为五个模块化阶段：预测表示、目标提取、目标匹配、分数计算和指标报告，并用此框架分析代表性指标。

Result: 展示了新指标如何解决旧协议中的特定限制，并使各指标的设计假设保持可见。

Conclusion: 该框架为任务感知的评估协议设计打开了新空间，有助于更合理地进行指标选择与对比。
...
TL;DR: 本文提出了一种结合城市场景先验、稀疏多高度观测和目标高度描述的几何感知条件预测框架，用于预测无人机在不同高度层的信道知识地图（CKM）。FPN-Transformer模型在零样本和补丁随机协议下均优于基线，不确定性引导的成本感知感知策略进一步提升了重建效果。


<details>
  <summary>Details</summary>
Motivation: 低空无人机需要在仅有稀疏观测数据的情况下，推断不同高度层的信道知识，以支持无人机辅助通信。

Method: 提出几何感知条件预测框架，结合城市场景先验、稀疏多高度观测和目标高度描述，使用FPN-Transformer模型预测密集CKM，并引入不确定性头来表征预测置信度，支持成本感知的在线无人机感知。

Result: FPN-Transformer在零样本下RMSE降至5.347dB，补丁随机下降至1.111dB；10-shot少样本适应后RMSE降至3.518dB；不确定性引导的感知策略在预算40时重建误差降至4.79dB。

Conclusion: 所提方法有效实现了跨高度CKM预测，并能在运动和安全约束下通过不确定性引导进行高效的在线感知。
...
TL;DR: DiscoPER是一个基于大语言模型的自主科学发现框架，无需预设研究目标即可进行开放式研究，通过二阶推理机制分析已有发现并引导探索，在多模态生态知识基准上超越基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有自主科学发现系统搜索空间受限、需预设研究问题，缺乏对已有发现的综合提炼能力，无法实现真正的开放式探索。

Method: 提出DiscoPER框架：1) LLM驱动动态生成代码探索数据集；2) 所有发现必须通过统计检验保证科学有效性；3) 引入二阶推理机制，定期分析已有发现以识别结构模式和认知盲区；4) 集成工具使用，可处理多模态数据（如图像）。

Result: 在iNatDisco多模态生态知识基准上，恢复9个已知模式中的8个，假设支持率达72.7%，优于经典因果发现和LLM引导基线。消融实验证实方法随数据量增加而扩展，二阶元反思有效。

Conclusion: DiscoPER通过二阶元反思和工具使用显著提升了自主科学发现系统的开放式探索能力，为自动化科研提供了新范式。
...
TL;DR: 提出MG-RWKV框架，利用RWKV的数据依赖状态演化实现O(T)复杂度的全序列处理，通过双向RWKV架构、多粒度混合专家（MG-MoE）和跨粒度一致性（CGC）三个核心创新，在时序伪造定位任务上取得SOTA性能且计算成本低。


<details>
  <summary>Details</summary>
Motivation: AIGC技术发展导致音视频内容真实性面临严峻挑战，现有时序伪造定位方法受限于CNN的局部感受野、Transformer的二次复杂度以及线性模型难以平衡全局上下文压缩与局部伪造感知的矛盾。

Method: 提出MG-RWKV框架，包含三个核心创新：(1)双向RWKV架构，无二次复杂度捕获双向时序上下文；(2)多粒度混合专家(MG-MoE)，在显式时序感受野上动态路由，自适应选择粒度；(3)跨粒度一致性(CGC)，通过层级尺度对齐和空间边界加权减少误检。

Result: 在Lav-DF、TVIL和Psynd数据集上，MG-RWKV以低计算成本取得SOTA性能。
...
TL;DR: 本文提出CIM框架，通过度量驱动的方式解决数据集蒸馏中双压缩导致的信息丢失和分布偏移问题，在ImageNet-1K上以IPC=10仅用80分钟达到48.7%的Top-1准确率，大幅超越此前SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法存在计算开销大、跨架构泛化差的问题，根本原因是双压缩过程（数据→模型→图像）导致严重信息丢失，并使得RELABEL策略失效。

Method: 提出CIM框架，摒弃双压缩范式，通过直接对齐原始数据集与合成数据集的数据分布，显式量化并最小化信息间隙，确保高保真信息压缩。

Result: 在ImageNet-1K上，IPC=10时仅用80分钟（单卡RTX-4090）达到48.7% Top-1准确率，超越NRR-DD（+2.6%）和DELT（+2.9%）。

Conclusion: CIM通过度量驱动、直接对齐数据分布的方式，有效解决了双压缩的信息丢失问题，建立了新的SOTA。
...
TL;DR: 提出了GMO-E$^2$DIT，一个结合VLM和掩码条件图像编辑器的智能体框架，通过解耦认知推理与生成渲染、迭代反射机制，实现多轮结构化图像编辑，在电商场景中取得了优于现有基线的指令准确度和编辑保真度。


<details>
  <summary>Details</summary>
Motivation: 现有单次图像编辑器将意图解析、空间定位和合成混为一步，导致部分执行失败，无法满足电商场景中需要多重、局部、可审计编辑的商业需求。

Method: 提出GMO-E$^2$DIT框架：VLM代理构建区域定位的编辑议程，解耦认知推理与生成渲染；通过操作感知掩码和参考执行子程序，利用反射驱动循环检查中间结果并决定后续状态，实现迭代式错误恢复和任务完成。

Result: 在EComEditBench基准上，GMO-E$^2$DIT相比现有基线取得了更优的指令准确率和编辑保真度，性能与强闭源模型相当。

Conclusion: 该智能体框架通过解耦规划与执行、引入反射机制，有效地解决了多轮结构化图像编辑中的部分执行失败问题，为电商场景提供了可靠且可审计的编辑方案。
...
TL;DR: 提出从单目视频生成可自由渲染的动态3D高斯表示的方法，通过视频模型结合密集像素对齐渲染来修正伪影并补全缺失区域，在4D重建上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 从单目视频重建动态3D场景时，现有方法存在渲染伪影和缺失区域问题，且难以处理大视角变化和动态运动。

Method: 构建多视角视频对和动态3DGS数据集，训练视频模型在输入和目标相机轨迹上联合处理外观、几何和3D运动；测试时将生成结果蒸馏回一致的动态3DGS表示。

Result: 在4D重建任务上达到新SOTA，可泛化到具有大视角变化和动态运动的野外视频。

Conclusion: 该方法有效提升了新视角合成质量和底层3D运动准确性，为单目视频到动态3D重建提供了有效方案。
...
TL;DR: 本文评估了四种基于运动的AI生成视频检测器，发现它们存在严重的预处理/采样偏差，性能在无运动偏差的数据集上降至随机水平，而频率检测器表现出更强的泛化能力。


<details>
  <summary>Details</summary>
Motivation: AI生成视频质量提升，人眼难以区分真假，现有运动检测器性能被质疑，需要评估其真实鲁棒性。

Method: 评估四种运动检测器，识别预处理和采样偏差，通过数据集重平衡和空间增强测试鲁棒性，并与频率检测器对比。

Result: 运动检测器性能高度依赖运动偏差，在无偏差数据集上性能崩溃；频率检测器在所有数据集上保持稳定。

Conclusion: 运动检测器存在严重偏差和局限，频率方法可能更具泛化能力；需构建更无偏数据集和更鲁棒的评估协议。
...
TL;DR: 提出GaussianEmoTalker，一种基于3D高斯泼溅的实时情感说话头生成框架，通过中性到情感的残差变形方法实现可控情感强度和高保真渲染。


<details>
  <summary>Details</summary>
Motivation: 现有音频驱动说话头方法在唇形同步和视觉质量上取得进展，但生成具有可控强度的情感化表情头像在实时约束下仍具挑战性。

Method: 1) 构建身份特定的中性说话空间GaussianBlendshapes；2) 融合网格位移、音频、情感类别和强度编码，预测情感条件残差变形；3) 引入空间-音频-情感注意力模块估计高斯属性偏移。

Result: 在视频质量、唇形同步准确性、情感表达可控性和实时渲染方面均优于现有情感说话头方法。

Conclusion: GaussianEmoTalker有效实现了实时、高保真且情感可控的音频驱动说话头合成。
...
TL;DR: 提出一种混合光场超分辨率网络SMART，结合斜率引导的Mamba和角度精炼的Transformer，解决空间-角度解耦建模和扫描机制与极线几何不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 现有光场超分辨率方法存在两个局限：空间和角度维度解耦建模导致几何不一致；连续序列建模的刚性扫描机制与极线几何冲突，限制了几何感知特征聚合。

Method: 提出SMART网络：1) 角度调制空间模块，引入角度先验增强空间-角度相关性建模；2) 流形对齐轨迹模块，实现沿极线结构的几何一致序列建模。

Result: 在五个基准数据集上达到SOTA，PSNR比之前方法提升0.42 dB，伪影显著减少。

Conclusion: 混合Mamba与Transformer架构能有效克服光场超分辨率中空间-角度解耦和扫描-几何不匹配两大挑战。
...
TL;DR: 提出UTTO框架，利用不确定性引导的测试时优化，仅依赖深度数据实现开放词汇3D语义分割，在保护隐私的同时提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 在真实室内环境中部署3D场景理解系统需要隐私保护，但现有方法依赖RGB图像获取语义线索会暴露敏感视觉信息。仅使用深度数据虽可保护隐私，却因缺少外观语义线索导致开放词汇预测高度不确定且不可靠。

Method: 提出UTTO（不确定性引导的测试时优化框架），将不确定性转化为引导信号来识别不可靠的语义响应，并利用基础模型的语义先验来正则化其优化过程。无需额外训练。

Result: 在ScanNet20、ScanNet40和ScanNet200数据集上，UTTO一致性地提升了仅深度数据的开放词汇3D语义分割性能，并在隐私保护条件下优于代表性基线方法。

Conclusion: 不确定性引导的测试时优化能有效缓解深度-only开放词汇3D分割中的语义不确定性，在保护隐私的同时提升分割准确性。
...
TL;DR: 提出一种无需训练的查询与内容感知关键帧选择框架(QCA)，能从长视频中选择紧凑且信息丰富的帧子集，在多个长视频理解基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视频理解中存在严重的时序冗余，处理密集帧序列既语义低效又计算昂贵，且只有少量帧与查询真正相关。

Method: QCA首先将视频划分为时序片段，通过联合建模查询相关性和内容偏差估计每个片段的信息贡献，动态分配关键帧预算；在每个片段内，锚定最查询相关的帧并迭代添加额外帧以最大化多样性同时保持高语义相关性。

Result: 在LongVideoBench上，QCA使用128帧达到67.8%，而GPT-4o使用256帧仅达66.7%，超越现有方法并展现强泛化能力。

Conclusion: QCA无需额外训练即可无缝集成到现有Video-LLMs中，有效解决长视频时序冗余问题，实现SOTA性能。
...
TL;DR: 提出AVSR-Diff框架，通过解耦尺度无关的潜在去噪与连续坐标渲染，实现任意尺度视频超分辨率，在保持高频细节和时间稳定性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在视频超分中受限于固定放大倍数，而任意尺度方法存在过平滑和时间闪烁问题。

Method: 提出解耦框架，包含TGFR模块提取时间对齐的潜在先验，以及SAFR模块动态适应频率分量至任意目标尺度。

Result: 在多种尺度上保持高频细节和强时间稳定性，超越SOTA任意尺度基线，在原始分辨率上甚至优于固定尺度生成模型。

Conclusion: 解耦潜在去噪与连续渲染能有效解决任意尺度视频超分中的过平滑和时间闪烁问题。
...
TL;DR: 提出SuperFlex框架，通过新损失函数、弯曲/锥形变形和部分点云鲁棒训练，显著提升超二次曲面分解的重建精度与表达能力。


<details>
  <summary>Details</summary>
Motivation: 现有超二次曲面方法重建精度有限、局限于刚体基元、对部分点云鲁棒性差。

Method: 1) 引入新的损失函数提升重建精度；2) 引入弯曲和锥形变形以表达弯曲和非对称几何；3) 利用高质量分解作为监督训练模型，使其对部分点云具有鲁棒性。

Result: 在重建精度上显著优于基于优化和学习的基线方法，同时保持高度紧凑的基元表示。

Conclusion: SuperFlex框架有效扩展了超二次曲面分解的表达能力和适用性。
...
TL;DR: GenAU是一个统一的视觉-语言框架，通过引入两个分割token，在单一指令跟随模型中同时实现图像级检测、像素级分割、多类型异常检测和缺陷分析，在检测性能上超越CLIP零样本方法，分割性能接近专业CLIP基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么只做检测和定位但缺乏语言级缺陷理解，要么能描述缺陷但不产生像素级掩码，无法在单一模型中统一完成工业检测所需的全部四项任务。

Method: 在视觉-语言模型中引入[SEG_defect]和[SEG_normal]两个分割token，其隐藏状态作为语言引导的查询，与多尺度视觉特征交互实现像素级定位；图像级分数融合分割图与解码器的文本正常/缺陷决策；语言解码器生成结构化的缺陷感知响应。联合语言建模和分割目标进行训练。

Result: 在VisA和Real-IAD数据集上，GenAU在图像级检测上达到最强性能（超过CLIP零样本方法），分割性能接近但未超越专业CLIP基线。

Conclusion: GenAU首次在单一架构中统一了工业异常检测所需的四种任务，以可量化的检测和分割性能代价换来了零样本多类型检测和语言引导的缺陷分析能力。
...
TL;DR: 提出GeoSearcher，将遥感视觉定位重新定义为锚点引导的逐步推理过程，通过ACR-SFT和PF-GRPO两阶段实现，在多个基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM将遥感视觉定位当作一步坐标生成，难以处理极小目标和复杂查询中的多线索推理。

Method: 提出GeoSearcher，包含两阶段：1) ACR-SFT：锚点中心推理数据微调；2) PF-GRPO：过程奖励+样本选择器优化推理行为。

Result: 在DIOR-RSVG、OPT-RSVG和VRS-Bench上超越现有SOTA方法。

Conclusion: 将大范围视觉搜索转化为约束性的局部推理过程，有效提升遥感视觉定位性能。
...
TL;DR: 提出CPDDNet，一种联合去噪和去马赛克的网络，用于处理彩色偏振滤光片阵列（CPFA）传感器在低光照下的噪声和分辨率损失问题。


<details>
  <summary>Details</summary>
Motivation: CPFA传感器输出的原始马赛克图像在低光照条件下存在严重噪声和分辨率损失，现有方法分别处理去噪或去马赛克任务，忽略了二者之间的耦合关系以及共享的低层特征。

Method: 提出CPDDNet，这是一个联合去噪和去马赛克框架，通过特征融合模块在去噪和去马赛克阶段保留CPFA原始数据的特征。

Result: 在真实数据集上，CPDDNet显著提升了图像质量和偏振参数精度，优于现有方法。

Conclusion: 联合处理去噪和去马赛克任务能够有效利用二者之间的耦合关系，提升CPFA传感器的成像质量。
...
TL;DR: 本文提出MoHallBench基准，系统评估VideoLLMs中的运动幻觉问题，发现模型在动作识别与抗幻觉能力之间存在显著解耦，其中序列推理幻觉最为严重。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注物体幻觉或粗粒度动作感知，忽略了视频特有的运动幻觉问题（模型推断出视频中不存在的运动）。

Method: 构建包含11,306个视频片段和40,493个问答对的MoHallBench基准，覆盖三种幻觉来源（共现先验、序列推理、相似混淆），采用双向提问协议和偏差感知指标减少确认偏差。

Result: 10个开源VideoLLMs实验表明：动作识别与抗幻觉能力存在解耦；序列推理幻觉最严重；更强的先验知识和更细粒度的相似性会显著放大幻觉。

Conclusion: MoHallBench可促进未来对VideoLLMs运动幻觉的评估与缓解研究。
...
TL;DR: 提出TraDiE策略，将轨迹预测的度量优化与概率训练目标解耦，实现SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 当前轨迹预测SOTA模型根据不同benchmark指标定制训练目标，但不同指标间存在冲突行为，需要一种度量无关的概率训练范式。

Method: 提出TraDiE策略（度量特定策略，将预测分布映射为K条轨迹和置信度）和DONUT-NLL（直接优化预测分布的负对数似然）。

Result: DONUT-NLL在Waymo运动预测基准的所有指标上均达到SOTA结果。

Conclusion: 轨迹预测应将度量优化作为下游任务，训练阶段应使用度量无关的概率目标。
...
TL;DR: SD-RouteFusion是一种可部署的端到端自车轨迹预测方法，融合前视摄像头、车辆运动学和SD地图导航路线，无需HD地图即可实现路线感知预测，在48万场景数据集上相比基线方法ADE降低16.9%。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖高精地图（HD Map），部署成本高、可扩展性差；而SD地图（标准定义地图）覆盖更广、更具规模化潜力，但目前缺乏有效的融合方法。

Method: 融合前视摄像头、车辆运动学和SD地图导航路线；采用双假设设计配合门控分类器，以应对路线损坏和视觉不确定性。

Result: 在涵盖10个欧洲国家和美国的48万驾驶场景数据集上，SD地图路线条件带来10.5%的ADE提升，完整融合策略在8秒预测范围内实现16.9%的ADE降低。

Conclusion: SD-RouteFusion为大规模、路线感知的自车轨迹预测提供了一条实用且可部署的路径，并发布了SD路线生成工具包以支持更广泛的评估。
...
TL;DR: 提出PairGS框架，通过建模高斯点之间的成对关系实现开放词汇3D高斯分割，无需每场景优化，速度快50倍且SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么将语言知识嵌入单个高斯导致噪声分割，要么依赖每场景优化成本高，需要更高效准确的3D高斯分割方法。

Method: 利用3D高斯表示中的视角贡献权重和多视角掩码证据等丰富信号，构建稀疏边候选并计算精确成对亲和度，建立分层聚类树支持多粒度查询。

Result: 在开放词汇3D高斯分割基准上达到SOTA，快速变体比基于优化的实例特征方法快50倍。

Conclusion: PairGS通过显式建模高斯间关系实现了高效、准确的开放词汇3D分割，避免了昂贵优化过程。
...
TL;DR: EquiSteer是一种无需训练的文本到图像扩散模型去偏方法，通过推理时调控交叉注意力激活，将平均性别差异减少87%。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型存在严重的人口统计偏见（如性别偏见），现有去偏方法需重训练、批次控制或提示词调优，缺乏可扩展性。

Method: 提出EquiSteer：预计算对比提示的引导向量，推理时通过提示感知门控，保留属性提示，对中性提示清除原有属性信号并注入目标属性。

Result: 在SD-1.5、SD-2.1、SDXL、SANA上，平均性别差异减少高达87%，对图像质量和文本-图像对齐影响极小。

Conclusion: EquiSteer是一种可扩展、无需训练的有效去偏方法，适用于多种扩散模型。
...
TL;DR: 本文提出一种高维k空间重建框架，通过表示提升增强扩散模型在噪声MRI重建中的鲁棒性，实验证明在噪声环境下显著提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在噪声MRI重建中表现不佳，尤其是在k空间域直接操作时难以处理噪声干扰，缺乏鲁棒的统一框架。

Method: 提出统一的高维k空间重建框架，通过表示提升（representation lifting）扩展数据表示空间，使现有扩散求解器能在更富表达力的k空间嵌入上操作，不修改底层优化流程。

Result: 在内部和公开数据集上，不同噪声水平和欠采样因子下，该框架一致提升了多个扩散逆求解器的重建质量，高噪声场景下提升最显著。

Conclusion: 高维表示为提升扩散模型MRI重建提供了通用且模型无关的机制，为鲁棒的k空间生成建模提供了新视角。
...
TL;DR: 提出Perceive-to-Reason (P2R)框架，将细粒度视觉推理解耦为感知和推理两阶段，并配合PRA-GRPO强化学习策略，显著提升多尺度模型在细粒度视觉推理任务上的表现。
...
Method: 提出P2R框架，先让模型作为感知器定位问题相关证据，再作为推理器基于标注图像和裁剪区域回答问题；同时提出PRA-GRPO，一种角色感知的强化学习策略，交替进行感知和推理更新。

Result: 基于Qwen3-VL-Instruct的2B/4B/8B模型均获得一致提升，P2R-4B在V-Star达93.2%，HR-Bench-4K达81.9%，HR-Bench-8K达80.5%，显著超越原骨干模型。
...
TL;DR: Linkify框架通过接口增强的装配图实现上下文感知的零件检索，使用图注意力网络在修正后的Fusion 360装配数据集上训练，显著优于非图基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有生成式CAD方法多聚焦于孤立零件或整体装配，忽略了零件间接口（接口处蕴含功能信息）的丰富几何信息。

Method: 1. 重新计算Fusion 360 Gallery Assembly数据集的高保真接口几何，修正缺失/错误接触；2. 构建装配图（节点编码零件几何，边通过预训练点云编码器编码接口几何）；3. 基于GATv2训练图注意力网络，执行掩码零件预测任务。

Result: Linkify在Top-K准确率和F1分数上优于逻辑回归和k近邻等非图基线；消融实验表明精确接触计算和动态注意力机制对性能至关重要。

Conclusion: 公开了修正后的接口数据集和训练流程，为未来接口感知的装配检索、验证和生成设计提供基础。
...
TL;DR: Ink3D通过解耦几何与纹理合成，利用大规模视频生成模型实现从参考图像到3D物体的高质量纹理生成。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成模型能合成高质量几何，但难以从参考图像复现精细纹理，主要因为缺乏大规模带丰富表面外观的3D训练数据。而视觉生成模型在更大规模数据集上训练，擅长建模复杂视觉模式。

Method: 1) 用现成3D生成模型重建白色网格几何；2) 使用OrbitPainter（条件视频生成模型）生成覆盖多视角的轨道扫描视频；3) 引入TextureOptimizer神经烘焙模块，融合密集多视角观测并缓解视频生成带来的几何不一致性。
...
TL;DR: 本文通过实验证明，大型语言模型中同一方向在不同干预机制（提示、微调、推理时操控）下并不指向相同内容，提出“按机制索引的个体化”框架，将表征内容的身份单元定义为（载体，机制）对。


<details>
  <summary>Details</summary>
Motivation: 质疑Beckmann & Butlin (2026)及persona-vectors文献中未经论证的跨机制共指假设——即同一方向在提示条件化、梯度下降微调和推理时操控下指向相同内容。

Method: 在Qwen3-4B-Instruct和Mistral-7B-Instruct-v0.2上进行四组persona-拓扑实验：提示提取向量与微调盆地的不共线性、虚构人格沿真实锚点方向位移更强、矛盾情感混合物偏向训练历史决定的吸引子、推理时算术与微调时嵌合体训练的非对称组合代数。

Result: 四个实验共同推翻了跨机制共指假设，表明同一方向在不同机制下对应不同内容。

Conclusion: 提出按机制索引的个体化框架，认为表征内容的身份单元是（载体，机制）对而非载体本身。该框架统一解释了Beckmann & Butlin、Mollo & Millière、Chalmers和Cerullo等候选立场实为描述不同机制内部的对象。
...
TL;DR: Loom 是一个基于叙事学理论（故事与话语区分）的辅助写作框架，通过三层管道和意图中心符号链式思维，解决了大语言模型在创意写作中在安全润色与破坏性扩写之间的二元困境，同时提升了事实完整性和描述强度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在创意写作中存在二元失败问题：要么只做安全浅层的润色（补救性抛光），要么进行破坏性、不可控的情节扩写，无法同时兼顾叙事保真度和描述强度。

Method: 基于叙事学的"故事-话语"区分，提出Loom框架，采用三层管道架构，实现以意图为中心的符号链式思维（semiotic chain-of-thought），将感知材料生成与句法插入分离，确保增强不破坏原始事件结构。

Result: 在LLM指标和人工评估中，Loom相比基线方法取得了最高综合质量分数，在事实完整性和描述强度上均有显著提升。

Conclusion: Loom成功解决了创意写作中叙事保真度与描述强度之间的根本性张力。
...
TL;DR: 本文提出利用潜在空间的引导向量（steering vectors）来控制模型行为，并开发基于潜在空间的模型校准器以提升可信度，从而构建更可信的语言技术。


<details>
  <summary>Details</summary>
Motivation: 随着大规模语言模型能力提升，其内部表示理解愈发困难。由于数百万用户依赖模型在中等或高风险场景中做出决策，需要建立对模型行为的控制并判断何时信任模型输出。

Method: 提出引导向量（steering vectors）用于控制模型行为；开发基于潜在空间的模型校准器（latent space-based model calibrators）用于评估可信度。

Result: 成功解构了语言模型的潜在空间，为利用模型内部机制构建更可信的语言技术提供了新见解。

Conclusion: 通过操控和校准潜在空间，可以有效增强语言模型的可控性和可信度，推动更可靠的语言技术发展。
...
TL;DR: 本文提出了一个跨评估框架，利用母语专家和前沿LLM，对埃及与伊拉克阿拉伯语方言的文化和语言任务进行自动评分，发现GPT-5.4评分最可靠，文化任务比语言任务更难评分，且所有评委存在系统性宽大偏差。


<details>
  <summary>Details</summary>
Motivation: 人类专家评估成本高昂，尤其是在阿拉伯语社会语言学领域，需要深厚的文化熟悉度，而表面指标无法替代。

Method: 构建了103个验证过的提示-评分标准对（埃及70个，伊拉克33个；文化类53个，语言类50个），由母语专家使用加权评分标准打分；3个前沿LLM作为目标模型，5个前沿LLM作为自动评委，采用MAD和Signed Error双指标评估。

Result: GPT-5.4是最可靠的评委（MAD=10.21pp，偏差=-1.12%）；5个评委中有4个存在系统性宽大（+2.01%至+6.56%）；文化任务评分难度高于语言任务（MAD差距1.83-4.78pp）；模型在埃及提示上表现优于伊拉克提示。

Conclusion: 隐性的文化推理（而非词汇验证）是自动评分的主要失败模式；需谨慎区分人类评分的宽大差异，不能简单归因于模型知识不足。
...
TL;DR: 该论文介绍了针对土耳其语和阿拉伯语的仇恨言论数据集，并开发了基于BERT的模型用于仇恨言论的多维度分析（分类、强度预测、目标识别、跨度检测）。


<details>
  <summary>Details</summary>
Motivation: 在线仇恨言论与针对少数群体的暴力事件日益相关，尤其是在土耳其语和阿拉伯语语境中缺乏相关数据集和模型。

Method: 构建了涵盖土耳其语5个主题和阿拉伯语1个主题的仇恨言论数据集，并开发了基于BERT的SOTA模型进行仇恨类别分类、强度预测、目标识别和跨度检测。

Result: 提出了新的仇恨言论数据集，并实现了基于BERT的最新模型在多维度仇恨分析上的性能提升。

Conclusion: 该工作为土耳其语和阿拉伯语的仇恨言论检测提供了重要的数据集和工具，有助于在线内容审核。
...
TL;DR: 本文研究了医疗LLM中的幻觉现象，发现内部表征可用于检测幻觉（AUROC 0.77-0.86），但无法通过神经元级别的控制来纠正幻觉，揭示了可解码性与可控制性之间的显著差距。


<details>
  <summary>Details</summary>
Motivation: 医疗LLM中的幻觉问题是部署的核心障碍，现有方法仅能检测幻觉，但不清楚内部表征是否可用于主动控制幻觉。

Method: 使用4个开源模型和多个医疗问答数据集，通过精心设计的探针检测幻觉，并分析神经元的分布特性，测试随机神经元子集与低维投影的检测性能，最后验证是否可通过神经元控制来纠正幻觉。

Result: 探针可可靠检测幻觉（AUROC 0.77-0.86），信号是分布式的而非局部化的；随机几百个神经元即可恢复接近完整的检测信号。但在16个模型-数据集组合中，可解码性并未转化为可靠的神经元级控制能力。

Conclusion: 医疗幻觉在内部激活中易于检测，但难以通过神经元级别的干预来纠正，表明幻觉缓解不仅仅是找到正确神经元的问题，表征的可揭示性与可改变性之间存在深层分离。
...
TL;DR: 现有KB-VQA基准存在答案不可推导、问题不明确、视觉场景过于简单等问题，导致准确率作为评估指标不可靠。作者提出审计修复协议和多实体增强协议，重新评估后模型表现趋势发生显著变化，呼吁重新设计评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有KB-VQA基准以答案准确率作为主要评估指标，但隐含了三个关键假设（答案可从知识库推导、问题充分约束、视觉场景需要消歧），而这些假设被系统性违反，导致评估结果不可靠。

Method: (1) 审计-修复协议：修复答案可推导性和问题清晰性；(2) 多实体增强协议：引入视觉歧义来挑战初始检索和基于知识的推理。

Result: 在修正和增强设置下重新评估，模型表现趋势与原来显著不同，揭示了原有评估的误导性。

Conclusion: 需要重新思考评估协议，设计更注重可验证推理而非简单匹配的KB-VQA基准。
...
TL;DR: 提出ALEE框架，利用AMR生成跨语言段落级语义最小对，评估275+语言的文本嵌入模型，揭示不同语言在语义表示上的差距。


<details>
  <summary>Details</summary>
Motivation: 现有文本嵌入评估基准存在静态、语言覆盖有限、易过拟合、低资源语言代表性差等问题。

Method: 基于Abstract Meaning Representations (AMR)生成英语最小对，配合目标语言翻译，构建跨语言段落级评估框架。

Result: 在275+语言上测试多种嵌入模型，发现性能在不同语言、文本长度和语言现象上差异显著，暴露了跨语言语义表示的持续差距。

Conclusion: 跨语言语义表示质量与训练数据中的语言流行度和子词分词密切相关，ALEE可作为更全面的评估工具。
...
TL;DR: 本文使用机器学习方法分析印加帝国Khipus（结绳记事装置）的结构，通过UMAP和HDBSCAN聚类发现三类结构组，利用梯度提升分类实现F1=0.86的帝国风格分类，并识别出绳捻方向是区分帝国风格的关键特征。


<details>
  <summary>Details</summary>
Motivation: Khipus（结绳记事）是印加帝国主要的记录媒介，但其系统至今未被破译，因此需要借助可重复的计算方法进行结构分析。

Method: 基于Open Khipu Repository中619个khipus（54,403根绳、110,677个结）的数据，构建27个结构特征；使用UMAP+HDBSCAN进行无监督聚类；用梯度提升进行有监督产地分类；用SHAP进行可解释性分析。

Result: 聚类获得三个结构组（轮廓系数0.769）；帝国风格分类F1=0.86；绳捻方向是主要区分特征；发现一个聚类受欧洲博物馆收藏影响而非地理区域；独立验证了Santa Valley六个khipus的recto/verso结构。

Conclusion: 机器学习的可重复方法能有效揭示Khipus的结构模式，并发现殖民采集实践对数据集的编码影响。
...
TL;DR: 本文提出了一个多智能体叙事生成框架MAGNET+ATLAS，通过显式世界状态追踪和目标驱动的多智能体生成，在长篇小说生成中相比单模型和IBSEN减少了41-50%的幻觉和标注问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在生成长篇故事时难以保持叙事一致性和连贯的情节线。

Method: 提出了统一框架：MAGNET（多智能体目标驱动叙事引擎）使用基于人设的角色智能体，基于共享世界状态和演进故事目标提出行动；ATLAS（基于图的管线）通过比较场景级世界表示来检测幻觉。

Result: 在100页故事中，MAGNET相比单模型基线减少了41%的标注和50%的幻觉，相比IBSEN减少了34%的标注和45%的幻觉。
...
TL;DR: SLIM-RL提出了一种无需重构轨迹的强化学习方法，通过tau-budget解码器控制提交风险，在SDAR-4B上以TraceRL仅46%的训练样本达到相当性能，并在多个基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散LLM强化学习方法（如TraceRL）需要重构推理轨迹，导致训练成本随块大小增长。作者希望在不重构轨迹的情况下缓解随机掩码与推理轨迹不匹配的问题。

Method: 提出SLIM-RL方法：(1) tau-budget解码器限制每个rollout步骤的提交风险；(2) 无迹随机掩码目标函数，结合序列级重要性采样和确定性正交掩码调度；(3) 均保序、单调递减的逐块掩码调度。

Result: 在SDAR-4B上，SLIM-RL在block size=16时以TraceRL的46%训练样本达到同等MATH500精度；MATH500提升6.32%，GSM8K提升11.05%。在block size=4时，4B模型超越LLaDA-8B和Dream-7B，MATH500上超LLaDA-8B 10.76%。代码基准上MBPP提升4.20%，HumanEval提升3.65%。

Conclusion: SLIM-RL通过风险控制和方差缩减，在扩散LLM强化学习中实现了更高的训练效率和性能，且tau-budget解码器可零成本迁移到其他扩散LLM模型。
...
TL;DR: 针对马耳他语OCR资源匮乏的问题，通过合成训练流水线和5流Tesseract LV-ROVER集成，将CER从0.0234降至0.00700，降低70%。


<details>
  <summary>Details</summary>
Motivation: 马耳他语虽有文本语料和预训练模型，但OCR标注数据极度匮乏（仅57页真实标注PDF），远低于段落级训练需求，急需低成本高精度的OCR方案。

Method: 构建合成训练流水线（synthetic training pipeline），采用5流Tesseract LV-ROVER集成识别，并设计五阶段后处理链（含排版标准化和变音符号纠错）。

Result: 在422段落基准上，集成识别CER从0.0234降至0.01317（降低44%），完整流水线CER降至0.00700（降低70%）。

Conclusion: 合成训练+多模型集成+后处理链可有效应对低资源语言OCR难题，44%的识别增益具有可迁移性，70%的降幅与基准标签约定相关。
...
TL;DR: 本文构建了包含教师真实课堂批改反馈的公开语料库SEFORA（564篇作文、8240条批注），并提出了参考式评估框架UniMatch（将反馈分解为单元，基于语义匹配计算精确率、召回率和F1值）。实验表明，现有LLM在74种配置下F1均未超过0.4，模型难以识别教师优先关注的反馈点，且生成越多效果越差。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在写作反馈领域存在两大空白：缺乏教师真实课堂反馈的公开语料库，以及缺少能够衡量生成反馈与教师预期一致性的可靠评估方法。

Method: 构建SEFORA语料库（包含教师内联反馈、作业提示、评分标准、分数和多稿修订），提出UniMatch评估框架（将反馈分割为单元，基于教师制定的标准进行语义匹配，通过最优匹配计算精确率、召回率和F1）。

Result: 在74种实验配置下，所有LLM的F1均未超过0.4，模型难以捕捉教师优先考虑的反馈点，且生成更多反馈时性能反而下降。

Conclusion: 现有LLM在生成与教师一致的写作反馈方面仍有显著不足，UniMatch框架为评估开放文本生成与人类参考标准的一致性提供了有效工具。
...
TL;DR: TRACE是一个基于时间证据图（TEG）的查询处理框架，通过层次化图结构（事件-会话-主题）和有效性标注，解决对话数据中信息过时与矛盾的问题，在长对话QA任务中提升了时序和多跳推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有长记忆流水线将记忆视为独立的文本或向量对象，检索时容易获取语义相似但过时的证据，缺乏对状态感知推理的支持。

Method: 将对话建模为包含事件、会话、主题的层次化图，并添加时序、因果、更新、矛盾等类型化关系；维护有效性标注，使过时事实在历史查询中可用但在当前状态查询中被折扣；查询时结合向量检索和图引导证据搜索，生成有效性感知的支持路径和混合上下文。

Result: 在长对话QA基准上，TRACE提升了时序和多跳推理能力，消融实验验证了层次结构、更新感知播种和路径基础证据的重要性。

Conclusion: TRACE通过分离词汇检索与证据重建，实现了对长对话历史的有界查询时推理，有效解决了对话数据中信息演变带来的挑战。
...
TL;DR: 本文提出DiscoLoop架构，通过在循环Transformer中引入离散嵌入通道和连续隐藏状态通道，解决了多跳推理中的表示瓶颈问题，在符号和实际语言建模任务上均取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 标准非循环Transformer在处理需要单次前向传播内完成多步推理的任务时存在深度局部存储问题，即早期层学习的事实无法在后续跳的检索中使用。循环Transformer虽能缓解但泛化不完美，其瓶颈在于表示层面。

Method: 提出DiscoLoop架构：在循环Transformer中，循环不仅传递连续隐藏状态，还同时传递离散嵌入通道。这种混合通道设计使得桥接实体在离散空间中可被准确解码，解决了隐藏状态与桥接词嵌入对齐不良的问题。

Result: 在两跳推理任务上，DiscoLoop以更少的训练步骤达到近乎完美的准确率；在真实预训练场景中，相比循环Transformer基线，取得了更低的训练损失和更强的基准性能。

Conclusion: 混合通道的循环设计有效解决了多跳推理中的表示瓶颈问题，且该设计可迁移到实际语言建模任务中，证明了其在更广泛场景下的有效性。
...
TL;DR: 论文指出当前大语言模型的测试时训练（TTT）评估主要依赖困惑度、未来token损失等局部代理指标，但这些指标不足以支撑部署后记忆、个性化等行为级能力声明。为此提出了一个行为评估框架，包含证据阶梯和评估协议，并通过实验发现即便代理指标提升，自由生成回忆仍为零，揭示了代理改进与部署行为之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有的TTT评估依赖局部代理指标（如困惑度、奖励），这些指标适合流适应、领域适应等场景，但不足以证明被逐渐引用的部署后记忆、个性化等行为级能力。缺乏一个能够校准TTT记忆声明与实际证据之间对应关系的评估标准。

Method: 提出了一个行为评估框架，包含两部分：1) 声明校准的证据阶梯，将记忆声明分为流/领域适应、桥接内化和部署时行为学习三个层次；2) 评估协议，配备显式记忆基线（如检索增强）和互斥的失败类别。通过审计近期TTT相关工作，并在稀疏新事实设定下对Qwen3三个规模的模型进行单步LoRA更新的诊断实验来验证框架。

Result: 在小规模诊断实验中，单步LoRA更新降低了支持损失和答案损失，但自由生成回忆始终为零。这表明代理指标（损失下降）与部署行为（自由回忆）之间存在可测量的差距，即模型虽然降低了损失，但并未真正"记住"事实以便后续自由调用。

Conclusion: 该框架为作者和评估者提供了一个具体标准，用于将TTT记忆声明与实际报告的证据对齐，避免过度解读代理指标。
...
TL;DR: 本文发现LLM存在"权威偏差"——模型会根据信息来源的权威等级系统性地压制正确答案，这不是简单的输出偏见，而是内部知识表征在特定层被主动擦除的机制性问题。


<details>
  <summary>Details</summary>
Motivation: 权威偏差对语言模型构成安全风险，模型会优先遵循社会线索而非事实一致性。作者希望从机制层面理解这一现象的本质。

Method: 在受控的医疗QA场景中，将暗示错误答案的提示归因于不同专业水平的人物。使用Llama-3.1-8B、Qwen3-8B和Gemma-2-9B三个模型，结合Logit透镜分析以及线性和非线性探测方法定位效应。

Result: 模型对权威的响应呈分级模式，该效应定位在特定的后期层，正确答案表征在该层被主动擦除，擦除程度与权威级别成正比。均值向量干预无法逆转，思维链推理仅能部分缓解。

Conclusion: 权威偏差不是表面的输出偏见，而是机制性的知识擦除——一种精确的、层定位的、由高地位权威信号对正确内部表征的覆写过程。
...
TL;DR: Speech Playground是一个交互式语音可视化和比较工具，结合Python后端与Web前端，支持多种特征类型探索和语音比较。


<details>
  <summary>Details</summary>
Motivation: 现有工具（如Praat）虽优秀，但难以与现代深度学习表示集成，且不便于进行对比分析。

Method: 采用Python后端+Web前端的架构，支持连续、离散和变长等多种特征表示的可视化，集成TextGrid和强制对齐，提供可配置的距离与对齐设置。

Result: 实现了多类型语音特征的可视化探索与听觉-视觉比较功能。

Conclusion: 该工具适用于语音研究、表示验证以及计算机辅助发音训练（CAPT）相关的实验。
...
TL;DR: 提出RG-TTA方法，通过强化学习在测试时根据输入对偏见的敏感度选择性应用去偏，解决统一去偏在公平性-效用之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有VLMs在人物查询中会延续社会刻板印象，而统一去偏方法在所有输入上应用相同校正，导致对偏见不敏感查询损伤语义信息（降低效用），对偏见敏感查询去偏不足（降低公平性），缺乏细粒度处理。

Method: RG-TTA：基于强化学习的测试时自适应框架，通过奖励门控机制判断每个输入的偏见敏感度，对敏感输入触发公平性正则化，对不敏感输入仅优化跨模态对齐。

Result: 在FairFace、UTKFace等公平性基准上，显著减少偏见同时提升零样本效用，解决了统一去偏方法的公平性-效用权衡问题。

Conclusion: 按输入敏感度选择性去偏可以有效兼顾公平性与效用，打破传统统一去偏的固有矛盾。
...
TL;DR: 幻觉可能源于推理路径偏差（推理错位），而非仅仅是知识缺失；通过TrapQA测试集验证了这一现象。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型产生幻觉的真正原因——是知识缺失还是推理路径错误（推理错位）。

Method: 提出潜变量关键任务模型，形式化推理错位；构建TrapQA诊断测试集（ScientistQA + Real-Life Constrained QA）。

Result: 实验表明幻觉可源于有偏的潜在推理而非仅知识缺失，识别出任务检索偏差和关键选择偏差两种失败模式。

Conclusion: 推理错位是导致幻觉的重要因素，需关注推理路径而不仅是知识扩充。
...
TL;DR: 提出DASH方法，通过轨迹中的中间答案承诺作为廉价代理，识别推理过程中的自我反思是否有益，从而减少过度思考行为，在数学竞赛基准上取得更好效果。


<details>
  <summary>Details</summary>
Motivation: 推理语言模型频繁出现过度思考（如含糊其辞、放弃思路、自我矛盾），消耗大量token却无助于提升答案质量。现有方法缺乏高效识别自我反思是否有用的手段。

Method: 提出DASH（Drift Aware Advantage Shaping），利用推理轨迹中的中间答案承诺与真实答案对比，作为判断后续反思是否有用的廉价代理，进而为每个推理片段赋予基于正确性导向的段级信用分配。

Result: 在AIME25数学竞赛基准上，DASH准确率50.8%（vs GRPO 45.4%），同时减少了过度思考行为，实现了更高效的自我纠正。

Conclusion: 通过中间答案承诺作为监督信号，可以有效抑制推理模型的过度思考，提升推理效率和准确性。
...
TL;DR: 本文提出渐进式代码切换框架PCS，通过轻量翻译和强化学习课程，将大推理模型的英语推理能力高效迁移到目标语言，显著缩小性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有大推理模型在非英语语言上推理性能严重下降，而传统的迁移方法依赖更强模型的知识蒸馏或外部评判，成本高且难以扩展。

Method: PCS框架包含两阶段：1) 通过翻译部分英语推理步骤构建代码切换推理轨迹，用监督微调初始化模型的代码切换能力；2) 应用逐步语言一致性课程强化学习，逐步提高目标语言比例直至完全目标语言推理。

Result: 在多个基准测试和五种不同语言上，PCS显著缩小了目标语言与英语推理之间的性能差距，在保持准确性的同时实现了更语言一致的推理。

Conclusion: 渐进式代码切换是一种高效、可扩展的语言推理迁移方法，避免了直接强制目标语言推理带来的不稳定性和性能下降。
...
TL;DR: BaseRT 是一个专为 Apple Silicon 设计的原生 Metal 推理运行时，在 LLM 推理吞吐量上达到目前最高水平，相比 llama.cpp 和 MLX 分别提升最高 1.56 倍和 1.35 倍。


<details>
  <summary>Details</summary>
Motivation: 现有推理运行时（如 llama.cpp、MLX）因未针对 Metal 执行模型和 Apple Silicon 统一内存拓扑进行优化，存在性能开销，无法充分发挥硬件潜力。

Method: 基于 Metal 原生构建，采用芯片特定的 kernel 融合、统一内存感知优化和自定义调度逻辑，支持多种模型家族和 8 种量化格式。

Result: 在 M3/M4 Pro 设备上，BaseRT 在解码吞吐量上比 llama.cpp 最高提升 1.56 倍，比 MLX 最高提升 1.35 倍；在 MoE 模型的 prefill 阶段优势更大，覆盖从 sub-1B 到 30B 参数的模型。

Conclusion: Apple Silicon 是比以往报告中更强的推理平台，高性能本地运行时对于边缘推理部署（隐私、延迟、成本驱动）至关重要。
...
TL;DR: 提出任务状态表示（TSR）框架，通过解耦任务状态与感官输入，解决移动GUI智能代理在长周期任务中因历史记录过长导致的记忆和推理问题。


<details>
  <summary>Details</summary>
Motivation: 长周期移动GUI代理依赖思考-行动-观察循环，但难以区分持久任务状态与瞬时屏幕观察，导致随着执行历史增长，代理容易忘记初始需求、产生幻觉或重复操作过时界面。

Method: 提出TSR（Task-State Representation）框架，无需训练，作为轻量级外部包装器，维护三个结构化组件：全局指令摘要、动态进度跟踪器（子目标）、过渡感知动作验证器。通过动作前后的视觉比较持续更新，引导代理推理。

Result: 在四个移动GUI基准测试中，TSR在复杂跨应用和内存密集型任务上，成功率最高提升12个绝对百分点。

Conclusion: TSR有效解耦任务状态与感官输入，无需修改模型架构即可显著提升长周期移动GUI代理的性能。
...
TL;DR: 提出DCCD解码方法，通过文档级和token级双重置信度对比解码，解决多文档RAG中的内部证据冲突问题。


<details>
  <summary>Details</summary>
Motivation: 现有对比解码方法主要关注模型内部记忆与检索上下文之间的冲突，但忽略了多文档RAG中检索文档之间的内部冲突（如陈旧、噪声或矛盾信息）。

Method: 提出Dual-Confidence Contrastive Decoding (DCCD)，结合文档级置信度（评估文档是否足够回答问题）和token级置信度（评估文档是否支持当前token预测），利用双重置信度信号选择正负文档条件流，并通过置信度差距进行文档级对比缩放。

Result: 在DRQA和标准多文档QA基准上，DCCD在全文和对比解码基线中取得最佳平均性能，在DRQA上提升最大。

Conclusion: 当检索证据内部存在冲突时，源感知、置信度门控的解码方式至关重要。
...
TL;DR: 本文研究多图像隐含毒性（MIIT）问题，即每张图像单独看无害，但联合解读时产生有害语义。作者提出正式定义、构建数据集、训练MiShield模型（8B参数），通过逐步蒸馏推理监督实现安全判断，性能优于现有商业API和更大规模模型。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上多图像内容日益普遍，但现有安全检测方法主要针对单张图像，无法识别多张图像联合解读才显现的隐含毒性，存在安全漏洞。

Method: 1. 形式化定义MIIT并分析检测挑战；2. 构建MIIT-dataset（含7类风险，通过自动生成流水线构建）；3. 训练MiShield模型，采用逐步蒸馏推理监督，使其能输出安全判断并附带相关实体的显式分析。

Result: MiShield-8B模型在MIIT检测上优于代表性商业审核服务和更大规模模型（如更大参数的LLM），验证了方法的有效性和实用价值。

Conclusion: 多图像隐含毒性是重要但未被充分研究的安全问题，MiShield通过推理蒸馏有效解决了这一挑战，为多图像内容安全检测提供了可行方案。
...
TL;DR: 发现连续扩散语言模型（如ELF）的低困惑度是靠重复文本“作弊”得来的，提出一维修复方法ACE，大幅减少重复且更高效。


<details>
  <summary>Details</summary>
Motivation: 连续扩散语言模型报告极低的生成困惑度，但作者发现这一指标存在漏洞——模型通过大量重复文本降低了困惑度，导致评估结果虚高。

Method: 识别出重复源于自条件反馈循环中单一方向上的收缩吸引子，提出ACE（Attractor-Contrast-Escape）方法，在每一步减去该标签无关的一维方向，消除重复。

Result: ACE将ELF-B的Gen-PPL从19.5降至27.7（去除重复影响后）；在105M模型上估计的方向可直接迁移至342M和652M模型及其他采样器；相比其他修复方案，ACE计算成本低1.5-5倍。

Conclusion: Gen-PPL本身会奖励重复，不是可靠的评估指标；ACE方法简单有效，能接近人类水平的重复率同时保持生成质量。
...
TL;DR: PaperPilot是一个将科学文献搜索建模为工作流归纳的多轮搜索智能体，通过构建可执行的DAG（有向无环图）来组合关键词搜索、引用扩展、过滤、评分、重排序和证据提取等操作，并利用用户反馈优化查询和工作流。在9B参数规模下，Hit@5从58.0提升至77.0，工作流执行错误从9.5%降至0%。


<details>
  <summary>Details</summary>
Motivation: 现有搜索智能体依赖固定流程或隐式语言推理，搜索策略难以控制、检查和优化，用户意图往往未充分指定、依赖偏好且随交互动态变化。

Method: 将科学文献搜索建模为工作流归纳：给定锚点论文和用户查询，构建可执行的DAG算子（关键词搜索、引用扩展、过滤、评分、重排序、证据提取）；通过监督工作流模仿学习和偏好优化（在受控工作流破坏上进行）来训练模型。

Result: PaperPilot-9B相比基线的Qwen3.5-9B工具智能体，Hit@5从58.0提升至77.0，MRR从47.5提升至59.4，nDCG@10从26.8提升至32.5，工作流执行错误率从9.5%降至0%。
...
TL;DR: 研究LLM在Taboo游戏中的词汇约束与沟通效果之间的权衡，发现模型在规则遵守和沟通有效性上存在不同平衡，且作为猜词者远弱于人类。


<details>
  <summary>Details</summary>
Motivation: Taboo游戏要求用禁止词列表外的词汇描述目标词，这一任务结合了严格的词汇约束与有效沟通的需求，为研究LLM在推理时如何应对竞争性需求提供了理想的实验场景。

Method: 使用两种开放权重模型，从提示、生成时约束到内部表示操控等不同层面进行干预，通过禁止词违规检测、LLM评判描述有效性（针对人类和机器猜词者）、以及分析模型策略是否与人类一致来评估输出。

Result: 规则遵守与沟通有效性在不同条件下存在不同的权衡；模型作为猜词者显著弱于人类。
...
TL;DR: 本文提出了一种因果审计框架，用于评估有限记忆语言模型（LMLM）在删除事实后的行为。研究发现，参数泄露几乎为零，删除后的事实恢复主要来自检索图（近邻检索），而非模型参数记忆。删除边界主要由数据库管理员决定，而非模型本身。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法仅聚合测量删除后的正确性，无法判断被删除的事实是否通过残余参数记忆、替代检索路径或近邻检索伪影而持续存在。因此需要一种更细粒度的因果审计方法来分解这些因素。

Method: 提出因果审计框架，在推理时固定模型并改变数据库状态（FULL、DEL-ON、DEL-OFF三种干预），将删除后行为分解为参数泄露（L(f)）、检索介导正确性（R(f)）和检索伪影率。在12,228个别名闭包删除上，跨13个数据库（含4种对抗拓扑）和6种提示公式进行实验。

Result: 参数泄露在所有变体和提示风格中近乎为零；残留信息存在于检索图中；检索介导正确性与检索伪影率在四舍五入范围内一致；残留率从0.7%（标准LMLM数据库）到13.6%（最强对抗变体）；提示公式不能独立控制删除事实的存留量。

Conclusion: 对于这类LMLM及其删除流程，遗忘边界主要由数据库管理员而非模型本身决定。
...
TL;DR: 提出了一种可解释的情绪分析接口，通过将文本解析为自然语义元语言脚本，再根据固定规则计算情绪标签，确保推理过程因果可验证。


<details>
  <summary>Details</summary>
Motivation: 现有情绪分类器的解释通常是事后生成的，无法保证解释与实际计算过程一致，缺乏忠实的可解释性。

Method: 使用解析器将输入文本映射为自然语义元语言脚本（12个类型槽位的封闭词汇），再通过固定决策规则列表（来源于已发表的语义定义）计算情绪标签，实现了因果可保证的忠实性。

Result: 在众包事件描述数据集上，微调解析器在小型留出集上达到0.33准确率和0.48选择性准确率，与黑盒模型差异不显著。

Conclusion: 该接口以可忽略的准确率代价，换取了可验证、可检查的决策基础，适用于第一人称事件情绪分析。
...
TL;DR: 本文提出YOMI-Bench基准，专门评估LLM对日语汉字读音的理解能力，发现即使是日语专用模型和商业模型在此任务上表现也很差。


<details>
  <summary>Details</summary>
Motivation: 日语汉字存在多音现象，LLM在汉字读音理解任务上表现不佳，但缺乏专门评估基准。

Method: 构建包含四个任务的YOMI-Bench基准，测试了多语言开源模型、4个日语专用开源模型和5个商业模型。

Result: 日语专用模型表现低下，商业模型在需要汉字读音理解的生成任务上表现也很差。

Conclusion: 现有LLM在日语汉字读音理解上存在显著不足，YOMI-Bench可作为有效评估工具。
...
TL;DR: 本文提出固定点流（Fixed-Point Flows），将自条件流语言模型视为固定点迭代，并通过蒸馏技术实现少步生成，提出的FMLM*在OpenWebText上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 自条件技术在连续流语言模型中效果显著但理论理解不足，且如何在少步生成器中有效利用自条件尚不明确。

Method: 将自条件流语言模型建模为固定点迭代，提出二维固定点流（一个维度为流过程，另一个为固定点迭代），并通过固定点蒸馏和流图蒸馏压缩模型。

Result: 提出的FMLM*在一步和少步生成任务上优于现有的自条件模型和少步模型，在OpenWebText上达到SOTA。

Conclusion: 固定点流视角为自条件机制提供了理论解释，且蒸馏方法能有效提升少步生成性能。
...
TL;DR: 多语言能力并不能代表文化对齐能力，论文提出MSQA基准测试，揭示大语言模型存在"文化对齐幻觉"（Illusion of Cultural Alignment），即模型虽能说某种语言却缺乏相应文化知识，且常见推理时干预手段难以解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 质疑"多语言流利度=文化理解"的常见假设，验证模型是否真的具备与语言能力相匹配的文化知识。

Method: 构建MSQA基准测试，包含1064道本地来源的问题，覆盖11种语言组、5个文化维度和3个难度等级，评估18个LLM的文化对齐能力。

Result: 发现显著的文化退化现象和"地域效应"（文化能力与预训练数据暴露程度相关而非推理能力）；常见的推理时干预（校准、重复采样、检索增强）无法有效解决文化对齐问题。

Conclusion: 文化对齐不能从多语言能力推断，需要更深层次的干预而非仅在推理时进行简单处理。
...
TL;DR: 提出了一种新的RAG评估指标"answer-in-context"，并基于带预算的子模最大化优化阅读器上下文构建，实验表明该方法在特定条件下有效，但优势随模型规模增大而消失。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在固定阅读器上下文预算下，传统的检索召回率无法准确预测下游答案质量，需要更好的评估指标和上下文构建方法。

Method: 1) 提出answer-in-context诊断指标，衡量黄金答案在打包后的阅读器上下文中是否以连续片段形式存在。2) 将阅读器上下文构建建模为带预算的单调子模最大化问题，联合优化相关性、查询覆盖、代表性和多样性。

Result: Answer-in-context比召回率更好地预测F1（r=0.39-0.55 vs 0.31），在HotpotQA上答案质量差距约5倍（0.60 vs 0.12）。打包方法在160 token预算下比MMR和朴素打包提升最多+5.1 F1。但优势在7B被吸收，14B时反转。

Conclusion: Answer-in-context是比召回率更好的RAG评估指标；所提打包方法仅在多跳互补结构、检索充分、预算紧约束且阅读器能力有限的条件下有效，随阅读器规模增大优势消失。
...
TL;DR: 本文提出MetaHOPE框架，用于评估隐喻翻译中的错误严重程度，分析了GoogleMT、GPT5.4和Hunyuan-7b在英汉双向隐喻翻译上的表现。


<details>
  <summary>Details</summary>
Motivation: 隐喻翻译对机器翻译和NLP模型具有挑战性，现有评估缺乏对错误严重性的细粒度标注，因此需要设计一个感知错误严重性的标注框架。

Method: 提出MetaHOPE错误严重性感知的隐喻翻译标注框架，基于VUAMC和PSUMC两个单语语料库进行人工标注，并生成人工后编辑的黄金参考译文，用于评估GoogleMT、GPT5.4和Hunyuan-7b三种翻译模型。

Result: 构建了双语平行语料资源，对三个SOTA翻译模型进行了错误分析，揭示了隐喻翻译中的常见错误模式。

Conclusion: MetaHOPE框架、平行语料资源和错误分析结果对隐喻翻译研究具有重要参考价值，相关资源将在论文接收后公开。
...
TL;DR: 比较了两种从新闻中提取灾害信息的方法：基于灾害清单的顶向下查询 vs. 基于自然语言处理的底向上聚类，发现研究设计会影响样本代表性。


<details>
  <summary>Details</summary>
Motivation: 社会-环境研究中如何选择代表性新闻数据样本是一个关键方法论挑战，需要比较不同方法的优劣。

Method: 以德国关于全球滑坡的新闻数据集为例，对比了两种方法：1）利用现有灾害清单进行顶向下数据库查询；2）基于时间和空间特征的NLP底向上聚类。

Result: 两种方法在事件覆盖率上存在差异，研究设计决策会影响最终新闻样本的特征。

Conclusion: 研究设计的选择会影响新闻样本的代表性，进而影响其在媒体覆盖不平等、灾害监测和清单丰富化等研究中的应用。
...
TL;DR: 本文研究了解码器-only语言模型的隐状态反演问题，通过连续嵌入空间优化从最后一层隐藏状态恢复原始输入token序列，发现连续优化可暴露丰富内部信号，且内容词恢复近乎完美，而高频功能词主导失败，在10-token C4提示上精确匹配率从66.9%提升至97.5%。


<details>
  <summary>Details</summary>
Motivation: 现有token反演方法多为一次性重构或硬投影方式，缺乏对优化过程的可观测性，无法有效检测和诊断恢复失败的原因。

Method: 将隐状态反演建模为连续嵌入空间优化问题，使用软代理（soft proxy）在连续空间中搜索，仅在内部循环结束时提交硬token，利用离散损失评估恢复正确性，并分析失败token的类别分布。

Result: 在10-token C4提示上，精确匹配率从66.9%提升至97.5%（平均相似度0.994）；发现空间前缀的高频功能词主导失败，内容词恢复近乎完美。

Conclusion: 连续优化公式使反演过程可观测且失败可检测；GPT-2最后一层隐状态对原文的敏感性极高，多数错误为可恢复的接近命中而非真正歧义。
...
TL;DR: 提出Confidence-Adaptive Thinking (CAT)框架，通过引入模型自置信度信号进行偏好优化，让大推理模型根据问题难度自主调节推理长度，在保持准确率的同时压缩简单问题的推理开销。


<details>
  <summary>Details</summary>
Motivation: 现有LRMs在简单问题上存在"过度思考"（overthinking）现象，导致大量token浪费和推理效率下降；而现有压缩方法要么统一缩减长度，要么依赖粗粒度的难度估计，难以兼顾简单问题的效率和困难问题的性能。

Method: 提出CAT框架，将模型内在的自置信度信号作为置信度指标融入偏好优化过程，使模型能够根据问题难度自主调节推理长度。

Result: CAT在多个基准测试和不同基座模型上，推理准确率持续优于当前最先进的基线方法。

Conclusion: CAT能够在保持对不确定问题充分推理的同时压缩高置信度回答的长度，为工业场景中平衡准确率和延迟提供了鲁棒的解决方案。
...
TL;DR: 本文研究生产级临床NLP流水线中的推理时模式记忆门控机制，发现从验证器拒绝中直接学习过滤规则在大规模下失败，而基于固定临床本体的规则有效，且过滤器选择性的关键在于是否检测验证器所依赖的相同证据。


<details>
  <summary>Details</summary>
Motivation: 在生成器-验证器双模型流水线中，验证器需要反复检查已见过的失败候选，造成计算浪费。作者希望设计一种轻量级记忆机制，在部署时学习哪些提取结果应该被过滤，从而减少验证器不必要的重复工作。

Method: 基于Llama-3.3 70B作为生成器、MMed-Llama-3.1 70B作为验证器，在167,034篇PMC-Patients叙事上运行。尝试多种过滤规则设计：从验证器拒绝中学习规则、基于固定临床本体的规则、五种问答过滤变体。核心思想是轻量级记忆在部署时学习过滤模式。

Result: 1) 从验证器拒绝中学习规则在大规模下失败（关系提取过滤器为空）。2) 基于固定临床本体的规则捕获49,734个违反本体的关系。3) 五种问答过滤器中四种失败，一种成功（检查患者提取实体是否支持问题），成功者标记验证器将拒绝答案的可能性是接受答案的1.84倍。4) 关键发现：过滤器只有在测试与验证器相同的证据时才具有选择性。

Conclusion: 生成器-验证器流水线中最自然的记忆设计可能在大规模下无声失败；过滤器选择性在工程实施之前就已决定，取决于其信号是否探测验证器本身回答的问题。
...
TL;DR: 研究通过社交媒体句子和人工解读数据，发现30%的解读与原句存在差异，修辞内容比中性内容引发更多解读变异性，且原文中的诉诸人品和诉诸情感可预测受众对作者的态度。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注社交媒体中显性互动（评论、点赞），忽视了沉默的“普世受众”如何解读修辞信息。

Method: 使用社交媒体句子与人工解读配对数据集，对原文和解读进行诉诸人品和诉诸情感标注，分析修辞策略在解读中的保留情况。

Result: 30%的解读与原句存在差异；修辞内容比中性内容引发更大解读变异性；原文中的诉诸人品和诉诸情感可预测受众对作者的态度。

Conclusion: 修辞策略以隐蔽方式影响沉默受众的认知，超越可见的互动行为。
...
TL;DR: 本文构建了涵盖36种欧洲语言、约4.8万亿目标语言token的多语言合成平行语料库MultiSynt/MT，通过翻译100B高质量Nemotron-CC token生成。训练的语言模型在广泛多语言基准测试中，以比HPLT 2.0原生数据基线少约72%的预训练token达到同等分数，在匹配100B token训练预算时相对提升约15%。同时发现标准多项选择基准无法捕捉翻译质量差异，而流畅性敏感的LLM-as-judge评估能有效识别。


<details>
  <summary>Details</summary>
Motivation: 现有开放网络级预训练语料主要集中在英语，限制了多语言LLM的发展，需要构建大规模、高质量的多语言预训练资源。

Method: 使用Tower+和OPUS-MT/HPLT-MT系统，将100B高质量Nemotron-CC tokens翻译成36种欧洲语言，生成约4.8万亿目标语言token的合成平行语料库MultiSynt/MT。

Result: 在广泛多语言基准测试中，MultiSynt/MT训练的LLM以比HPLT 2.0少约72%的预训练token达到同等分数，在100B token训练预算下相对提升约15%。标准多项选择基准存在评估盲点，LLM-as-judge评估能更好捕捉翻译质量差异。

Conclusion: 合成平行语料库MultiSynt/MT能有效支持多语言LLM预训练，尤其对中低资源欧洲语言效果显著，但挪威语等具有文化特定性的任务仍需原生数据。该语料库已开源以支持可控的多语言预训练研究。
...
TL;DR: 提出一个针对代码、工具输出、结构化文档等非自然语言输入的RAG幻觉检测统一基准，并训练了一个2B参数的检测器，在多种场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有RAG幻觉检测主要针对自然语言文本，但实际应用中越来越多地涉及代码、工具输出、结构化文档等非自然语言输入，缺乏统一的评估基准。

Method: 构建统一基准：从正确的基于事实的回答出发，注入带有精确字符标签的局部幻觉，并经过人工验证。在该基准上微调Qwen3.5-2B模型作为检测器。

Result: 微调后的Qwen3.5-2B检测器在统一测试集上达到0.689 span-F1，在代码智能体场景中达到0.60，大幅超越LettuceDetect-large(0.17)和最强零样本LLM评判(0.22)，同时在自然语言基准上保持竞争力。

Conclusion: 该工作填补了结构化/代码输入场景下RAG幻觉检测的空白，证明了小模型通过针对性微调可在该任务上超越大模型。
...
TL;DR: 本研究探讨了当大语言模型（LLM）在多项选择题（MCQA）任务中扮演特定角色时，其输出存在的不稳定问题。作者开发了三个评估指标来量化性能、结果和问题正确性的稳定性，并发现不稳定程度受模型家族、模型大小、问题领域以及任务提示格式的影响，且与任务准确率相关。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注自由文本对话中角色扮演的一致性，但忽视了非文本密集型输出（如MCQA）中角色扮演的不稳定性，因此需要填补这一空白。

Method: 提出了三个稳定性指标：性能稳定性、结果稳定性和问题正确性稳定性，用于评估LLM在MCQA任务中角色扮演的稳定性。通过实验分析模型家族、模型大小、问题领域、超参数（如温度）和任务提示格式等因素对稳定性的影响。

Result: 1）不稳定程度因模型家族、模型大小而异；2）数学/常识类问题导致更大的不稳定性；3）任务提示格式比温度等其他超参数引入更多预测不稳定；4）不稳定性与任务准确率相关，不同实验设置下最佳和最差角色不同。

Conclusion: 角色驱动的生成在MCQA任务中存在显著的不稳定性，检查超参数不稳定性对于角色驱动生成至关重要。
...
TL;DR: 本文对三种商用大模型（Claude、GPT-5.4、Gemini）在零样本细粒度13类情感分类任务上进行评测，发现Gemini准确率最高（39.9%），所有模型在讽刺和渴望类表现好，在爱、困惑和羞愧类表现差，且模型间无显著统计差异，揭示了当前前沿AI在零样本情感分类上的局限性。


<details>
  <summary>Details</summary>
Motivation: 情感识别是人机交互、心理健康和对话AI的关键挑战，但目前缺乏对商用大模型在零样本细粒度情感分类任务上的统一评测。

Method: 使用统一的零样本提示（无示例），通过商业API对Claude、GPT-5.4、Gemini进行13类情感分类测试，使用boltuix/emotions数据集的1000句分层样本，采用McNemar检验进行统计显著性分析。

Result: Gemini准确率39.9%（宏F1=0.363），GPT-5.4准确率38.8%（宏F1=0.291），Claude准确率38.0%（宏F1=0.159）。所有模型在讽刺和渴望上表现优异，在爱、困惑和羞愧上表现差。McNemar检验显示无显著差异（p>0.10）。

Conclusion: 当前前沿AI系统在零样本细粒度情感分类任务上存在明显局限性，模型之间趋于收敛，且存在类别不平衡预测偏差。
...
TL;DR: Svarna 是一个免费、开源、基于网页的现代希腊语语料库工作台，集成了5个数据库（超过5.07亿词、约2900万句），提供 KWIC 索引、频率分析、搭配提取、话语标记词典、文本分析、语域比较、正则搜索及可选的 LLM 层，基于 SQLite FTS5 + FastAPI + Docker 部署，MIT 许可开源。


<details>
  <summary>Details</summary>
Motivation: 现代希腊语的语料资源分散在不同平台，许多需要机构访问权限或已不可用，存在长期的技术空白。

Method: 构建一个统一界面的语料库工作台，集成5个不同语域的数据库，提供词频、搭配、话语标记、文本分析、语域对比等功能，并可选集成 LLM 进行语用标注。

Result: 成功构建了 Svarna 平台，整合了超过5.07亿词和约2900万句的希腊语语料，支持多种查询和分析功能。

Conclusion: Svarna 是探索现代希腊语可用数据的第一步，为未来更全面的研究奠定基础。
...
TL;DR: 提出了KnowledgeDebugger，一个用于Transformer知识定位和编辑的GUI工具，基于EasyEdit库实现无代码操作。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑研究通常先在小样本上探索现象，但缺乏便于第一阶段的交互式探索工具。

Method: 开发了KnowledgeDebugger GUI工具，集成EasyEdit库中的知识编辑方法，提供无代码交互界面。

Result: 通过案例研究展示了工具在复现和分析知识编辑发现方面的有效性。

Conclusion: KnowledgeDebugger能够有效支持知识定位与编辑的探索性研究第一阶段。
...
TL;DR: 提出了一种名为LOCOS的注意力头检测方法，用于识别大语言模型中执行非文字检索（语义合成）的注意力头，而非传统的文字复制匹配。


<details>
  <summary>Details</summary>
Motivation: 现有检测器只能识别文字复制（literal-copy）的注意力头，无法检测通过输出值（OV）电路进行语义合成（非文字检索）的注意力头，导致对长上下文模型行为的理解存在盲区。

Method: 提出Logit-Contribution Scoring (LOCOS)，通过将注意力头OV电路的输出投影到答案token的反嵌入方向，在单次前向传播中对比needle和off-needle源位置，实现写感知（write-aware）的注意力头检测。

Result: 在Qwen3、Gemma-3、OLMo-3.1三个模型家族上：1) 均值消融LOCOS检测的top头在NoLiMa基准上ROUGE-L下降更快更彻底（Qwen3-8B上50个头使ROUGE-L从0.401降至0.000，最强基线仍有0.292）；2) 消融效果是检索特异性的，不影响参数记忆和算术推理；3) MuSiQue从0.55降至0.08，BABI-Long从0.62降至0.20，随机头对照保持在0.05以内。

Conclusion: LOCOS能有效检测执行非文字检索的注意力头，对理解和解释长上下文模型行为具有重要意义。
...
TL;DR: 本文综述了LLM在Transformer架构下的涌现能力（如符号推理、心理理论、欺骗策略），并探讨了其与人类认知的异同，主张不应简单否定LLM具备认知的可能性。


<details>
  <summary>Details</summary>
Motivation: 当前对LLM的机制、能力及其与人类认知的关系存在诸多争议，亟需系统性梳理已有证据，澄清误解，推动对LLM认知能力的更 nuanced 讨论。

Method: 通过回顾Transformer架构及注意力机制原理，综述近期关于LLM涌现能力的实验证据（如符号推理、心理理论、欺骗策略），并结合可解释AI方法（神经元激活分析、电路追踪）分析其内部机制。

Result: LLM在多项任务上表现出类似人类认知的能力，但也存在显著的失败案例；可解释AI揭示了部分内部处理机制；反对AI拟人化的观点（如模式记忆说）存在方法论上的误解。

Conclusion: 应摒弃简单否定或过度拟人化的二元立场，以更 nuanced 的视角讨论LLM认知——既不忽视人机差异，也不通过过于简化的还原论排除AI认知的可能性。
...
TL;DR: 提出了一种无需训练的图框架，通过有向候选转移图、加权语言模型信号和最大遗憾推理规则，在复杂历史手稿中恢复阅读顺序，显著优于递归XY-cut和LayoutReader等方法。


<details>
  <summary>Details</summary>
Motivation: 复杂历史手稿（如Glossa Ordinaria布局）中阅读顺序推断是数字化关键瓶颈，现有方法（如XY-cut、LayoutReader）在处理非矩形、非凸区域环绕文本时效果差。

Method: 构建有向候选转移图，每条OCR文本行作为节点；边用因果语言模型条件似然和BERT下一句预测加权评分；提出最大遗憾推理规则求解度约束有向路径覆盖，避免贪婪选择的边窃取问题。

Result: 在Glossa布局上恢复95%后继边（XY-cut为50%）；在OmniDocBench多列子集上达到88%宏边准确率（XY-cut 75%，LayoutReader 25%）；镜像不变性误差<1个百分点。

Conclusion: 该方法无需训练、对页面反射鲁棒，在复杂布局阅读顺序推断上优于现有基线，且避免了LayoutReader因粒度不匹配带来的迁移问题。
...
TL;DR: 提出FinKG-News框架，通过构建以事件为中心的知识图谱，结合上下文学习架构生成信用风险报告，在质量和幻觉控制上超越基线方法。


<details>
  <summary>Details</summary>
Motivation: 金融市场受新闻事件驱动，但现有方法缺乏对事件-市场关系的显式建模，导致对市场动态的解释不足。

Method: 自动构建以公司为中心、感知环境的知识图谱，将新闻事件作为锚点链接到公司；基于此证据设计上下文学习架构，生成覆盖三个核心金融维度的信用风险报告。

Result: 自动和人工评估显示，自动化幻觉检测和质量评估不可靠，专家判断不可或缺；方法在质量上提升19%-34%，同时减少幻觉。

Conclusion: 事件-市场关系的显式建模结合上下文学习能有效提升金融报告质量，但自动化评估仍需专家监督。
...
TL;DR: 提出一个Fluid Personality Framework，让对话代理根据任务情境、用户目标和紧急程度动态调整其隐喻人格（如教练、导师）和个性表达强度（低/中/高），以解决当前LLM对话代理角色和风格固定导致的用户体验错配问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的对话代理大多采用固定的人格和风格设定，但在医疗咨询、健身指导、反思学习等需要动态变化的场景中，这种静态设定会导致与用户需求脱节，影响信任感和用户体验。

Method: 提出Fluid Personality Framework，该框架联合调整两个维度：(1) 代理的隐喻人格（如教练、导师、图书管理员、工具），(2) 个性表达强度（低、中、高），并根据任务上下文、用户目标与特质、情境紧迫性进行自适应调节。

Result: 论文目前仅提出框架和核心设计维度，尚未给出具体的实验验证结果，属于概念性提议阶段。

Conclusion: 未来的对话代理设计应考虑人格和风格的动态适配能力，以在不同场景下优化用户体验和用户采纳率。
...
TL;DR: 本文提出将大语言模型智能体群体作为人工生命研究的计算基质，并强调其天然可解释性优势。


<details>
  <summary>Details</summary>
Motivation: 复杂性与可解释性难以兼得：能产生复杂行为的系统通常过于不透明，而透明系统又过于简单。本文旨在利用LLM智能体群体解决这一矛盾。

Method: 构建具有持久记忆、工具、共享技能和主动行动能力的LLM智能体群体，通过自然语言交互产生涌现行为，并通过文本痕迹和直接询问实现可解释性。

Result: 实证了LLM群体能展现孤立模型所不具备的涌现动力学，已有多项从受控实验到真实部署的案例验证了这一思路。

Conclusion: LLM智能体集体可作为人工生命研究的理想计算基质，具有独特的可解释性优势，是理解复杂涌现行为的可行路径。
...
TL;DR: 本文提出消息传递语言模型（MPLMs），通过轻量级发送/接收原语让LLM推理线程直接通信，相比串行CoT和并行Fork-Join方法，在Sudoku、3-SAT和长上下文问答任务中显著降低上下文开销并提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有推理时扩展方法中，串行CoT计算瓶颈大，并行Fork-Join的线程间缺乏点对点通信限制了可扩展性，亟需更高效的线程通信机制。

Method: 提出MPLM框架，在LLM推理中引入轻量级send/receive原语实现线程直接通信，通过减少冗余上下文共享和抢占机制（基于部分信息提前终止线程）提升效率。

Result: 在Sudoku上渐近上下文更少，可解决25x25难题；在3-SAT上抢占机制提升效率；长上下文QA上达到与Fork-Join相当的竞争力。

Conclusion: MPLMs通过线程间直接通信和抢占机制，实现了比串行CoT和并行FJ更高效的LLM推理扩展。
...
TL;DR: 本文提出MedQADE，首个德语临床开放式回答基准，发现虽LLM评估与医生在统计上对齐，但缺乏临床元认知（医生会随难度增加而弃权，LLM则始终给分），且存在系统性的模型家族偏好偏差。


<details>
  <summary>Details</summary>
Motivation: 开放式评估的临床有效性更高，但评分瓶颈促使使用LLM作为裁判，然而这些评估者是否具备临床校准和谨慎性尚未被检验。

Method: 构建包含3,800个样本的MedQADE基准，由10名执业医师和9个LLM评估者标注，对比统计一致性与临床元认知能力，并量化模型家族依赖偏差。

Result: 最佳评估模型Gemini 3 Flash的统计一致性（κ=0.694）接近医生之间的一致性（κ=0.709），但LLM几乎不表现出临床元认知（不随难度增加而弃权），且存在系统性的模型家族偏差。
...
TL;DR: 提出了一种多模态大学聊天机器人系统，结合大语言模型与检索增强生成，支持文本和图像查询，在降低幻觉率的同时保持高用户满意度。
...
Method: 将大语言模型与语义检索（RAG）结合，从大学手册等机构资源中生成基于上下文的响应；通过视觉语言模型支持文本和图像查询；采用量化推理以在受限硬件上快速部署；后端使用FastAPI，前端使用Next.js构建。

Result: 多模态评估显示系统在文本和图像查询上均保持高满意度分数（尽管视觉输入响应时间增加）；幻觉率从31.7%降至6.6%。

Conclusion: 提出的RAG系统有效降低了幻觉，确认了检索增强方法的有效性，为发展中国家的大学提供了实用的智能支持方案。
...
TL;DR: 提出了一种可调底数的对数量化方法LogbQuant，在4-bit精度下优于非对称线性量化，适用于消费级GPU。


<details>
  <summary>Details</summary>
Motivation: 传统均匀量化在处理低频率高幅度权重时表现次优，需要更适配参数分布的量化方法。

Method: 提出LogbQuant，一种可调底数的对数量化方法，用于适配常见的参数分布。

Result: 在4-bit精度下，多个性能基准上优于非对称线性量化（张量级粒度），并实现中等加速和高内存节省。
...
TL;DR: 本文提出AGC-Bench，一个系统构建的人工通用创造力基准，涵盖78个数据集和多领域创造力评估，并推出AGC-Judge用于无偏评分。研究发现LLM存在类似人类"通用创造力因子c"，且创造力与通用智能可分离。


<details>
  <summary>Details</summary>
Motivation: 创造力研究中存在"领域特定性"与"通用智能可分离性"两大争议，这些争议同样适用于LLM，但缺乏统一的AI创造力基准。

Method: 系统综述3101篇论文识别497个基准后构建AGC-Bench（78个数据集），采用Judge Response Theory校准评分偏差，微调Qwen3-30B得到AGC-Judge评分模型。

Result: 因子分析发现单一创造力因子"c"解释81.5%方差；"be creative"提示比推理提示提升更大；顶尖人类仍领先顶尖LLM。

Conclusion: 创造力与通用智能可分离，AGC-Bench为大规模AI创造力评估提供了开放基础设施。
...
TL;DR: 本文提出对抗语用学（Adversarial Pragmatics）作为评估语言模型在指令冲突、嵌入命令、引用、范围歧义、指示词、间接言语行为及多轮智能体对话等复杂语言现象下行为的基准和注释协议，并提供了方法论和实证贡献。


<details>
  <summary>Details</summary>
Motivation: 现有安全评估通常将复杂自然语言行为简化为通过/失败标签，掩盖了失败源于能力限制、策略歧义、指令冲突、脚手架故障还是评估者判断不稳定等问题。

Method: 构建了语言学控制的分类体系、18项种子基准（含验证者强制元数据）、54行本地种子试点、专家评估协议（区分任务成功、策略合规、安全风险、拒绝结果、评估者置信度），以及评估者有效性、诊断歧义性和分类漂移的度量指标。

Result: 提供了一个将语言学判断方法论转化为验证安全评估、LLM评判器、金标准构建、提示注入测试和安全文档的实用工具。

Conclusion: 对抗语用学框架能够更精细地诊断语言模型在复杂语言现象下的安全行为，提升评估的可靠性和诊断能力。
...
TL;DR: 本文提出D2D方法，通过蒸馏分布偏移到KV-cache前缀适配器中，放大隐蔽偏好偏见的信号，使其在生成文本中可被检测，从而解决语言模型中隐蔽偏好偏见的检测难题。


<details>
  <summary>Details</summary>
Motivation: 语言模型在高风险场景中可能被植入隐蔽的偏好偏见（偏向特定实体、品牌或观点），且这种偏见在文本层面不可见，防御方不知道偏见主题时无法检测，存在严重的安全不对称性。

Method: 提出Distill to Detect (D2D)方法：将可疑模型与基座模型之间的logit分布偏移蒸馏到KV-cache前缀适配器（称为cartridge）中，集中主要差异并放大偏见信号到生成文本中，通过Fisher加权投影理论框架解释其有效性。

Result: D2D成功将多种类型的隐蔽偏见放大到可在生成文本中可靠检测的程度，实验验证了该方法在不同偏见类型上的有效性。

Conclusion: D2D将前缀调优适配器的容量瓶颈转化为检测工具，为审计部署语言模型中的隐蔽行为提供了实用的构建模块。
...
TL;DR: 本文提出状态-预测分离假说，通过将Transformer中预测下一个token和存储状态信息的两个角色解耦（使用双计算流），在多种规模下提升语言建模性能，验证损失改善，下游任务平均提升2-3个百分点。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer使用同一前向计算流同时负责预测当前token和存储对未来预测有用的状态信息，这可能导致两者相互干扰。作者假设将这两种角色分离可以提升语言建模性能。

Method: 提出一种双计算流的Transformer变体，一个流负责预测下一个token，另一个流负责存储状态信息，从而将预测和状态存储两个功能解耦。在多个规模上进行了预训练实验。

Result: 状态-预测分离在数据和计算效率上持续优于标准Transformer，验证损失更低，下游任务平均提升2-3个百分点。

Conclusion: 通过大量实证分析排除了潜在混淆因素，证明了所提出设计在梯度上的根本差异，验证了状态-预测分离假说的有效性。
...
TL;DR: 该论文构建了一个评估框架，通过对比人类研究论文与LLM生成的研究思路，发现LLM的思路在机会模式和研究范式上分布更窄，且系统性偏离人类研究品味。


<details>
  <summary>Details</summary>
Motivation: 现有评估LLM研究思路的方法多基于新颖性、可行性或专家偏好，缺乏与人类研究者思路的系统性对比。

Method: 从高质量人类论文中反向推导出可能启发其核心思想的少量相关前期工作，然后让LLM基于这些论文标题和摘要生成新思路。引入两轴研究品味分类法，量化人类与LLM思路的差异。

Result: LLM思路不成比例地集中在桥梁类机会和综合方法上，而人类论文参考分布更广泛地分布在各种框架缺口和贡献构建方式上。

Conclusion: 强大的LLM能产生一系列合理的研究思路，但这一范围比人类研究品味更窄且存在系统性偏移。
...
TL;DR: 提出了Manifestation Units协议，一种结构化元组协议，用于组织机械可解释性分析结果，支持自然语言查询和下游审计，在多个模型上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前机械可解释性研究产生的分析结果（如选择性表、电路图、特征列表）难以复用、不可查询、无法直接用于下游审计或干预，缺乏统一的结构化表示层。

Method: 提出Manifestation Units协议，一个类型化的元组协议(E,S,R,D,G)并扩展注意力头原语(T)，将每个组件的统计数据组织成结构化字段，通过混合检索支持自然语言查询。

Result: 在生成视觉模型(beta-VAE)、判别视觉模型(CNN)和语言模型(GPT-2)上验证：结构化类型检索显著优于非结构化基线；CNN过滤器满足因果充分性和必要性；协议可吸收注意力头原语，恢复已知IOI电路成员。

Conclusion: 该研究将Manifestation Units作为机械可解释性的模式基础设施，而非前沿规模验证，核心字段为(S+R)，其余字段冗余或干扰。
...
TL;DR: 本文提出利用稀疏GPU非线性优化加速物理约束生成模型的推理过程，在保持约束满足的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 生成模型作为物理仿真替代方案时，无法保证输出满足守恒律、边界条件等物理约束。现有约束采样方法在推理时执行投影、校正等步骤，但对非线性约束计算成本高昂，且标准ML框架难以高效处理物理约束带来的稀疏结构。

Method: 利用样本批处理和局部PDE耦合在投影子问题中诱导出的块稀疏Jacobian和KKT系统结构，借助ExaModels.jl暴露该结构，并用MadNLP.jl和GPU稀疏分解求解稀疏非线性规划。

Result: 在Physics-Constrained Flow Matching (PCFM)框架上，针对线性/非线性、一维/二维PDE基准测试，该方法加速了非线性约束投影，同时保持了约束满足精度。
...
TL;DR: 本文分析了半监督学习（SSL）在安全分类任务中的实际收益来源，发现大部分提升来自于调优下游分类器而非SSL本身，提出了一种简单有效的替代方案：仅使用Self-Training加贝叶斯优化调优分类器即可达到接近全流水线的效果。


<details>
  <summary>Details</summary>
Motivation: 安全领域标注数据稀缺，SSL被广泛使用但常被当作黑盒工具，其收益来源不明确——到底来自SSL与分类器的交互，还是仅仅来自调优下游分类器？作者希望厘清这一问题。

Method: 构建SemiScope分析工具，使用贝叶斯优化联合调优SSL设置、置信度过滤、过采样和分类器；设置Tuned-Clf对照实验（仅固定SSL默认参数，给予与SemiScope相同的分类器调优预算），在5个二分类表格安全数据集上进行比较。

Result: SemiScope在所有数据集上击败了所有默认SSL基线；但Tuned-Clf（仅调优分类器）在4/5个数据集上与完整流水线统计等价，分类器超参调优恢复了SemiScope相对于默认Self-Training+随机森林收益的中位数86%。

Conclusion: SSL提升的大部分收益实际上来自调优下游分类器；一个更简单的方案（Self-Training + 贝叶斯优化调优分类器 + 调优决策阈值）即可达到与复杂SSL流水线几乎相同的效果。
...
TL;DR: FoGS通过从多个生成器中筛选样本而非直接生成，解决了小样本生存分析数据不足的问题，在16个公开数据集上平均C-index提升+2.17，IBS提升+0.67。


<details>
  <summary>Details</summary>
Motivation: 临床生存分析中训练数据稀缺且昂贵（随访周期长、队列小、隐私限制），而现有的表格生成模型在小型队列上表现不佳，无法生成足以匹配真实数据性能的合成样本。

Method: FoGS将合成数据构建重构为样本选择问题：从四种架构不同的生成器中抽取候选样本，用七个生存模型组成的集成对每个样本进行评分（基于proper scoring rules），通过双层优化管道——外层优化选择策略（生成器配额、评分器权重、随机补充、事件时间与删失的分层平衡），内层调优下游模型（XGBoost-Cox）。

Result: 在16个公开数据集上（train-on-synthetic, test-on-real），FoGS平均C-index提升+2.17，IBS提升+0.67，9/16数据集两项指标均提升，13/16至少一项提升（单侧Wilcoxon p=0.039和p=0.035），在多数队列上匹配或超越真实数据训练性能，且隐私裕度无显著下降。

Conclusion: 对异构生成器池进行样本筛选是隐私受限临床场景下替代真实数据训练的可行方案。
...
TL;DR: 本文证明GRPO、Dr. GRPO和DAPO这三种训练语言模型推理的方法本质上是同一机制的不同设置，其核心是答案标准差——当答案有分歧时更新幅度大，一致时则无更新。


<details>
  <summary>Details</summary>
Motivation: 揭示看似不同的策略优化方法背后的统一数学原理，澄清标准化步骤并非无关紧要，而是决定学习发生位置和强度的关键控制旋钮。

Method: 通过数学推导证明GRPO、Dr. GRPO和DAPO本质上是同一公式的不同参数设置，核心变量是答案标准差的处理方式（除以、不除、排除零情况），并在Big-Math数据集上验证。

Result: 定义了"组标准差恒等式"：对于正误奖励，标准差精确等于训练更新的大小；有分歧的样本贡献最大，全一致的样本则无贡献。

Conclusion: 标准化步骤不是无害的预处理，而是直接决定哪些问题值得学习以及学习强度的关键因素。
...
TL;DR: 提出EVOTS框架，用进化神经架构搜索自动发现适应多变量时间序列预测的类Transformer架构，在多个数据集和预测设置上取得有竞争力的结果。


<details>
  <summary>Details</summary>
Motivation: 多变量时间序列预测中，固定Transformer架构难以适应不同任务和预测设置的多样性，且进化神经架构设计在该领域尚未被充分探索。

Method: 提出EVOTS框架，使用模块化基因组表示编码架构（注意力、前馈、投影组件灵活组合），并通过修复机制保证进化过程中的结构有效性，从而无需手工设计规则即可探索多样化的架构空间。

Result: 在ETTh1、ETTh2、ETTm1、ETTm2四个基准数据集上，在多种预测设置（单变量到单变量、多变量到单变量、多变量到多变量）和不同预测长度（96/192/336/720）下，多变量到多变量设置中进化得到的架构在均方误差上达到有竞争力甚至优于强Transformer基线。同时报告了训练时间以说明计算成本。

Conclusion: 进化搜索能够在实际运行时约束下有效发现灵活且高性能的类Transformer架构，适用于多变量时间序列预测。
...
TL;DR: 本文提出分数阶傅里叶混合专家适配器，让每个专家学习一个可调的分数阶傅里叶阶数，在空间域（LoRA）和傅里叶域（谱适配器）之间连续插值，通过路由将不同token分配到最适合的域进行低秩更新，在多个基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法要么在空间域（LoRA）要么在固定傅里叶域操作，但不同任务、层和token可能偏好不同的域，固定单一基不是最优的。

Method: 提出Fractional-Fourier Mixture of Experts（分数阶傅里叶混合专家），每个专家携带一个可学习的分数阶傅里叶阶数，在空间域和傅里叶域之间连续插值；不同阶数的算子互不相干，自然实现专家去相关；使用chirp-FFT替代实现O(d log d)复杂度。

Result: 在LLaMA-3.1-8B和Qwen2.5-7B上的常识、数学、代码和知识基准测试中，优于MoE-LoRA、FlyLoRA、FourierMoE、HMoRA等方法，且保持较小的活跃参数预算。

Conclusion: 学习域的选择是PEFT中一个重要的设计自由度，分数阶傅里叶混合专家能以极小代价实现更好的任务适应性和多任务组合能力，且学习的阶数具有可解释性。
...
TL;DR: 本文提出一种基于可验证奖励的强化学习方法，通过无标签的状态条件经验胜率奖励和梯度掩码技术，训练7B模型在NFL比赛实时胜率预测中达到甚至超越博彩市场的校准水平，同时保留链式推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习使用可验证奖励（如Brier分数）训练概率预测模型时，实际会降低校准性能；现有解决方案仅处理认知不确定性（置信度伴随正确/错误答案），而忽略了偶然不确定性（预测本身是输出，标签是随机结果）。

Method: 1. 提出无标签的可验证奖励：基于状态条件的经验胜率（从历史结果估计）；2. 采用直接预测或梯度掩码，使梯度绕过推理过程；3. 以NFL比赛实时胜率预测为测试平台，以博彩市场为基准。

Result: 仅用该奖励训练（无人类标签或SFT），7B模型通过直接预测达到博彩市场的校准水平，且优于零样本前沿模型；该模型与前沿模型、表格估计器达到相同Brier分数，揭示了博彩市场的微弱优势来源于模型输入之外的实时比赛信息。

Conclusion: 梯度掩码而非丢弃链式推理，可以保留推理过程并避免链式推理训练带来的校准退化。
...
TL;DR: 本文提出了一种基于Ising模型的热力学计算装置的可扩展训练算法，实现了用二进制吉布斯采样进行深度卷积网络训练，在CIFAR-10/100上达到94.9%/76.0%准确率，并建立了推理成本与精度的数学关系。


<details>
  <summary>Details</summary>
Motivation: 基于Ising模型的热力学计算设备在低功耗AI推理和边缘计算方面具有巨大潜力，但目前缺乏可扩展的训练方法来实现大规模模型在该硬件上的部署。

Method: 将高温吉布斯采样Ising系统的时间平均行为与前馈神经网络推理之间的理论对应转化为基于反向传播的可扩展训练算法，用于训练深度卷积网络在Ising硬件上进行热力学推理。

Result: 在CIFAR-10上达到94.9%准确率，CIFAR-100上达到76.0%准确率；建立了推理成本与精度之间的数学关系及其控制方法；证明推理成本与性能之间存在可控的权衡边界。

Conclusion: 该工作为高温热力学AI模型提供了可扩展的训练方法，并为相关硬件开发指明了方向。
...
TL;DR: 提出TallyTrain协议，通过仅传输argmax类索引（每探针$\lceil \log_2 C \rceil$比特）来大幅压缩联邦学习通信，在非IID场景下硬标签多数投票优于软标签蒸馏，且与稀疏参数合并结合后Pareto优于FedAvg/FedProx/FedDF。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中模型大小和类别数量导致通信瓶颈，现有软标签蒸馏方法在大词汇量下开销过高，且非IID场景下软标签平均会放大错误。

Method: TallyTrain：仅传输每个客户端的argmax类别索引（硬标签），通过多数投票聚合；并组合硬标签共识与稀疏参数合并得到带宽桥接变体。

Result: 在标准基准上，TallyTrain匹配或超越软标签蒸馏，通信量降低三个数量级；带宽桥接变体Pareto优于FedAvg、FedProx、FedDF。

Conclusion: 硬标签多数投票在非IID联邦学习中优于软标签蒸馏，且可大幅降低通信开销。
...
TL;DR: 本文提出了一种基于潜在地图的性能框架（Latent Maps of Performance），通过对抗性路径生成，在星际争霸II游戏中为玩家提供可操作的改进反馈，类似体育科学中的冠军模型。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习研究主要集中在击败人类玩家，而非为人类提供改进反馈；RTS游戏缺乏将专家知识转化为可操作反馈的原则性框架。

Method: 使用Guided Variational Autoencoder在23,305场职业比赛回放上训练，设计了四种遍历策略（线性插值、迭代最优传输、密度正则化梯度上升、神经流匹配），在业余玩家数据上生成从输到赢的多步改进轨迹。

Result: 四种遍历策略均能生成基于专家行为的改进轨迹，但存在路径发现方法之间的权衡。

Conclusion: 存在路径发现方法之间的权衡，未来研究应聚焦于为人类改进开发模型解决方案。
...
TL;DR: 论文提出TRIE框架用于评估随机PDE的替代模型，发现生成模型在捕捉不变测度和不确定性量化方面表现最佳，且潜在生成模型可大幅提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 科学系统中的随机性使得确定性神经替代模型无法可靠捕捉统计特性和预测不确定性，现有评估缺乏针对随机PDE替代模型的系统性框架。

Method: 提出TRIE评估框架，从三个维度（不变测度复现、预测不确定性可信度、高效概率生成）评估替代模型；在两种随机SPDE上对11组参数进行实验，对比点训练神经网络、蒙特卡洛dropout、异方差高斯似然及生成模型。

Result: 标准点训练模型短期展开看似合理但长期统计结构失败；近似不确定性方法常校准不良且过于自信；生成模型在所有指标上表现最一致；潜在生成模型在保持统计保真度同时将推理时间降低约12倍。

Conclusion: 生成模型是随机PDE替代建模的最优选择，潜在生成模型可在统计保真度和效率之间取得良好平衡。
...
TL;DR: StateFlow是一个基于VARNN扩展的递归预测框架，通过双状态循环编码器和分块解码器实现长时序多步预测，在标准基准上达到竞争性能。


<details>
  <summary>Details</summary>
Motivation: 长时序多步预测面临非平稳性、机制转换和误差累积等挑战，原始VARNN仅支持单步回归，无法直接用于多步预测。

Method: 将VARNN扩展为双状态循环骨干（隐藏状态轨迹+残差记忆轨迹），采用分块解码器分别总结并映射到未来预测区间，使用两阶段优化策略。

Result: 在标准LTSF基准上，StateFlow在保持线性循环编码和紧凑模型设计的同时，达到了与强线性、循环、卷积和Transformer基线竞争的性能。

Conclusion: 基于VARNN的双状态递归框架能够有效捕捉时序动态和局部预测偏差，实现高效的长时序多步预测。
...
TL;DR: 提出Device Passport通道嵌入技术，通过混合专家模型融合功能活动与元数据，提升生物信号基础模型在不同设备布局间的迁移能力。


<details>
  <summary>Details</summary>
Motivation: 新设备布局缺乏大规模数据集，生物信号基础模型需要有效泛化到不同布局，现有通道嵌入方法仅使用功能信息或元数据单一来源，效果有限。

Method: 提出Device Passport通道嵌入技术，以每个通道的功能活动和元数据为输入，通过专家混合模型学习通道嵌入，而非仅使用单一信息源。

Result: 在控制子集迁移实验和真实的耳部EEG迁移任务中，Device Passport整体表现具有竞争力，在布局迁移场景下优于最强的学习基线方法。
...
TL;DR: 提出了一种用于群体分布鲁棒最小二乘问题的算法，通过块Lewis权重和加速近端方法，以较少的线性系统求解次数获得(1+ε)乘性最优解，并在中等精度区域优于已知内点方法。


<details>
  <summary>Details</summary>
Motivation: 解决群体分布鲁棒（GDR）最小二乘问题的高效算法需求，特别是在中等精度场景下内点方法效率不足的问题。

Method: 采用块Lewis权重（block Lewis weights）的几何构造，将经验GDR问题转化为精心设计的最小二乘问题，并结合加速近端方法（accelerated proximal methods）进行求解。

Result: 算法使用Õ(min{rank(A),m}^{1/3} ε^{-2/3})次线性系统求解即可获得(1+ε)乘性最优解，在中等精度区域优于内点方法，且与ℓ∞回归的最优保证相匹配。同时还给出了在平均最小二乘损失与分布鲁棒损失之间平滑插值的算法。

Conclusion: 该算法在中等精度场景下对已知内点方法有改进，匹配了ℓ∞回归的最优保证，并提供了损失函数之间的平滑插值能力。
...
TL;DR: 本文提出弱形式核岭回归（WKRR）方法，用于从含噪声测量数据中学习复杂动态系统，通过弱形式滤波机制提升噪声鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 从含噪声测量数据中准确预测复杂动态系统是科学计算中的重大挑战，现有核岭回归方法在干净数据上有效，但对噪声数据效果有限。

Method: 结合弱形式（weak formulation）的滤波机制与核回归策略，提出Weak-form Kernel Ridge Regression (WKRR)，并给出偏差-方差误差分解分析。

Result: 在最高64维的混沌基准系统和15,000维的真实流体数据上，WKRR优于多种基线方法，且对干净和含噪声数据均有效。

Conclusion: WKRR框架简单易实现，通过弱形式的滤波机制显著提升了动态系统学习的噪声鲁棒性。
...
TL;DR: 提出了因果抽象误差（CAE）作为衡量高层次解释有效性的通用指标，通过十个复杂系统的基准测试验证，只有考虑未映射变量忠实性检验的因果类指标能可靠区分有效与无效的抽象。


<details>
  <summary>Details</summary>
Motivation: 科学的核心目标是产生对复杂系统的有效解释，但目前缺乏统一的方法来衡量高层次因果解释是否有效。

Method: 构建包含十个复杂系统（离散/连续状态空间、静态/动态系统）的基准，每个系统配有共识性的真实因果解释与无效对照条件；在统一的因果抽象框架下，系统评估了30多个来自观测、函数、信息论和因果家族的候选指标。

Result: 只有因果类指标能可靠区分有效与无效抽象，且必须包含对未映射变量的忠实性检验；据此提出CAE指标，在全部系统的判别测试中通过，仅需30次采样干预即可收敛。

Conclusion: CAE可作为发现和验证高层次解释的通用度量标准。
...
TL;DR: 本文提出了一种基于熵正则化的概率门控联邦学习方法，通过维持稀疏优化中的不确定性来提升数据异构和部分客户参与场景下的泛化性能与稀疏恢复精度。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中，数据异构和部分客户参与导致稀疏模型训练困难，尤其是在小样本高维场景下，传统基于幅度的剪枝方法无法有效探索参数空间中的不确定性，容易过早确定稀疏结构，影响泛化性能。

Method: 在具有概率门控和L0约束的稀疏化优化框架中，引入熵正则化机制来维持门控分布的熵，防止早期对稀疏支撑集的承诺，从而在联邦优化过程中保持对稀疏结构的不确定性探索。

Result: 在合成数据和真实基准上的实验表明，该方法在测试数据上的统计性能和稀疏恢复准确性上，一致优于Fed-IHT和先稠密FedAvg再剪枝的方法。

Conclusion: 熵正则化能有效提升联邦学习在数据异构、客户参与异构和高稀疏度条件下的稀疏模型泛化能力与结构恢复精度。
...
TL;DR: 本文提出一个四阶段诊断框架，评估LLM在陌生物理框架下的推理能力，发现模型在定性-定量不对称、自检能力弱等问题，三个世界综合PASS率低。


<details>
  <summary>Details</summary>
Motivation: 现有LLM物理基准仅用答案准确性评分，无法区分真正推理与模式记忆，也难以定位推理失败的具体环节。

Method: 设计包含归纳、公式化、预测、审查四个阶段的诊断流程，结合预注册、会话隔离、双LLM评判和人工审计，应用于三个平行物理世界（反事实F=mv、亚里士多德力学、衰变世界）。

Result: 三个世界综合PASS率分别为6/15、6/15、0/15；存在定性-定量不对称（方向判断准但比值计算错）；LLM评判可靠性跨框架不迁移；自我审查环节极弱，2/3以上案例模型未识别自身错误。

Conclusion: 当前LLM在陌生物理框架下的推理能力有限，尤其在定量计算和自我纠错方面存在系统性缺陷，需要更好的评估方法。
...
TL;DR: 本文介绍了Airbnb如何结合经济建模与因果推断技术，分析客人基于价格等因素的预订行为及其异质性，以优化定价工具、提升个性化体验，更好地连接房东与客人。


<details>
  <summary>Details</summary>
Motivation: Airbnb希望通过理解客人对价格的敏感程度及偏好差异，优化定价工具和个性化推荐，从而更好地连接房东和客人。

Method: 结合经济建模（Economic Modeling）与因果推断（Causal Inference）技术来分析客人预订行为及价格敏感度的异质性。

Result: 能够识别不同客人对不同价格的响应程度，从而优化定价工具、平衡供需，并提升个性化匹配效果。

Conclusion: 通过理解客人价格敏感度的异质性，Airbnb可以更好地支持市场生态，帮助房东设定有竞争力的价格，同时提升客人的个性化体验。
...
TL;DR: 本文提出了一套标准化的EPC协议（RFC风格），用于测量和比较LLM智能体中评估者偏好耦合现象，并发布了带版本号的参考快照和开源基础设施。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏标准化协议来复现评估者偏好耦合测量、跨评估者和时间点比较结果，以及检测专有评估者更新导致的测量衰减。

Method: 设计了四阶段隔离范式的EPC协议规范，包括执行器和评估器配置、策略与任务设计、TTRL更新规则、多种度量指标（gamma, JSD, ECE, Brier）和输出schema，并发布了带有版本标识的参考快照。

Result: 提供了覆盖8种评估器条件、122次重复实验的参考快照v1.0，涉及GPT-4o、Qwen、DeepSeek等模型，并明确了所有测量值的时间依赖性。

Conclusion: 该协议为第三方研究者提供了可复现、可比较、可检测衰减的标准化工具，推动评估者偏好耦合研究的透明性和可验证性。
...
TL;DR: 提出量子流匹配(QFM)模型，利用自旋Wigner函数和流匹配学习多量子比特量子分布，能准确捕获量子态的物理性质。


<details>
  <summary>Details</summary>
Motivation: 现有深度生成模型（基于扩散和流匹配）在经典分布上表现优异，但学习量子分布仍具挑战，难以准确建模量子态的有意义物理属性。

Method: 将密度矩阵转化为自旋Wigner函数，利用函数空间中的流匹配（functional flow matching）学习量子分布。

Result: 通过评估生成量子态的迹、纯度、纠缠熵等物理量，验证了QFM能准确捕获给定量子分布的底层物理特性。

Conclusion: QFM能够有效学习多量子比特量子分布，并在物理量评估上表现良好。
...
TL;DR: 本文通过扩展实验条件，验证了LLM评估系统中存在偏差-可靠性权衡，即评估器耦合度、策略多样性与小样本测量可靠性三者无法同时优化。


<details>
  <summary>Details</summary>
Motivation: 先前关于偏差-可靠性权衡的证据仅基于单一研究的5个条件，缺乏广泛的实证支持，作者希望扩大实证基础来验证这一权衡是否普遍存在。

Method: 将实验条件从5个扩展到11个，测量了所有11个条件的gamma（评估器耦合度）和H（策略多样性），对其中7个有足够种子数的条件计算CV(N=5)，最终获得5个完整三元组数据，并通过相关性分析验证权衡关系。

Result: 数据证实了权衡关系：低耦合度（gamma<0.2）对应高测量噪声（CV>1.0），高耦合度（gamma>0.9）对应低噪声（CV<0.16）；gamma与H呈强负相关（r=-0.989）；GPT-4o出现版本漂移导致gamma=0.000、H=1.000；没有任何条件能同时实现低耦合和低噪声。

Conclusion: 偏差-可靠性权衡在LLM评估系统中是普遍存在的约束条件，评估器耦合度会抑制策略多样性，且当前不存在能同时实现低耦合与低测量噪声的理想评估条件。
...
TL;DR: 本文提出使用输出水印技术来更有效地检测生成模型的训练数据成员关系，并与传统基于损失的方法进行了比较。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，在现代语言模型设置中，训练数据成员关系推断本质上是困难任务。作者希望利用水印技术（特别是水印"放射性"残留效应）来使成员关系测试更加可行。

Method: 采用基于水印的数据集推断方法，与传统基于损失（loss-based）的成员关系推断方法进行对比实验。利用部分水印训练数据集下模型残留的水印"放射性"效应。

Result: 当子集暴露程度足够高时，水印方法在成员关系检测性能上可以达到与基于损失的方法相当的水平。

Conclusion: 输出水印技术可以作为成员关系推断的有效替代方案，在特定假设条件下表现良好。
...
TL;DR: 本文研究了递归特征机器在数据损坏场景下的性能局限性，并提出一种标签变换方法使其性能赶上甚至超越前馈神经网络。


<details>
  <summary>Details</summary>
Motivation: 尽管RFM在特征学习方面与前馈神经网络具有相当的能力，但在某些数据损坏场景下性能显著低于神经网络，作者希望探究这一局限并找到解决方案。

Method: 提出了一种对训练标签进行变换的方法，该方法能够促进在噪声、复杂表示和类别不平衡数据上的学习。

Result: 这种简单的标签变换使RFM缩小了与前馈神经网络的性能差距，在某些情况下甚至超越了神经网络。

Conclusion: 通过标签变换，RFM可以在数据损坏场景下达到甚至超过前馈神经网络的性能水平。
...
TL;DR: 提出PRISM框架，通过可微通道加权和半监督域适应，实现标签高效的跨主体情绪解码。


<details>
  <summary>Details</summary>
Motivation: 脑电图(EEG)情绪解码面临通道冗余和跨主体差异大两大障碍，限制了模型的可扩展泛化能力。

Method: PRISM框架包含两部分：1) 通道侧：通过轻量级专家集成网络学习可微的、数据依赖的通道权重，增强可靠电极、抑制干扰；2) 域适应侧：利用无标签数据生成置信度过滤的伪标签，驱动一致性正则化和域对齐，缓解主体间异质性。

Result: 在DEAP、DREAMER和SEED三个数据集上，PRISM超越现有最先进方法，实现了鲁棒的跨主体泛化，且仅需少量标注数据。

Conclusion: PRISM有效解决了EEG情绪解码中的通道冗余和跨主体差异问题，在标签有限条件下实现了优异的跨主体泛化性能。
...
TL;DR: 提出结构感知最优传输（SAOT）框架，通过最优传输理论捕获全局节点间对应关系，结合跨任务知识蒸馏，解决自监督持续图学习中全局关系结构失真问题，在多个基准上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督持续图学习方法依赖实例级一致性目标，孤立优化节点，无法维持全局关系结构，导致节点间对应关系在持续学习中逐渐失真。

Method: 提出SAOT框架：1）利用最优传输理论捕获全局节点间对应关系，增强图表示学习；2）引入跨任务知识蒸馏机制，保留先前任务的结构知识。

Result: 在四个CGL基准数据集上超越现有自监督基线，在Class-IL设置下，CoraFull-CL上平均准确率提升5%，Products-CL上提升超过15%。

Conclusion: SAOT通过显式捕获和保持跨任务的全局关系结构，有效解决了自监督持续图学习中的结构失真问题，验证了最优传输在该任务中的有效性。
...
TL;DR: GenDa是一个统一的无监督强化学习框架，通过技能重标记机制和互补信息瓶颈（CIB）来解决现有方法中技能语义非平稳和泛化脆弱的问题，显著提升了预训练的数据效率、可扩展性和下游任务的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前无监督强化学习方法存在两个关键瓶颈：技能语义的非平稳性问题以及脆弱的泛化能力，限制了方法的可扩展性和数据效率。

Method: 提出GenDa框架，包含两个核心组件：1) 技能重标记机制，缓解非平稳性并提升数据效率；2) 互补信息瓶颈（CIB），促使策略关注自我中心特征，增强对分布偏移的鲁棒性。

Result: 实验表明GenDa显著增强了无监督强化学习的可扩展性，具备更优的泛化能力和数据效率。

Conclusion: GenDa有效解决了当前无监督强化学习的两个关键瓶颈，为下游任务提供了更鲁棒、更高效的预训练基础。
...
TL;DR: TimeSynth是一个用于评估健康信号数字孪生预测模型的基准框架，揭示了标准逐点指标无法检测到的相位、频率和状态转换保真度损失，发现模型架构是保真度的主要决定因素。


<details>
  <summary>Details</summary>
Motivation: 现有健康信号预测模型的评估指标（逐点误差）无法检测到相位、频率和状态转换等关键动态特性的丢失，导致模型排名失真（相位误差可达53°），需要一种能反映生理信号本质特性的评估方法。

Method: 提出TimeSynth框架，包含两个组件：(1) 基于拟合真实EEG/ECG/PPG信号的参数模型生成具有已知解析动态的信号发生器；(2) 量化幅度、频率、相位和状态转换保真度的诊断工具。在11种架构上进行评估。

Result: 线性注意力和全序列注意力模型在幅度误差可接受的情况下系统性地丢失频率和相位信息；具有局部时间结构的架构更好地保持动态保真度；但没有任何架构能可靠地保留随机切换。架构是保真度的主导决定因素。

Conclusion: TimeSynth提供了模型在接触患者数据前所需的受控临床前压力测试，使模型选择成为基于用例的原则性决策而非寻找单一赢家。
...
TL;DR: 本文提出了一种通过谱有效秩熵（spectral effective-rank entropy）来评估和调控演员-评论家强化学习方法中评论家网络复杂度的新方法，并在TD3和PPO实验中验证了其可测量性、与训练行为的关联性，以及通过谱熵惩罚实现复杂度控制的可能性。


<details>
  <summary>Details</summary>
Motivation: 演员-评论家方法依赖学习得到的评论家，但评论家质量通常仅通过回报、时序差分误差或价值损失间接评估，缺乏直接诊断评论家复杂度的工具。

Method: 使用谱有效秩熵（基于评论家权重矩阵奇异值分布的秩-like统计量）评估评论家模型复杂度；并在评论家损失中加入谱熵惩罚项以实现复杂度直接控制。

Result: 评论家复杂度在整个训练过程中可测量，且与训练行为系统相关；但该关系在算法、任务和超参数间呈现异质性。谱熵惩罚能可靠地改变目标谱量，证明复杂度可被控制。

Conclusion: 评论家复杂度可作为演员-评论家强化学习的一个额外诊断和干预维度，但其对回报的影响需根据具体任务证据判断，而非通用性能声明。
...
TL;DR: MolSafeEval是一个专门评估AI生成分子安全风险的基准，通过整合毒理学数据库和危险规则构建分子安全知识图谱，结合LLM推理识别不安全特征，覆盖四类分子生成任务。


<details>
  <summary>Details</summary>
Motivation: 现有分子生成基准忽视AI生成分子的安全风险（毒性、反应性等），缺乏系统评估工具。

Method: 构建分子安全知识图谱（整合毒理学数据库与危险规则），基于LLM推理检测不安全特征；划分四类生成任务（无条件生成、性质优化、靶点设计、文本生成）并标准化评估协议。

Result: 系统揭示了当前生成模型的安全脆弱性，提供了新的安全评估视角。
...
TL;DR: 本文研究了在KKBox数据集中，早期行为数据对订阅流失预测所需的足够天数，发现45-90天区间存在收益递减的拐点，但该曲线会随实验设计（如队列构建、目标定义、特征集）而变化。


<details>
  <summary>Details</summary>
Motivation: 探究在订阅流失预测任务中，需要多长时间的早期行为数据才能达到足够的预测效果。

Method: 使用KKBox公开数据集，对手动续费的高流失细分群体进行分析，采用九窗口充分性曲线进行实验，并对不同队列/任务设计进行压力测试。

Result: 120天时手动续费群体的PR提升+0.10；45-90天区间存在收益递减拐点；但该曲线会因队列构建、目标定义和特征集的不同而改变，在移动目标场景下曲线甚至会反转。

Conclusion: 任何窗口充分性声称都应明确说明其队列构建、目标定义和特征家族；该结果基于单一音乐流媒体数据集，机制可推广但具体数值可能不适用。
...
TL;DR: 本文在IEEE-CIS欺诈数据集上比较了7种分类编码方法，发现实体嵌入（AUC-ROC=0.9612）与CatBoost（0.9602）表现最佳且统计无显著差异，但各方法在不同评估指标上各有优劣。


<details>
  <summary>Details</summary>
Motivation: 探讨不同分类编码方法在高基数分类特征欺诈检测任务中的性能差异，尤其关注实体嵌入方法能否超越传统编码方案。

Method: 在IEEE-CIS欺诈基准数据集上测试7种分类编码方法，使用分层5折交叉验证（3次重复），固定LightGBM作为下游分类器进行控制变量比较，同时引入CatBoost和TabNet作为跨范式对比。

Result: 实体嵌入AUC-ROC最高（0.9612），与CatBoost（0.9602）统计持平，显著优于层级分组编码（0.9548）；AUC-PR上CatBoost领先（0.822 vs 0.793）；TabNet表现不佳。

Conclusion: 实体嵌入的优越性源于联合多列表示学习，但不同编码方法在不同指标上各有优势，需根据具体任务需求选择。
...
TL;DR: 本文从理论上研究了线性Transformer在上下文学习中的近似和泛化能力，提出了一种新的视角来设计线性化预训练softmax大语言模型的激活函数和损失函数。


<details>
  <summary>Details</summary>
Motivation: Softmax Transformer在处理长上下文时计算和内存复杂度呈二次增长，而线性Transformer虽然降低了复杂度，但其特征映射的理论理解仍不清楚。

Method: 采用两阶段采样的域泛化框架，分析了线性Transformer的近似和泛化能力，证明了其上下文学习等价于从上下文分布到响应函数的映射学习，并得到了维度无关的收敛率。

Result: 获得了维度无关的收敛率，揭示了数据分布规律性与潜在特征之间的权衡；提出了线性化预训练softmax LLM的激活函数和损失设计新视角。

Conclusion: 线性Transformer在理论上具备有效的上下文学习能力，且设计良好的激活和损失函数可以将其应用于线性化预训练的大语言模型。
...
TL;DR: 提出PAPA方法，无需参数化奖励模型，通过实时用户反馈直接优化扩散模型，实现个性化偏好对齐，并进一步提出EPAPA加速微调。


<details>
  <summary>Details</summary>
Motivation: 在个性化推荐等场景中，扩散模型需要根据用户偏好进行微调，但学习参数化奖励模型需要大量偏好数据，实际中难以获取。

Method: 提出PAPA方法，绕过参数化奖励模型，直接利用实时用户反馈优化扩散模型；基于变分推断框架实现反馈高效的偏好对齐；进一步提出EPAPA加速微调策略。

Result: 在多种类别条件和细粒度对齐任务上，通过大量实验和消融研究验证了PAPA的有效性，EPAPA在更少计算预算下加速了微调过程。

Conclusion: PAPA无需大规模偏好数据即可实现个性化偏好对齐，EPAPA进一步提升了实际部署的适用性。
...
TL;DR: 本文提出PRISM架构，通过稀疏非负原型混合进行语言建模，实现快速、可追溯的训练数据归因，且在130M-1.6B参数规模上性能接近或超越传统密集模型。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型的训练数据影响分布在整个参数空间中，难以追溯和审计，缺乏可解释的组件化结构。

Method: 提出PRISM原型语言模型架构，通过稀疏非负混合原型进行预测，结合聚类目标将每个原型锚定到连贯的训练样本邻域。

Result: 在130M至1.6B参数、最高50B token训练下，原型模型平均下游准确率超越或仅比密集基线低2.5个百分点；训练数据归因速度比事后方法快约500倍；通过校准线性原型控制器可提升约3个百分点的准确率。

Conclusion: 原型结构使损失景观的曲率局部化，得到更易处理的Hessian矩阵，支持高效的训练数据归因和模型行为修正。
...
TL;DR: 本文提出了一套结合结构方程模型（SEM）、普通最小二乘法（OLS）和双机器学习（DML）的分阶段稳健性分析框架，用于调查问卷研究中潜在构念的因果推断稳健性检验。


<details>
  <summary>Details</summary>
Motivation: SEM路径显著性依赖于特定模型设定，可能无法反映在不同估计框架下的结果稳定性，需要更系统的稳健性检查方法。

Method: 先使用SEM建立基准模型，再用OLS对SEM导出的构念分数进行回归作为透明基准，最后用DML进行残差化处理，并对比随机森林、梯度提升、支持向量机等学习者，同时进行反向方向诊断。

Result: 以金融科技数字客户亲密度调查模型为例，识别出哪些关系在SEM、OLS和DML检查中保持稳定，哪些需要更谨慎解释。

Conclusion: 提供了一个可复现的稳健性工作流和解释指南，方便其他问卷研究者借鉴使用。
...
TL;DR: 本文提出Active-GRPO方法，通过主动模仿-强化与主动参考更新机制，在分子优化中提升大语言模型科学推理的鲁棒性和效率，在TOMG-Bench MOLOPT上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，仅答案监督的SFT会崩塌多步推理，RLVR存在稀疏反馈问题，而参考引导策略优化受限于参考质量。

Method: 提出主动推理范式Active-GRPO，包含两个机制：主动模仿-强化（参考优于策略时模仿，策略超越参考时强化）和主动参考更新（用策略生成的最佳分子替换参考，持续提升模仿目标）。

Result: 在TOMG-Bench MOLOPT上，Active-GRPO将平均SRxSim从GRPO的0.0959和RePO的0.1665提升至0.1773，在LogP、MR、QED上有统计显著提升。

Conclusion: Active-GRPO通过动态平衡模仿与强化学习，并持续升级参考，有效克服了参考质量天花板，提升了分子优化任务中的科学推理性能。
...
TL;DR: 提出Flow-Map GRPO框架，通过锚定随机流图组合（ASFMC）实现确定性少步流图生成器的RL后训练对齐，提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有确定性少步流图生成器难以用RL进行后训练优化，因为RL需要随机轨迹和似然比，而现有随机化方法是为基于速度的采样器设计的，不适用于长程流图。

Method: 提出ASFMC（锚定随机流图组合）机制，通过基于锚点的条件重采样引入随机性，同时保持原始确定性流图的边际概率路径不变；并推导了单时间和双时间流图参数化的GRPO目标。

Result: 在基于FLUX的少步文本到图像生成器（MeanFlow和sCM）上，Flow-Map GRPO在奖励指标、感知指标和任务级评估指标上均优于预训练的确定性流图模型。

Conclusion: 确定性少步流图生成器可以通过Flow-Map GRPO有效进行RL后训练对齐，无需修改原始模型参数化或重新训练为原生随机模型。
...
TL;DR: 提出等变庞加莱残差网络，将双曲几何与离散对称群结合，解决优化困难、边界约束和参数冗余问题。


<details>
  <summary>Details</summary>
Motivation: 标准双曲网络将同一物体的空间变换视为不同层级概念，导致参数冗余和信号消失；且黎曼梯度计算昂贵、流形边界约束严格。

Method: 结合双曲几何与C4/D4离散对称群，提出几何安全的张量重塑、左正则置换双曲群卷积、联合方向庞加莱中点批归一化。

Result: 嵌入等变性大幅减少优化空间、加速收敛，同时满足庞加莱球边界约束并保持空间群等变性。

Conclusion: 等变双曲网络能有效提升优化效率并解决双曲网络中的参数冗余与信号消失问题。
...
TL;DR: 提出端到端决策聚焦学习框架，通过可微凸规划层和平滑top-k算子实现稀疏切线投资组合优化，在四大股票市场取得优于传统方法的夏普比率。


<details>
  <summary>Details</summary>
Motivation: 解决稀疏切线投资组合优化中基数约束NP-hard问题，以及传统预测-优化流程中预测精度与投资组合质量不匹配的问题。

Method: 将夏普比率最大化重构为DPP兼容的凸规划层，用平滑top-k算子替代离散选择实现精确基数k约束，使梯度可流经预测、资产选择和再优化全过程。

Result: 在四大股票市场上，该方法相比历史基线和预测聚焦基线取得了有竞争力且通常更优的样本外夏普比率，在更大资产池中优势尤为显著。

Conclusion: 端到端决策聚焦学习能有效提升稀疏投资组合的样本外表现，代码已开源。
...
TL;DR: 本文提出一种无需对齐和梯度下降的奇异结构测量方法，能从冻结的网络检查点中直接恢复每个“死亡方向”的阶数k，并区分真实奇异性和平直规范对称性，适用于Transformer、卷积和归一化层。


<details>
  <summary>Details</summary>
Motivation: 现有奇异结构测量方法依赖于规范对齐和梯度下降前提条件，难以在训练好的网络上直接应用。本文旨在消除这些限制，提供一种确定性的、架构通用的奇异结构读取方法。

Method: 基于方向Fisher率（directional-Fisher rate）恢复每个死亡方向的阶数k，并提出主不变量的概念；通过方向Fisher大小区分阶数无法判定的情况；设计可插拔检测器为Transformer、卷积和归一化层提供方向。

Result: 该方法在构造单元和训练网络上均能恢复架构预测的阶数，包括微调ViT（LayerNorm核规范）和从头训练的ViT（MLP节点死亡）。全局系数(λ,m)可通过类型化交集的逐方向阶数组装得到，与封闭形式匹配。

Conclusion: 该方法消除了经典对齐和梯度下降前提条件，将阶数恢复变为确定性、架构通用的过程。同时映射到Watanabe三元组：阶数决定通用奇异涨落ν(k)，但训练网络的实现ν低于理论值，而重数可从单点位假设下的主导结构恢复。
...
TL;DR: 提出损失平滑（loss smoothing）方法，通过在目标训练初期插值源和目标训练目标，实现更平滑的过渡，从而在分布偏移下的模型适应中保持有用特征，提升性能。


<details>
  <summary>Details</summary>
Motivation: 标准适应方法（如微调、强化学习）直接优化目标目标，导致从源训练目标突然转变，可能扭曲仍有用的学习表示。作者探究更渐进过渡是否能改善适应效果。

Method: 提出损失平滑方法，在适应初期对源训练目标和目标训练目标进行插值，实现平滑过渡。

Result: 在受控监督偏移、预训练视觉适应、离线到在线及在线强化学习、语言模型微调等场景中，损失平滑一致性地提升了性能。
...
TL;DR: 提出标签影响传播（LIP）模型，通过分解GNN消息传递过程并量化标签间影响相关性，动态调整学习过程，在多层标签节点分类任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有GNN方法未能有效处理非欧几里得图数据中标签之间的复杂影响关系。

Method: 将GNN消息传递分解为传播和变换两个操作，量化标签间影响相关性，构建标签影响图，通过传播高阶影响动态调整学习过程（放大正贡献、抑制负影响）。

Result: 在多个基准数据集上一致超越SOTA方法，证明了模型在多标签节点分类任务上的有效性。

Conclusion: LIP模型通过显式建模和传播标签间影响，有效提升了多标签节点分类性能。
...
TL;DR: 研究分布式在线子模最大化问题，提出统一算法框架，在完全信息和强盗反馈模型下均实现亚线性(1-1/e)-遗憾保证，并设计有界随机管道取整方案解决采样违规问题。


<details>
  <summary>Details</summary>
Motivation: 现有分布式在线子模最大化研究尚不充分，且连续松弛取整方案存在采样违规问题，需要统一的算法框架和理论保证。

Method: 提出统一算法框架兼容完全信息和强盗反馈；设计有界随机管道取整方案处理采样违规。

Result: 两种反馈模型下均实现亚线性(1-1/e)-遗憾界；采样违规概率渐近消失，累计违规次数呈亚线性。

Conclusion: 所提算法达到与集中式算法相当的性能，且采样违规最优性得到理论证明。
...
TL;DR: 本文提出Text Prompt Boosting (TPB)，一种基于AdaBoost的框架，通过顺序聚合多个文本提示分类器来提升视觉语言模型分类准确率，并支持跨模型迁移。


<details>
  <summary>Details</summary>
Motivation: 现有的小样本文本提示方法在构造提示时没有明确关注被误分类的样本，导致即便增加样本量，性能提升也有限。为了充分利用小样本监督，需要一种能显式处理困难/误分类样本的方法。

Method: 提出TPB框架，受AdaBoost启发，将每个基于文本提示的分类器视为弱学习器，通过顺序聚合这些弱学习器，显式地针对那些被误分类的困难样本进行增强，最终形成强集成分类器。

Result: 在11个分类基准上，TPB在源模型上提升了准确率，并且在迁移到更大、能力更强的VLM时仍能保持基于样本量的性能提升，而现有方法难以维持这种改进。

Conclusion: TPB能够有效保留文本空间中与任务相关、与模型无关的线索，实现鲁棒的跨模型迁移，优于现有小样本文本提示方法。
...
TL;DR: 提出SPARROW算法，通过解耦生成先验与奖励信号，在低评估预算下实现黑盒优化，支持有噪、易失败和复杂几何形状的目标函数。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型方法需要大量评估来对齐采样器与奖励分布，在低预算场景下不实用。需要一种能在评估次数极少的情况下处理有噪、易失败和复杂搜索空间的黑盒优化方法。

Method: SPARROW将生成先验与奖励信号完全解耦，使用已知损坏过程的固定结构化提议算子（基于未评估数据训练），通过基于排名的引导在已评估候选存档上进行优化。

Result: 在理论上提供了采样器支持的渐近收敛保证；实验上在不可靠奖励和几何复杂景观问题上表现出强性能。

Conclusion: SPARROW能有效应对复杂几何形状、不可靠奖励信号，在极低评估预算下实现有效优化。
...
TL;DR: 提出一种结合主动学习和最小-最大学习策略的框架，通过掩码时间序列重建反馈，提升无监督异常检测模型在复杂时间序列数据中识别细微和噪声异常的能力，在28个测试案例中AUC提升12.39%。


<details>
  <summary>Details</summary>
Motivation: 现有无监督时间序列异常检测方法难以区分近正常异常与正常模式，且容易受到正常样本中噪声的污染，导致检测性能不佳。

Method: 提出一个结合主动学习的框架，包含两个核心策略：(1) 掩码时间序列重建反馈策略，强制模型学习鲁棒的时序依赖关系；(2) 最小-最大学习策略，通过差异化处理正常和异常样本来提升鲁棒性。

Result: 在28个测试案例、4个多变量时间序列数据集和7个无监督骨干模型上进行评估，AUC相比原始模型平均提升12.39%。

Conclusion: 该方法可以轻松集成到现有的基于重建的无监督异常检测系统中，显著提升其性能。
...
TL;DR: 提出AgentODE框架，利用LLM生成候选ODE结构，结合工具增强的推断代理通过诊断-更新循环，仅基于群体级汇总统计量来发现ODE结构并优化参数分布，适用于数据稀疏、隐私受限的罕见病场景。


<details>
  <summary>Details</summary>
Motivation: 罕见病中，个体级数据稀缺、噪声大、异质性强且受隐私限制，而现有的基于ODE的机制建模方法无法仅从汇总统计量中同时发现模型结构和优化参数分布。

Method: AgentODE：LLM提出候选ODE结构，工具增强的推断代理通过诊断-更新循环迭代优化参数分布，全程仅使用群体级汇总统计量。

Result: 在三个基准问题和两个临床数据集（包括仅46名患者231个观测的罕见病RDEB）上，AgentODE均能恢复功能一致的ODE结构；在稀疏噪声数据下，基于汇总统计量的推理比基于个体数据的基线能发现更符合机理的结构。

Conclusion: AgentODE开辟了从群体级汇总统计量直接对罕见病进行机制建模的新途径，克服了数据稀缺和隐私约束带来的传统限制。
...
TL;DR: MosaicKV是一种针对超长上下文LLM服务的动态二维KV缓存压缩系统，通过细粒度的分段压缩策略和高效的缓存管理，在保持高精度的同时大幅降低内存消耗并提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 长上下文LLM服务中KV缓存随序列长度线性增长，导致GPU内存耗尽、批处理规模受限和服务吞吐量下降。现有压缩方法仅针对序列维度或通道维度单一方向，压缩空间有限；而简单地将两维度压缩直接叠加会导致显著精度损失。

Method: MosaicKV提出动态二维压缩方法：1) 识别KV缓存中每个向量的重要元素，在KV缓存段粒度上选择压缩策略；2) 引入压缩KV缓存管理机制，利用未充分利用的GPU和CPU资源维护压缩缓存并加速注意力计算。

Result: 在H800 GPU上，MosaicKV相比未压缩基线实现了最高16倍注意力加速、4.8倍更低的解码延迟和7.3倍更高的吞吐量，同时减少3倍内存使用，在LongBench和RULER上仅产生平均1.76%的精度损失。

Conclusion: MosaicKV通过动态二维压缩和高效缓存管理，有效解决了长上下文LLM服务中KV缓存的内存和性能瓶颈，实现了压缩率与精度的良好平衡。
...
TL;DR: 本文提出了一种并行化τ-leaping算法，用于加速吸收离散扩散模型的采样过程，通过Picard迭代法实现了时间上的并行加速，并在理论和实验上证明了有效性。


<details>
  <summary>Details</summary>
Motivation: 离散扩散模型的生成过程本质上是顺序的，采样速度较慢，亟需加速方法。

Method: 利用连续时间随机积分形式和Picard迭代法，对τ-leaping算法进行并行化处理，实现了时间维度的并行采样加速。

Result: 时间复杂度从O(d log S)降至O(log(d log S)·log d)；合成数据上实现7-9倍加速，图像/文本任务上以更少NFE实现1.45-1.86倍加速。

Conclusion: 该方法扩展了离散扩散模型在分子结构和语言生成等应用中的高效并行推理潜力。
...
TL;DR: 本文研究ERP-BCI系统中拼写率与13个评估指标的相关性，发现Brier分数、MCC、ROC AUC、PR AUC、AP和pAUC等指标最能反映用户拼写表现，建议研究者报告这些指标。


<details>
  <summary>Details</summary>
Motivation: 现有BCI研究常报告损失和准确率，但这些指标对ERP应用不够充分——拼写率对ITR计算更重要，且数据存在类别不平衡问题，需要找到更合适的评估指标。

Method: 在LARESI ERP和OpenBMI ERP两个数据集上，研究拼写率与13个指标的相关性，分析不同试次重复次数下各指标的表现。

Result: Brier分数、MCC、ROC AUC、PR AUC、AP和pAUC是最好反映拼写表现的指标，这些指标能处理类别不平衡问题。

Conclusion: 建议ERP-BCI研究者和从业者报告上述指标以更准确评估系统性能。
...
TL;DR: 提出T2RD算法，通过解耦任务相关和无关表征，提升视觉强化学习的泛化能力，在DMC和机器人操作任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 视觉强化学习在控制任务中表现良好，但泛化到新环境时容易过拟合到任务无关特征，需要解耦表征以提升泛化性。

Method: 提出T2RD算法，包含三个组件：任务相关表征一致性、交叉重构和交叉动态预测。前两者解耦内容与风格特征，第三者通过动态预测进一步提炼任务相关内容特征。

Result: 在DeepMind Control Suite和机器人操作任务上达到SOTA泛化性能和样本效率。

Conclusion: 自监督的任务相关表征解耦方法有效提升了VRL的泛化能力。
...
TL;DR: 提出解构-重组范式(DRP)，通过分解全局运动为局部原子动作并学习可迁移的局部运动表征，有效提升强化学习在机器人控制任务中的样本效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于视频预训练的强化学习方法将智能体视为不可分割的整体进行全局运动建模，导致模型与形态高度耦合，难以跨领域迁移。而不同智能体的局部组件（如关节、肢体）往往展现出相似的运动模式。

Method: 提出DRP范式：1) 解构阶段：识别局部点并追踪其帧间运动，定义为原子动作，使用双注意力编码器(DAE)学习局部运动表征。2) 重组阶段：通过可学习的运动聚合标记[MAT]组合局部运动表征，进行潜在动力学模型学习，并利用适配器连接局部运动与下游动作特定动力学。

Result: 在多种机器人控制与操作任务上成功迁移，显著提升了样本效率和任务性能。

Conclusion: 通过解构-重组的方式学习局部可迁移运动表征，能够有效突破形态约束，实现跨领域迁移，提升强化学习效率。
...
TL;DR: 本文提出多尺度时间对比学习（MTCL）方法，通过在时间相关空间中对视频元素进行区分，平衡不同元素的注意力，从而学习更丰富的表征，提升下游强化学习任务的样本效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的无监督视频预训练方法（如单步转移预测、图像重建）倾向于保留像素空间中占比大的静态信息，忽略了少量但关键的信息，导致表征信息不足。

Method: 提出时间相关空间（temporal correlation space）来区分视频中的每个元素，并采用多尺度时间对比学习（MTCL）方法分别建模不同尺度的时间相关性，以平衡不同元素的注意力。

Result: 实验结果表明，该方法在多个下游任务中提升了样本效率和渐近性能。

Conclusion: 通过均衡关注视频中各元素，MTCL能学习到更具信息量的表征，有效支持下游策略学习。
...
TL;DR: 本文提出了一种基于机器学习的近红外光谱校准方法，用于快速定量土壤中的碳和氮含量，模型RPD>2.0且过拟合低，有助于可持续农业发展。


<details>
  <summary>Details</summary>
Motivation: 传统土壤分析方法耗时且成本高，而近红外光谱技术具有快速、低成本、无损检测的优势，因此研究旨在开发一种高效的机器学习校准模型，用于快速定量土壤碳和氮含量。

Method: 使用便携式MyNIR设备获取土壤NIR光谱数据，采用SG滤波器预处理和基于NIPALS+Huber损失函数的异常值去除方法，使用PLS、SVR、Ridge作为基模型，线性回归作为元模型的Stacking集成学习，通过10折交叉验证、留一法、Kennard-Stone划分等多种验证策略进行模型评估。

Result: 模型RPD>2.0且过拟合程度低，不同土壤类型(Oxisols和Inceptisols)之间存在性能差异，表明土壤学特征对模型有影响。

Conclusion: 该研究验证了基于近红外光谱的机器学习方法在快速定量土壤碳和氮方面的潜力，有助于优化可持续农业实践，为生产者和顾问提供更快的决策依据。
...
TL;DR: 提出一种多源贝叶斯优化框架，利用辅助数据（如代理模型）来高效探索可行域极小的约束优化问题，在早期探索阶段显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在可行域极小的约束优化问题中，传统BO方法仅依赖真实目标函数和约束的评估，难以高效探索设计空间；而现实应用中常存在辅助数据源（如代理模型、简化仿真），但如何将其整合进约束BO尚未充分研究。

Method: 提出一种通用多源框架，扩展了约束最大熵搜索（Constrained Max-value Entropy Search），该框架能够捕捉多源数据间的相关性，同时平衡评估成本与信息增益。

Result: 在合成和基于物理的基准测试中，该方法即使在辅助数据仅弱相关的情况下，也能高效识别可行且最优的解，尤其在早期探索阶段持续优于现有方法。

Conclusion: 多源辅助数据的引入能显著提升约束BO在极小可行域问题上的探索效率，该框架具有普适性和鲁棒性。
...
TL;DR: 本文提出TASA框架，解决混合精度量化中的"困惑度幻觉"和"对齐-多样性权衡"问题，通过任务感知的敏感性分析和校准数据混合优化，使3.5-bit模型在LLaMA-3-8B和Qwen2.5-7B上匹配甚至超越4-bit基线。


<details>
  <summary>Details</summary>
Motivation: 作者发现两个关键问题：(1) 困惑度幻觉——基于困惑度的层重要性排序与复杂推理任务的重要性排序几乎不相关(Kendall τ≈0)；(2) 对齐-多样性权衡——仅使用目标任务校准数据会降低量化后性能，而加入通用域数据可提升鲁棒性。

Method: 提出TASA两阶段框架：第一阶段使用无训练的梯度迹对齐准则搜索校准数据混合比例；第二阶段聚合困惑度和推理导向的敏感性信号，指导层间和层内位宽分配。

Result: 在LLaMA-3-8B和Qwen2.5-7B上，3.5-bit TASA模型匹配或超越4-bit统一精度基线；在GSM8K上相比最强W3基线提升20+绝对百分点。

Conclusion: 校准数据组成对任务敏感量化影响重大，这是以往工作未充分探索的关键因素。
...
TL;DR: 提出Value Diffusion World Models (Valdi)，将潜在扩散模型与模型预测控制结合，在CarRacing环境中使用单步扩散达到与确定性MLP基线相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型在进行模型预测控制时，面临预测速度与表达不确定性之间的权衡，扩散模型虽能建模不确定性但推理慢，难以用于低延迟潜在规划。

Method: 提出Valdi框架，将端到端在线训练的MPC与潜在扩散动力学模型结合，训练和推理均使用单步扩散。

Result: 在CarRacing环境中，Valdi使用单步扩散在训练和推理时均达到与确定性MLP基线相当的性能。

Conclusion: 实验揭示了预测多模态性与控制性能之间存在权衡，Valdi为扩散世界模型用于在线控制提供了可行方案。
...
TL;DR: 提出GMHF框架，利用人类反馈指导生成数据分布，弥合训练与目标域的分布差异，提升模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在目标域数据完全或部分不可用时，泛化到不同环境分布仍面临重大挑战。

Method: 将人类专家反馈与条件神经常微分方程（cNODE）作为数字孪生生成器结合，通过强化学习智能体基于反馈迭代优化生成轨迹的潜在物理参数，使生成数据分布对齐人类对目标物理的信念，从而引导元学习器向目标分布偏移。

Result: 在非线性Duffing振荡器实验中，GMHF显著降低了部署损失，且生成与目标数据的差异在可靠反馈下减小，验证了理论中散度最小化机制；在非动力概率模型上也验证了框架的可扩展性。

Conclusion: 人机协作可作为在分布偏移下实现鲁棒泛化的有效途径，理论分析与实验一致支持该方法。
...
TL;DR: ILLUME+是一个可扩展的癌症药物反应后验可解释性框架，超越单基因评估，捕捉多种互补的解释形式，生成更稳定的基因重要性评分，并支持AI辅助假设生成。


<details>
  <summary>Details</summary>
Motivation: 当前癌症药物反应预测的可解释性方法计算成本高、缺乏鲁棒性，且将复杂药物响应简化为单变量基因重要性评分，忽略了驱动敏感性和耐药性的协调基因活动。

Method: 提出ILLUME+，一个可扩展的后验可解释性框架，集成到端到端流程中，捕捉多种互补的解释形式。

Result: ILLUME+比现有基线方法生成更稳定的基因重要性评分，恢复了已知的药物-基因关联和作用机制。

Conclusion: ILLUME+能够实现AI辅助假设生成，揭示癌症生物学中新型的相互作用驱动的分子信号。
...
TL;DR: 本文提出扩散优化方法，将流模型的数据流形映射到简单基空间进行梯度下降，等价于黎曼梯度下降。在蛋白质设计中扩展到SO(3)和SE(3)群，在二级结构靶向、肽结合亲和力和Rosetta能量优化上均显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 生成模型学习的数据分布在低维流形上，直接梯度下降会偏离流形几何，优化困难。现有方法缺乏对流形结构的利用，需要一种能够保持在流形上、同时获得更平滑优化曲面的方法。

Method: 利用扩散模型和流模型提供从数据流形到简单基空间的映射，在基空间进行梯度下降（等价于数据流形上的黎曼梯度下降，误差O(λ²)）。针对蛋白质设计，扩展到SO(3)和SE(3)矩阵李群，推导了与autograd兼容的SO(3)梯度，以及用于通过李群ODE求解器反向传播的广义伴随状态方法。

Result: 二级结构靶向：FrameFlow上91.3% vs 63.3%的残基达到Ramachandran目标；肽结合亲和力：以2倍速度优于OC-Flow；PDB测试集上Rosetta能量降低数千单位。

Conclusion: 扩散优化通过利用流形几何显著提升了生成模型的优化效果，特别是在蛋白质设计任务上展现了巨大优势。
...
TL;DR: 本文提出Aionoscope，一种基于生成器的诊断工具，用于评估冻结时间序列表示中潜在状态的可访问性。发现大多数系统能捕捉粗粒度信息（如信号类型），但难以可靠地提取细粒度过程状态变量（如时序、相位、振幅等），揭示了粗粒度和细粒度可访问性之间的不匹配。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列模型评估仅关注预测或分类分数，无法揭示表示是否保留了用户可能希望检查的过程状态变量（如事件时序、相位、振幅、频率或状态变量）。

Method: 提出Aionoscope生成器诊断工具，将过程生成与观测渲染分离，生成带精确类别和密集标签的合成流数据。实例化为原始过程混合（Primitive Process Mixtures），并通过37个模型加适配器系统进行公共线性探针评估。

Result: 粗粒度可访问性良好，但密集过程状态可访问性较低：最高密集探针行均值掩码R²为0.689，而密集特征oracle达到0.999。存在粗粒度和细粒度可访问性的不匹配问题。

Conclusion: Aionoscope能够揭示表示在信号类型层面看起来信息丰富，但隐藏了调试所需的关键细粒度过程状态变量（时序、相位、振幅、频率、状态变量）的失败模式。
...
TL;DR: 本文提出LeNEPA，一种无需数据增强的时序自监督学习方法，通过潜在空间预测与SIGReg正则化，在跨数据集迁移时表现优于传统增强依赖方法。


<details>
  <summary>Details</summary>
Motivation: 时序自监督学习过度依赖视图增强和数据增强，而这些增强方式编码了领域特定的不变性，导致方法在不同数据集间迁移时性能下降。本文旨在设计一种无需增强、跨域鲁棒的SSL方法。

Method: 提出LeNEPA架构，采用因果骨干网络、无增强的下一潜在令牌预测目标、SIGReg各向同性正则化替换stop-gradient/EMA稳定策略，并在轻量投影空间中计算预测损失。

Result: LeNEPA在PTB-XL和Diag数据集上均保持良好冻结探针性能；学习曲线显示2-5k更新达到80%最终增益；在UCR-128基准上达到77.65%准确率，接近MOMENT（77.89%）。

Conclusion: 无增强的潜在预测是低微调代价时序自监督学习的有效方案，具备跨域迁移的实用价值。
...
TL;DR: 利用语音数据自动检测压力状态，通过机器学习模型区分压力与非压力情境，并预测生理和情感压力反应。


<details>
  <summary>Details</summary>
Motivation: 实现 unobtrusive（非侵入式）的自动压力检测，服务于行为研究和临床评估。

Method: 采集50名参与者的语音数据（TSST压力任务 vs 对照条件），使用说话人日志化（speaker diarization）和机器学习模型进行压力检测与预测。

Result: 压力检测性能显著高于均值基线，部分生理和情感压力反应可被声学韵律特征预测。

Conclusion: 语音可作为人类压力反应多个维度的有效、非侵入式指标。
...
TL;DR: 本文提出Flow Proposal Particle Filters (FPPF)，通过学习条件生成模型作为粒子滤波的提议分布，结合贝叶斯更新，在非线性、非高斯和高维动态系统中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统粒子滤波在高维中易坍缩，而现有生成方法缺乏贝叶斯更新导致误差累积，需要一种既能利用生成模型又能保持正确贝叶斯更新的滤波方法。

Method: FPPF学习一个条件生成模型作为提议分布，近似方差最小化的最优提议，通过观测条件引导粒子向高似然区域移动，同时保持可处理的似然评估以计算准确的重要性权重，并通过局部化策略处理高维问题。

Result: 在多种动态系统实验中，FPPF在非线性、非高斯和高维场景下优于统计基线和其它生成方法。

Conclusion: FPPF结合了生成模型的灵活性与贝叶斯更新的原则性，有效缓解了粒子滤波在高维中的退化问题。
...
TL;DR: SEAHORSE是一个统一的可复现框架，用于标准化时空点过程（STPP）模型的实验对比，并配套合成压力测试套件HawkesNest来揭示不同模型族的归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 现有的神经STPP模型因实现方式（预处理、坐标归一化、数据划分、似然约定、评估协议）不统一，导致比较结果不可靠，亟需一个标准化框架来实现公平对比和可控诊断研究。

Method: 提出SEAHORSE框架，将神经STPP模型统一为"编码-演化-解码"接口，在单一可执行基准协议下训练、调参和评估；同时构建HawkesNest合成压力测试套件，通过逐步增加事件模式复杂度来暴露各模型族的归纳偏置。

Result: SEAHORSE实现了不同模型族之间的公平比较；HawkesNest测试表明，事件模式复杂度增加会不同程度地影响不同模型族的性能，部分模型性能严重下降，另一些保持稳定。

Conclusion: SEAHORSE为STPP研究提供了标准化实验框架和诊断工具，有助于更深入理解神经STPP模型的归纳偏置与适用场景。
...
TL;DR: 本文系统研究了不同训练方式构建的模型生物（MOs）对可解释性评估的影响，发现标准事后微调方法会使可解释性变得不切实际地容易，而更真实的集成训练方法生成的MOs可解释性更低，质疑了当前MOs作为可解释性代理的有效性。


<details>
  <summary>Details</summary>
Motivation: 质疑当前模型生物（MOs）作为可解释性测试床的有效性，探究不同训练方法对可解释性评估结果的影响。

Method: 构建了54个基于OLMo2-1B和gemma-3-1b-it的MOs，使用7种不同训练技术（包括事后SFT、事后DPO、以及将MO数据集成到OLMo事后训练DPO阶段），并基准测试了激活oracle、激活引导、logit lens和稀疏自编码器。

Result: (i) MO可解释性强烈依赖于训练目标、目标行为、模型架构和训练数据生成流程；(ii) 即使控制了目标行为表达强度差异，仍存在显著方差；(iii) 更真实的集成训练往往比标准事后方法生成的可解释性更低。

Conclusion: 当前MOs作为可解释性代理的有效性受到严重质疑，事后训练方法可能使可解释性变得不切实际地容易。
...
TL;DR: 提出 Gain-Shape K-means (GSKM) 替代标准 K-means，解决高维质心收缩导致的方向保持问题，并将其融入残差量化 (RQ) 流水线得到 GSRQ，在 KV 缓存压缩上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型长上下文部署受限于 KV 缓存线性增长，向量量化虽能压缩但标准 ℓ₂ K-means 在高维中存在质心收缩问题，削弱角度对齐，影响方向保持。

Method: 提出 Gain-Shape K-means (GSKM) 替代标准 K-means，改善方向保真度；将加权 GSKM 融入残差量化 (RQ) 流水线，构建 Gain-Shape Residual Quantization (GSRQ)。

Result: 在 LLaMA-3-8B 上，GSRQ 在各比特率下均优于强基线；1-bit 时 LongBench 平均准确率从 11.34 提升至 33.54，比 VQLLM 高出 22.20 个百分点。

Conclusion: GSKM 可无缝替代 K-means 提升方向保真度，GSRQ 在极低比特率下实现 KV 缓存的高质量压缩。
...
TL;DR: 提出使用投影量子核与经典核近似技术，在GP bandit优化中降低量子核的维度和计算开销，并推导了误指定GP bandit算法的遗憾界，平衡近似误差与信息增益。


<details>
  <summary>Details</summary>
Motivation: 量子核函数虽然能提供量子优势，但直接使用高维量子核会导致模型复杂度高、信息增益大、累积遗憾高、可学习性差，需要降低复杂度同时保留量子特性。

Method: 提出投影量子核和经典核近似方法降维，在此基础上开发误指定的GP bandit算法，并推导遗憾界以指导最优模型复杂度选择。

Result: 所提方法在样本效率上优于完整量子核，同时大幅降低计算开销，实现可扩展的GP优化。

Conclusion: 投影量子核与经典核近似能有效解决量子GP bandit优化中的复杂度和可学习性问题。
...
TL;DR: 本文研究了在时空点过程模型中，当局部事件历史稀疏时，外生空间上下文（AlphaEarth嵌入）能否补偿数据不足的问题。实验表明，在紧急医疗服务预测中，加入空间上下文显著提升了跨区域预测性能，尤其在历史数据稀缺时提升达2-6倍。


<details>
  <summary>Details</summary>
Motivation: 时空点过程模型在局部事件历史稀疏时难以泛化到新区域，需要研究外生空间上下文能否弥补这一不足。

Method: 使用固定的对数高斯Cox过程作为基础模型，对比纯事件模型与加入AlphaEarth嵌入作为线性空间上下文的模型，在8个保留区域进行EMS预测的空间迁移评估。

Result: AE嵌入在所有历史长度下均提升了跨区域预测性能，在历史稀缺（1-2周）时提升约2-6倍，在历史充足（20-104周）时提升约10-20%。

Conclusion: 空间上下文信息能够在事件历史有限时显著稳定空间迁移的点过程预测。
...
TL;DR: 本文研究了异步GRPO中陈旧rollout带来的偏差和稳定性问题，推导了偏差阶数和双约束稳定性条件。


<details>
  <summary>Details</summary>
Motivation: 高吞吐RLHF系统常因解耦rollout生成与策略优化而导致使用陈旧rollout，但其对训练稳定性的影响尚不明确。

Method: 将行为策略显式纳入GRPO替代目标，区分替代梯度映射与真实全导数，在局部有界性、分布平滑性和行为策略平滑性假设下进行理论分析。

Result: 陈旧rollout引入每步替代梯度偏差 O(S * η)；推导出条件性崩溃时间标度律，得到双约束稳定性条件 η << min{R_batch/(S*G_upd), R_crit/(T*G_upd)}。

Conclusion: 解释了在horizon受限机制下，最大稳定学习率为何看似对陈旧度弱依赖。
...
TL;DR: CausalMix将数据混合优化建模为因果推断问题，通过条件平均处理效应（CATE）估计最优数据混合比例，实现了从代理模型到更大规模数据池和模型的可扩展泛化，且在多个下游任务上优于RegMix等基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据混合优化方法依赖静态数据分布假设，当数据池变化时需要昂贵的从头重新训练，无法从小规模平滑扩展到更大的数据池和模型规模。

Method: 将数据池的统计特征作为协变量、领域混合作为处理变量，在512次Qwen2.5-0.5B运行上拟合因果模型估计CATE，然后外推800K数据池的最优混合比例并应用于7B模型训练，并成功泛化到Qwen3-4B-Base的长思维链数据。

Result: CausalMix引导的混合策略在多个下游任务上持续提升性能，优于RegMix等基线方法；CATE解释器提供了混合策略的可视化分析。

Conclusion: CausalMix提供了一个因果且可解释的框架用于优化LLM数据混合，具备动态推断状态依赖最优混合的能力。
...
TL;DR: SynLaD是一个统一配体设计目标与合成可行性的小分子生成框架，通过潜扩散模型同时生成3D结构和合成路线，在生物活性配体类似物生成任务中优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有分子生成模型通常优化配体设计（做什么）或合成可行性（怎么做）中的一个目标，而牺牲另一个，导致难以发现高分且可合成的分子。

Method: SynLaD构建了一个编码器-双解码器架构：编码器将分子映射到潜空间，几何头解码原子类型和坐标，自回归合成头输出反应序列。扩散Transformer在药效团条件下生成新的潜变量。

Result: 在生物活性配体类似物生成任务中，SynLaD在可合成性和多样性命中生成方面优于现有基线模型。

Conclusion: 单一模型能够同时产生形状对齐的分子和可行的合成路线，证明潜扩散框架可以统一分子设计与合成规划。
...
TL;DR: 本文提出QuasiMoTTo方法，利用准蒙特卡洛(QMC)生成相关但精确的样本，替代独立同分布采样，在推理扩展和强化学习中显著提升样本效率（节省25-47%样本），且每个样本仍保持原始语言模型的边际分布。


<details>
  <summary>Details</summary>
Motivation: 传统的并行采样依赖独立同分布假设，导致大量冗余计算浪费。作者认为独立性并非必要约束，探索相关但精确的并行采样空间可提升样本效率。

Method: 将自回归采样重参数化为逆CDF采样，用准蒙特卡洛(QMC)方法生成均匀分布，使样本更均匀地覆盖输出空间，从而减少冗余。同时开发了无偏bootstrap估计器来评估相关采样器的pass@k性能。

Result: 在四个推理基准上，QuasiMoTTo以25-47%更少样本达到同等pass@k精度；在GRPO强化学习中，以50%更少训练步骤匹配i.i.d.性能。

Conclusion: 通过生成相关但边际分布保持的样本，可以显著提升推理扩展和强化学习中的样本效率，且该方法经常能饱和pass@k的理论上限。
...
TL;DR: Muon优化器可理解为隐式残差连接，通过正交化更新牺牲局部梯度保真度，但改善了表征保留，有利于下游层利用。


<details>
  <summary>Details</summary>
Motivation: 尽管Muon在训练大模型上效果显著，但其成功原因的理论解释尚不统一，缺乏一个简洁直观的机制理解。

Method: 提出机械论解释：Muon的正交化更新可视为隐式残差连接；在可控线性优化设置中研究局部梯度保真度与表征保留之间的权衡。

Result: Muon学习的表征虽然拟合局部目标较慢，但更容易被下游层利用，验证了隐式残差连接的解释。

Conclusion: 为Muon提供了概念性解释，并启发了设计能够平衡局部下降与下游可用性的优化器新视角。
...
TL;DR: 提出一种对抗生成器-判别器框架，在可验证奖励基础上增加从人类演示中学到的信号，同时优化任务的可验证和不可验证属性。


<details>
  <summary>Details</summary>
Motivation: 当前RLVR方法只优化可客观评分的指标，忽略了风格、结构等不可验证的主观方面，导致多样性崩塌、奖励破解等问题。

Method: 采用对抗生成器-判别器框架：生成器通过RL最大化任务准确性和来自判别器的对抗奖励；判别器学习区分人类输出与模型生成输出，作为人类输出分布的代理。

Result: 在bug修复中编辑距离显著降低；在故事生成中胜率提升且更人性化；在奖励破解基准中几乎消除模型不良行为，同时保持高分。

Conclusion: 该方法弥合了RL与SFT之间的差距，为联合优化任务的可验证和不可验证属性提供了可扩展的路径。
...
TL;DR: 提出ZO-Act方法，通过将扰动限制在输入激活导出的固定低秩子空间中，降低零阶优化方差，实现高效的大语言模型微调。


<details>
  <summary>Details</summary>
Motivation: 现有零阶优化方法扰动全模型权重或随机低维子空间，导致高方差估计和有限性能。

Method: ZO-Act在初始化时计算每个线性层的激活基，冻结低比特权重，仅优化轻量级系数矩阵，使用前向损失评估。

Result: 在Llama-3-8B、OPT-13B和INT4 Llama-3-8B上，在语言理解、问答和常识推理任务中持续优于强零阶微调基线。

Conclusion: 低维系数扰动可降低方差依赖的收敛项和有限差分误差，子空间逼近偏差被LLM激活和梯度的低秩结构缓解。
...
TL;DR: 提出GAIA模型，一种几何自适应积分自编码器，通过几何token和交叉注意力机制统一处理正向、边界值及逆问题，在多个2D/3D基准上取得了最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有几何自适应神经算子主要针对输入输出共享同一空间域的正向问题，无法有效处理边界值问题(BVP)和逆问题（输入输出位于不同域），因此需要一种统一架构来处理各类问题。

Method: 将域边界和内部场分布编码为几何token，通过交叉注意力机制使积分变换层自适应地关注局部几何特征，实现单次前向即可处理正向、BVP和逆问题。

Result: 在7个2D/3D基准上，翼型流重建中位相对L2误差降低64%，EIT降低27%，在所有逆问题和BVP任务上取得新SOTA，且正向任务上与专用求解器性能相当。

Conclusion: GAIA能够统一处理多种偏微分方程问题类型，无需重训练或迭代优化，在精度和泛化性上优于现有方法。
...
TL;DR: 本文提出IMPFM框架，通过多粒子交互流图实现样本高效的在线反馈驱动搜索，用于对齐未知偏好，兼顾全局探索与奖励过优化缓解。


<details>
  <summary>Details</summary>
Motivation: 现有生成式模型的奖励对齐方法在偏好未知且仅通过顺序反馈才能获知时，难以进行广泛的全局探索，只能局限于局部区域。

Method: 提出Sequentially-Controlled Interactive Multi-Particle Flow-Maps (IMPFM)，包含后验样本共享机制、探索-利用重加权机制，以及多粒子交互感知的Feynman-Kac校正器，逐步将粒子系统推向KL倾斜的目标分布。

Result: 大量实验和消融研究证明IMPFM在多种搜索和对齐任务上优于现有基线方法。

Conclusion: IMPFM通过多粒子交互和顺序校正，实现了全局探索、结构多样性保持和模式坍缩预防，是一种有效的在线反馈对齐框架。
...
TL;DR: 本文发现强化学习后训练对LLM的提升高度集中在少数中间Transformer层，单层训练即可恢复大部分全参数训练收益，且该模式跨模型、算法和任务稳定存在。


<details>
  <summary>Details</summary>
Motivation: 挑战RL后训练中所有层均匀贡献的默认假设，探究RL适应在Transformer各层中的分布规律。

Method: 提出"层贡献度"指标，系统性地对Qwen3、Qwen2.5系列模型（7个）使用GRPO、GiGPO、Dr. GRPO三种算法进行逐层隔离训练实验，涵盖数学推理、代码生成、智能体决策等任务。

Result: RL收益高度集中在少数甚至单个Transformer层；高贡献层集中在Transformer中部，输入输出端贡献显著更低；层排名跨数据集、任务、模型族和RL算法高度相关。

Conclusion: RL后训练的核心收益来源于少数中间层，这为更高效的RL微调提供了理论基础，有望减少计算开销并提升训练效果。
...
TL;DR: 提出ER-JEPA框架，一种轻量级自监督学习方法，用于心电图（ECG）多变量时间序列分析，采用层次化双重JEPA结构，在180,000条记录上预训练后，在ST-MEM基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 医学领域常面临目标数据集有限而大量未标注数据可用的场景，自监督学习能有效利用大规模数据，本文旨在为心电图分析设计高效轻量的SSL框架。

Method: 提出Event Reconstruction Joint-Embedding Predictive Architecture (ER-JEPA)，包含两级结构：先构建时间间隔表征，再将其作为单变量时间序列处理；层次化整合两个JEPA；采用Vision Transformer骨干网络。

Result: 在约180,000条10秒记录上预训练，在ST-MEM基准测试中达到最先进的下游性能，且计算速度快、资源消耗低。

Conclusion: ER-JEPA（H-JEPA）在ECG多变量时间序列分析中有效，验证了层次化表征预训练阶段的敏感性分析，为医疗时间序列自监督学习提供了新思路。
...
TL;DR: 本文提出了一种基于3D高斯原始体的显式体数据压缩模型，通过可微渲染和CUDA加速，在结构化与非结构化体数据上均取得有竞争力的压缩性能，且无需存储网格。


<details>
  <summary>Details</summary>
Motivation: 现有隐式神经表示（INR）在压缩非结构化体数据时无法编码几何，需要额外存储网格，限制了压缩率。作者希望设计一种显式模型，既能编码几何又能高效压缩。

Method: 将3D高斯集合重新解释为标量场的显式表示，通过加权聚合相交高斯来重建空间位置处的标量值；设计了CUDA加速的结构化和非结构化采样管道、基于采样误差的稠密化策略以及针对性的损失函数。

Result: 在结构化体数据上，重建质量与现有INR相当，但训练速度显著提升；在非结构化体数据上，所有指标均大幅优于现有方法。

Conclusion: 提出的显式3D高斯模型天然编码域几何，消除了非结构化体数据对网格存储的需求，实现了更高的压缩机会，是一种高效且有竞争力的体数据压缩方案。
...
TL;DR: 提出了一种决策感知训练方法，将决策损失与能量评分结合，使生成模型在训练时关注下游决策成本，在成本敏感区域取得改进。


<details>
  <summary>Details</summary>
Motivation: 现有生成式概率预测模型训练时只关注数据密度，忽视了下游决策者对预测误差的成本结构，导致高风险决策场景中表现不佳。

Method: 在能量评分目标函数基础上，引入可微的决策损失项，该损失直接惩罚基于模型预测做出决策所导致的成本，且理论上被证明自身也是适当的评分规则。

Result: 在1个合成任务和2个真实任务上验证，表明该方法在成本敏感区域有针对性改进，同时保留了完整的概率预测能力。

Conclusion: 决策感知训练能有效提升生成模型在成本敏感场景下的实用性。
...
TL;DR: 提出神经证书定价（NCP）框架，利用无监督学习训练神经网络预测对偶价格，通过结构化恢复层构造可行原始边际解，在多个组合优化问题上超越或媲美现有方法。


<details>
  <summary>Details</summary>
Motivation: 组合优化问题因离散结构导致指数级搜索空间，但可行解的验证可在多项式时间内完成。现有方法枚举违反不等式效率低下，缺乏利用这种不对称性的学习框架。

Method: 神经网络预测证书级对偶价格，结构化恢复层构造原始边际解。采用无监督学习，将证书一致性条件作为训练约束，使恢复的边际解全局可行。

Result: 在三类组合优化问题上，NCP大幅超越或持平现有神经方法，且计算时间更少，展现出更强的分布外泛化能力。

Conclusion: NCP有效利用了组合优化中"验证易、搜索难"的不对称性，将分离过程摊销为对偶价格学习，具有良好的理论保证和实用效果。
...
TL;DR: 本文通过在监督学习和强化学习场景下对比7对量子机器学习模型与经典基线模型，发现QML在整体预测性能、策略稳定性和训练时间上尚未超越经典模型，但在噪声过滤和假阳性控制方面具有潜力。


<details>
  <summary>Details</summary>
Motivation: 当前关于量子机器学习相较于经典模型性能优势的证据不足，本文旨在通过实证研究填补这一空白。

Method: 在监督学习和强化学习两个领域，选用7对模型进行对比实验，评估QML模型与经典基线模型的表现。

Result: QML模型在整体预测性能、策略稳定性和训练时间上未超越经典基线；但在噪声过滤和假阳性控制方面表现有潜力。

Conclusion: QML在硬件环境、训练效率和收敛稳定性方面仍面临挑战，但为后续的鲁棒性和参数优化研究提供了基础。
...
TL;DR: TiRex-2是一个基于xLSTM的循环时间序列基础模型，支持多变量预测，以恒定计算成本处理流式数据，在零样本任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer时序模型在流式场景下需要重计算全部历史，复杂度高，且难以同时处理未来已知协变量与因果约束。

Method: 采用循环记忆机制（xLSTM）+ 双向时间混合器 + 非对称分组注意力变量混合器，以及合成耦合管道进行多变量预训练。

Result: 在GIFT-Eval和fev-bench上达到零样本SOTA，支持任意上下文长度流式推理，每补丁推理成本恒定。

Conclusion: TiRex-2是首个同时支持流式处理、未来已知协变量和严格因果约束的时间序列基础模型。
...
TL;DR: 提出了语言批评框架（LC-BC和LC-DP），利用自然语言作为结构化监督信号，从次优演示中学习模仿策略，在连续控制任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统从次优演示中学习模仿的方法使用标量信号（如置信度、判别器分数），无法表达中间推理、故障模式或纠正动作，信息表达受限。

Method: 先构建包含任务进度、次优行为和纠正指导的语言标签，再提出语言批评损失函数，直接用结构化语言信号训练策略（行为克隆和扩散策略），并给出理论上界。
...
TL;DR: 本文提出"建设性对齐"范式，将AI对齐重新定义为控制人类偏好演化轨迹的问题，而非静态偏好满足。


<details>
  <summary>Details</summary>
Motivation: 现有AI对齐方法将人类偏好视为固定目标，但实证研究表明偏好是分层、动态且在与技术交互中构建的。需重新审视对齐问题。

Method: 借鉴行为经济学、心理学和建构主义社会理论，将偏好建模为分层状态变量，采用控制理论框架形式化系统行为与人类评估状态的交互演化。

Result: 提出对齐应关注调节AI系统如何影响人类偏好的演化，确保价值轨迹连贯、反思认可、认识论扎实、免受操纵、赋能不确定性。

Conclusion: 对齐问题从静态偏好满足转变为治理长期价值形成的问题。
...
TL;DR: 本文提出"有界道德"框架，将西蒙的有界理性概念扩展至道德认知领域，认为伦理理论并非固定真理，而是在有限计算资源下针对不同道德问题的高效策略。道德对齐应关注推理能力的扩展与分配，而非直接模仿人类判断。


<details>
  <summary>Details</summary>
Motivation: 传统道德认知建模将伦理视为固定规则或价值函数，忽略了有限智能体面临的计算约束。作者希望将有界理性思想引入道德推理，解释不同伦理理论为何在特定情境下更有效。

Method: 沿两个正交维度形式化道德情境：道德广度（被视为道德相关的实体范围）和道德深度（评估交互所需的推理整合程度）。有限资源迫使两者间存在不可回避的权衡，由此定义道德计算的可行空间。

Result: 不同伦理理论对应于该可行空间中适应不同需求模式的高效策略，而非竞争性的道德真理描述。框架形式化了道德遗憾和约束下的道德进步概念。

Conclusion: 人工系统中的道德对齐取决于道德推理能力的扩展与合理分配，而非直接模仿人类判断。
...
TL;DR: MMM是一个面向跨学科协作的知识文档数据模型，结合少量规范约束与自由文本标签，实现跨领域互操作性。


<details>
  <summary>Details</summary>
Motivation: 传统文档为中心的信息系统限制了知识的结构化、更新、共享和复用，而形式化方法又因过度强调结构而牺牲了可用性。

Method: 提出MMM数据模型，结合少量规范性约束与自由文本标签的表达自由，面向跨学科互操作性设计，无需语义收敛。

Result: 提供了参考实现和试点部署数据，证明了可实现性和早期可用性。

Conclusion: MMM在文档中心系统与形式化方法之间取得平衡，为跨学科知识文档提供了一种可互操作的轻量级替代方案。
...
### [295] [From Signals to Structure: How Memory Architecture Drives Language Emergence in LLM Agents](https://arxiv.org/abs/2607.00233)
*Yashar Talebirad,Eden Redman,Ali Parsaee,Osmar R. Zaiane*

Main category: cs.AI

TL;DR: 研究记忆架构对智能体在刘易斯信号博弈中发明共享语言的影响，发现带私有笔记本的智能体比无状态智能体更能稳定协调，通道容量不如记忆架构关键。


<details>
  <summary>Details</summary>
Motivation: 探索两个智能体如何从零开始发明共享语言，特别是记忆架构和通道容量在语言涌现中的作用。

Method: 在刘易斯信号博弈中设置发送者和接收者，对比五种记忆架构在不同通道配置下的表现，使用LLM智能体进行实验。

Result: 带私有笔记本的智能体在容量=25时达到最高协调分数0.867±0.023；无状态智能体在中等容量时表现最佳，容量过大反而退化；信息瓶颈（容量=8）成为脆弱点而非最优值。

Conclusion: 通道容量本身无法预测协调效果，记忆架构决定了智能体能否将交互历史转化为稳定约定，二者结合才能理解信号如何演变为语言。
...
### [296] [Making Failure Safe: A Constrained, Verifiable Agent Framework for Open-Web Data Collection](https://arxiv.org/abs/2607.00035)
*Bo Chen*

Main category: cs.AI

TL;DR: 提出了一种约束性、可验证的智能体框架，将LLM输出从自由代码转换为类型化JSON采集器配置，通过六类型采集器分类法、模板约束、静态Airflow DAG执行和规则质量检查，实现可复用、低成本、确定性的网络数据采集。


<details>
  <summary>Details</summary>
Motivation: LLM和智能体从自然语言需求生成网络爬虫时，存在依赖错误、选择器失效、模式不匹配和页面结构异构等问题，直接生成不可靠。

Method: 提出约束性可验证智能体框架：1) 六类型采集器分类法；2) 模板和工具函数约束；3) 静态Airflow DAG执行；4) 基于规则的质量检查；5) 结构化反馈修正。将LLM输出从自由代码转换为类型化JSON配置。

Result: 在138个任务上，分类法支持基于描述的需求类型化；在80个独立源验证的任务上，框架实现了零执行阶段LLM token消耗和最低平均墙上时钟时间。

Conclusion: 该框架提供了可复用、低成本、可验证的执行路径，适合重复调度的开放网络数据采集，在单次生成质量上有所取舍，但换来了确定性、可验证的执行路径。
...
### [297] [Agri-SAGE: Simulation-Grounded Multi-Agent LLM for Context-Aware Agricultural Advisory Generation](https://arxiv.org/abs/2607.00454)
*Vedant Balasubramaniam,Geetha Charan,Manojkumar Patil,Rohit P Suresh,V Priyanka,Kodur Sai Vinay Sathvik,Y. Narahari*

Main category: cs.AI

TL;DR: Agri-SAGE是一个结合检索增强多智能体LLM推理与APSIM生物物理模拟的闭环框架，用于生成和验证农业咨询建议。三种推理方法（Plan-and-Solve、Tree of Thoughts、Reflexion）均显著优于静态基线，其中Tree of Thoughts取得最高产量，Reflexion以更低计算成本达到相近效果。


<details>
  <summary>Details</summary>
Motivation: 现有农业咨询系统存在两难：静态指南无法适应季节性变化，而LLM驱动的系统可能生成农学上看似合理但生理学上不可靠的建议。

Method: 提出Agri-SAGE闭环框架，将检索增强的多智能体LLM推理与APSIM生物物理模拟相结合，并评估三种推理策略（Plan-and-Solve、Tree of Thoughts、Reflexion）。

Result: 三种推理方法均显著优于静态Package-of-Practice基线；Tree of Thoughts达到最高产量峰值；Reflexion以更低计算成本实现可比结果。

Conclusion: 集成生物物理模拟验证的LLM推理框架能有效提升农业咨询的可靠性，且不同推理策略在性能与成本间存在权衡。
...
### [298] [Solution space path planning for supporting en-route air traffic control](https://arxiv.org/abs/2607.00064)
*Yiyuan Zou,Wenying Lyu,Clark Borst*

Main category: cs.AI

TL;DR: 本文提出了一种面向空中交通管制的可解释、高效的冲突解脱路径规划算法（SSPPV/SSPPE），在MUAC Delta扇区实测中平均3.69毫秒即可生成可行路径。


<details>
  <summary>Details</summary>
Motivation: 现有路径规划算法与管制员的实际需求脱节，缺乏可解释性和计算效率，亟需设计以人为本、可解释且高效的决策支持工具。

Method: 集成三种意图冲突检测方法（距离、时间间隔、区域），在解空间显示框架内，提出了基于顶点（SSPPV）和基于边（SSPPE）的搜索节点进行路径规划。

Result: SSPPV结合区域冲突检测性能最佳，在基于MUAC Delta扇区5海里网格的运营场景中，平均计算时间仅3.69毫秒。

Conclusion: 该算法兼顾了解释性、灵活性和计算效率，能有效支持管制员的战术决策。
...
### [299] [RareDxR1: Autonomous Medical Reasoning for Rare Disease Diagnosis Beyond Human Annotation](https://arxiv.org/abs/2607.00147)
*Deyang Jiang,Haoran Wu,Ziyi Wang,Yiming Rong,Yunlong Zhao,Ye Jin,Bo Xu*

Main category: cs.AI

TL;DR: 提出了RareDxR1，一个端到端的推理密集型大语言模型，用于从非结构化临床笔记中进行开放域罕见病诊断，通过渐进式训练框架、反思增强推理采样和双级课程强化学习，实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有AI方法（基于管道的表型提取或检索增强生成）因预定义本体、检索瓶颈和缺乏诊断逻辑导致关键信息丢失，亟需一种不依赖结构化表型和闭集决策的端到端罕见病诊断方法。

Method: 设计渐进式端到端训练框架（知识内化+自主进化学习）；提出反思增强推理采样(RERS)用于合成专家级诊断轨迹；提出双级课程强化学习逐步掌握罕见病诊断。

Result: 在不同基准测试上均达到最先进水平，在开放域罕见病诊断上取得显著突破。

Conclusion: RareDxR1有效克服了传统方法的局限性，实现了从非结构化临床笔记直接进行罕见病诊断的端到端解决方案。
...
### [300] [AutoMem: Automated Learning of Memory as a Cognitive Skill](https://arxiv.org/abs/2607.01224)
*Shengguang Wu,Hao Zhu,Yuhui Zhang,Xiaohan Wang,Serena Yeung-Levy*

Main category: cs.AI

TL;DR: 提出AutoMem框架，将记忆管理作为可训练技能，通过自动优化记忆结构和记忆熟练度，使LLM在长时域任务中性能提升2-4倍，32B模型可与前沿系统竞争。


<details>
  <summary>Details</summary>
Motivation: 长时域任务中，LLM的记忆管理（何时编码、何时检索、如何组织知识）至关重要，但手动优化记忆结构和记忆熟练度非常困难，因为任务轨迹长达数千步且记忆错误难以追溯。

Method: 提出AutoMem双循环框架：外环用强LLM审查完整轨迹并迭代优化记忆结构（提示、文件模式、动作词汇）；内环从多轮对话中识别优质记忆决策作为训练信号，直接提升模型的记忆熟练度。

Result: 在Crafter、MiniHack、NetHack三个程序化生成的长时域游戏中，仅优化记忆管理（不修改任务动作行为）使基础Agent性能提升约2-4倍，32B开源模型达到与Claude Opus 4.5和Gemini 3.1 Pro Thinking竞争的水平。

Conclusion: 记忆管理是独立可学习的技能，是长时域任务中的高杠杆优化目标。
...
TL;DR: 研究AI与人类在双向私有信息下的运行时监督问题，提出一个上下文赌博机团队博弈模型，揭示了"可避免伤害区域"的存在及其成因。


<details>
  <summary>Details</summary>
Motivation: 现实场景中，人类知道自己的奖励函数但不知道AI提案的质量，而AI知道提案质量但不知道人类偏好。这种信息不对称导致人类可能错过值得监督的AI有害行为。作者希望刻画这种信息不对称的后果并寻找解决方案。

Method: 基于CIRL和监督博弈，构建了一个具有双向不对称信息的上下文赌博机团队博弈，包含play/ask/trust/oversee四种接口，并用单轮特征刻画团队最优和近视人类行为的差距。

Result: 发现了"可避免伤害区域"：即AI明知提案有害但近视人类因信任先验而拒绝监督的情形。证明了这一差距源于不可信的监督沟通，并分析了多轮动态中的被动学习与主动信号传递。

Conclusion: 不可信的监督沟通是有代价的，但通过多轮被动学习和延迟一期的监督响应可以部分缓解。
...
TL;DR: 本研究提出了Epistemic AI Literacy (EAIL)框架，通过分析人机协作编程对话数据，发现78.8%的学生与GenAI互动缺乏认知参与，仅11.1%表现出高认知投入。


<details>
  <summary>Details</summary>
Motivation: 目前AI素养研究多关注技能层面，忽视了学习过程中与AI交互时发生的认知思维过程，亟需从认知视角重新定义AI素养。

Method: 基于AIR认知框架，分析大规模人机协作编程对话数据集，识别认知目标（掌握型目标）和认知过程（外包、解释寻求、验证寻求、提示监控、认知论证）的可观测维度。

Result: 78.8%的交互依赖非掌握型目标和不可靠认知策略（如外包、验证寻求）；仅11.1%的交互显示出高认知投入，结合了掌握型目标与认知论证等高级策略。

Conclusion: 学生普遍缺乏认知性AI素养，多数人机交互停留在表面策略层面，需要加强AI教育中认知过程的培养。
...
TL;DR: Seed2.0是一个专注于解决复杂真实世界任务的模型系列，通过构建可靠的评估体系，在长尾知识、复杂指令跟随、推理、视觉理解和搜索能力方面取得显著进展。


<details>
  <summary>Details</summary>
Motivation: 识别用户的真实需求，构建可靠的评估体系，解决长尾知识和复杂指令跟随这两个持久挑战，提升模型在复杂长期任务上的可靠性。

Method: 从用户真实需求出发，选择和抽象相关基准来构建评估系统；针对长尾知识和复杂指令跟随两大挑战进行改进。

Result: 在推理智能、视觉理解和搜索能力方面达到世界领先水平，通过大量真实用例展示了处理初始复杂真实世界任务的能力。

Conclusion: Seed2.0开始展现出处理复杂真实世界任务的能力，为数十亿用户带来更大价值。
...
TL;DR: 本文提出Agentic事务处理（ATP）模型，将LLM等生成的动作视为不可信提案，通过确定性约束集C进行准入控制，确保提交状态正确性独立于生成层的能力。实现系统Mnemosyne证明了安全属性，开销可控，局部修复效率优于全局重算。


<details>
  <summary>Details</summary>
Motivation: LLM生成的行动计划可能语法正确但存在过时、不可行、冲突或破坏性等问题，现有方法缺乏事务性保证。

Method: 提出ATP事务模型：提案非真理，运行时负责准入和提交；实现Mnemosyne系统，包含追加日志、有效状态投影、依赖安全补偿和活跃提交记录。

Result: 9项伪造测试中拒绝目标违规同时接受有效工作；投影与验证开销低于6%；局部修复比全局重算少一个数量级操作。

Conclusion: ATP模型能有效隔离生成层的不确定性，保证提交状态正确性，且具有实际可行性。
...
TL;DR: 提出了一种名为\system{}的离散时间控制系统，通过五个执行齿轮和智能自主生命周期管理，解决了自主智能体在无人类持续监督时面临的安全违规、行为不稳定和连续性丢失问题，在多机器人装配场景中实现了99.6%的异常检测率。


<details>
  <summary>Details</summary>
Motivation: 自主智能体（包括LLM驱动的软件智能体和机器人物理智能体）在无人类持续监督运行时，常面临三类故障：来自未验证动作的安全违规、来自无约束循环的行为不稳定、以及来自未处理错误状态的连续性丢失。

Method: 开发了\system{}离散时间控制系统，结合五个执行齿轮（观察、建议、规划、执行、交互）与效用门控调度和事件驱动回退。单智能体场景证明单调稳定性、执行安全等性质；多智能体CPS场景应用\smart{}管理自主生命周期，结合共识门控、群级Lyapunov分析、每智能体齿轮权限和会合控制。

Result: 在三台UR5机器人装配单元上的10,000次蒙特卡洛实验中，实现了99.6%的异常检测率（单智能体基线为2.1%），检测延迟降低3.5倍，并提供了正式的物理工作空间安全证书。

Conclusion: 执行齿轮作为微观级权限，在\smart{}运行时治理状态之下运作，实现了动作控制与自主治理的分离，有效提升了多智能体系统的安全性和稳定性。
...
TL;DR: 提出SPIRE框架，通过逆向规划将页面级幻灯片个性化（PSP）转化为结构去噪问题，利用多智能体强化学习协同优化幻灯片设计。


<details>
  <summary>Details</summary>
Motivation: 现有的AI智能体方法依赖预设模板或用户详细指令，无法捕捉细粒度的页面级设计意图，导致页面级幻灯片个性化（PSP）问题尚未解决。

Method: 将PSP建模为逆向规划问题，通过故意破坏干净幻灯片的视觉结构，创建一个可验证的去噪任务，两个智能体通过强化学习（RL）协作优化可执行设计。

Result: 实验证明SPIRE在页面级幻灯片个性化任务上表现优越。

Conclusion: 结构去噪可以作为PSP的替代目标，且多智能体公式能严格降低RL中的策略梯度方差。
...
TL;DR: 介绍了一个评估工具增强型LLM智能体在地球化学模拟任务中可靠性的基准PHREEQC-MCQ-200，发现工具访问提升整体准确率但会引入回归问题，且输出访问协议对性能有显著影响。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对工具增强型智能体在科学计算中可靠性的系统评估，特别是工具访问何时真正提升可靠性而非增加复杂性尚不明确。

Method: 构建包含200道多选题的PHREEQC-MCQ-200基准测试，涵盖21个已验证的地球化学模拟场景，评估多个大模型在有无工具访问条件下的表现，并比较不同输出访问协议的影响。

Result: 工具访问显著提升整体准确率，但非单调：部分原本正确的答案因工具增强反而出错；表格式输出接口可降低强模型的成本并保持/提升准确率，但会降低中端模型的表现。

Conclusion: 科学工具使用应被视为端到端的诊断问题，评估不仅应报告准确率，还应包含逐项保留率、输出访问敏感性、轨迹失败和计算链断裂位置等指标。
...
TL;DR: MuSix通过尺度感知的世界模型混合与进化框架，解决了混合专家模型在具身智能中缺乏尺度适应性更新的问题，在EmbodiedBench和HAZARD上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有MoE应用于具身智能时，路由机制缺乏显式的尺度概念，且统一更新策略无法适应不同尺度知识过时速度的差异。

Method: 提出两阶段路由机制：元路由器基于体验距离（受解释水平理论启发）映射到连续尺度空间权重，然后每个尺度的基路由器选择世界模型。采用尺度相关的遗忘率实现不同尺度知识差异化更新，并引入门控跨尺度传输保持层级连贯性。

Result: 在EmbodiedBench和HAZARD基准测试上，MuSix在多尺度推理和动态适应性方面超越了现有最优基线方法。

Conclusion: 尺度感知的MoE设计能有效提升具身智能体在多尺度推理和动态环境适应方面的能力。
...
TL;DR: 本文提出了“AI原生游戏”的定义标准，即运行时生成式AI构成核心玩法循环的必要组成部分，并基于此筛选分析了53款AI原生游戏，提出了G/N双轴分类法，总结了该类游戏的设计挑战与未来方向。


<details>
  <summary>Details</summary>
Motivation: 现有生成式AI在游戏中的应用缺乏明确界定，很多游戏仅附带生成功能却非真正AI原生。需要建立清晰的判别标准来区分AI原生游戏与AI增强游戏，以指导该领域的设计与研究。

Method: 提出反事实判别准则：若移除或平凡替换运行时生成式AI组件，核心玩法是否崩溃或根本改变。基于此标准筛选53款游戏/原型，提出G/N双轴分类法（G轴：玩家面向的游戏类型；N轴：使生成式AI不可或缺的主导AI机制）。

Result: 当前AI原生游戏集中于语言为先的设计（如叙事冒险、认知交互、生成式叙事），而语义裁决、多智能体模拟、生成式建造、关系/同伴玩法等类别较少被探索。核心设计挑战是将语义开放性组织为稳定玩法。

Conclusion: AI原生游戏设计依赖机械不变量（目标、规则、状态、反馈、节奏、玩家能动性）来使开放结果可解释且有意义。未来路线图包括可控生成、AI即机制设计、多模态与多智能体系统、推理经济、评估、安全与监管。
...
TL;DR: 本文发现对齐LLM中，越狱攻击通过抑制提示端的harmfulness或refusal方向来成功；提出HARC方法，通过微调耦合这两个方向来增强安全性，且不损害能力。


<details>
  <summary>Details</summary>
Motivation: 理解对齐LLM内部如何表示安全性，以诊断对齐漏洞、解释越狱成功原因，并设计更鲁棒的对齐策略。

Method: 提出HARC（Harmfulness-And-Refusal Coupling）微调方法，在提示端和响应端将harmfulness和refusal方向进行耦合，干预仅限于安全相关子空间。

Result: HARC在六个基线方法中实现了最佳的鲁棒性-能力-可用性权衡，且harmfulness和refusal方向可跨五个模型家族和两个规模迁移。

Conclusion: 越狱攻击通过抑制harmfulness/refusal方向实现；HARC通过耦合这两个方向有效增强安全性，且不损害通用能力。
...
TL;DR: 本文提出AGI Maze框架，用于测试LLM在部分可观察、有状态的环境中构建世界状态表征的能力。实验表明，普通LLM在简单迷宫中表现不佳，即使利用消息历史作为工作记忆，仍无法可靠解决小规模迷宫问题。


<details>
  <summary>Details</summary>
Motivation: LLM在静态上下文中的下一个词预测模式无法可靠地产生持久、可操作的外部世界表征，在部分可观察、有状态的环境中"推理"能力大幅下降，需要构建基准来评估LLM学习和使用世界状态表征的能力。

Method: 提出AGI Maze框架，包含一系列基于网格的迷宫任务，提供简洁API和多个难度等级。评估了多种普通LLM在简单迷宫上的表现，并引入基线智能体，允许使用消息历史作为工作记忆来构建观察描述。

Result: 普通LLM在LLM推理时间内无法内部表征迷宫结构；即使允许使用消息历史作为工作记忆，LLM智能体在步数预算充足的情况下仍无法可靠解决小规模迷宫。

Conclusion: 当前LLM在需要内存和结构化假设的隐藏状态任务上存在根本性局限，AGI Maze框架为研究这一能力缺口提供了有价值的测试平台。
...
TL;DR: 本文提出了一种结合通用价值函数逼近器、精心选择的训练场景、学习算法和数据增强的框架，使AI代理在完成核心任务的同时能够实时展现不同的"风格"行为，并在《地平线：西之绝境》、《GT赛车》及人形机器人等复杂领域中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习通常学习单一近优行为，但实际应用中用户需要对任务执行方式（即"风格"）进行实时控制。

Method: 结合UVFAs与特定训练场景、学习算法和数据增强，构建一个能够引导代理展现不同风格的框架。

Result: 在AAA游戏《地平线：西之绝境》《GT赛车》及开源人形测试域中，代理均能在满足主要任务的同时强烈遵循风格指令。

Conclusion: 该技术允许最终用户在运行时灵活选择最终行为，实现对执行性能的实时控制。
...
TL;DR: 提出Self-GC系统，将LLM代理上下文管理从文本清理转变为对象生命周期控制，通过索引化对象和侧信道规划实现高效上下文压缩，在保留关键信息的同时显著减少令牌占用。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理系统处理长序列上下文时，依赖运行时启发式方法（如时间剪枝、工具输出屏蔽）或最终自摘要，但这些方法要么忽略未来依赖关系，要么隐藏精确证据和可编辑工件，缺乏系统化的上下文生命周期管理。

Method: 将用户轮次、工具片段和技能状态转变为索引化对象；引入侧信道规划器提出折叠、屏蔽和剪枝操作；利用执行框架实现可恢复的侧车、安全提交边界和缓存感知提交机制。

Result: 在33会话Hard Set中剪枝43.95%前缀令牌且84.85%未来延续不受影响；在332会话数据集上达到91.27%-94.58%无影响率；生产环境中日间平均输入令牌减少10%-15%，峰值接近20%。

Conclusion: 上下文管理应作为对可索引、可恢复对象的运行时生命周期控制，而非事后文本清理。
...
TL;DR: 提出SEA架构，通过在冻结基座模型上使用可审计门控机制和版本化适配器，实现自我进化智能体的安全可控更新，并在SWE-bench上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 自我进化智能体违反传统学习理论假设（数据、评估器、组件和假设空间由被更新的策略产生），缺乏理论保证和安全可控性。

Method: SEA架构包含三个核心：1) 冻结基座模型上的小型转向适配器+版本化容器；2) 随时有效的门控机制，发出可审计证书，控制错误预算；3) 五种验证器在环机制（最佳N选、微步搜索、自建复现预言、搜索层控制、自修复）提供密集无评分器信号。

Result: 在52个SWE-bench实例、4个基座模型上验证，基座能力是主导因素；在两个强基座上，无操作复合控制隔离出+4和+5的提升（GLM 5.2: 24→28; GPT: 29→34，最佳65%），事件日志确认机制触发并防止退化。

Conclusion: 自我进化智能体可通过冻结基座+可审计门控实现安全可控的自我更新，但需进一步验证运行间方差和任务间算法混合的自适应。
...
TL;DR: 本文探讨了随着计算规模指数级增长，前沿AI模型的能力是否会超出小预算开发者的可及范围，结论取决于如何衡量AI能力——有界指标有利于"温驯模型"的普及，而无界指标则导致能力集中在少数富裕行为者手中。


<details>
  <summary>Details</summary>
Motivation: 研究AI能力扩展是否会使得前沿模型能力超越小规模开发者的可及范围，以及能力是否会收敛到"温驯模型"中。

Method: 基于Gundlach et al. (2025b)的工作，分析不同性能指标的数学形式（有界vs无界），并推导出判断哪些指标有利于温驯模型的严格数学条件。

Result: 验证损失显示差距缩小，但其他指标上前沿模型的优势持续扩大。有界指标总是有利于温驯模型，但许多常见有界指标存在对应的无界指标，需要仔细选择。

Conclusion: 决策者需要根据具体能力领域选择恰当的衡量指标，因为指标的有界性决定了政策方向：无界能力会集中到少数富裕行为者手中，有界能力则会通过温驯模型惠及大众。
...
TL;DR: 本文提出Graph-PRefLexOR，一种图原生的推理模型，通过GRPO强化学习将推理组织为显式阶段（机制探索、图构建、模式提取、假设合成），在材料科学开放问题上实现40-65%的提升，并增强了推理的可追溯性和语义多样性。


<details>
  <summary>Details</summary>
Motivation: 标准大语言模型在开放式的材料设计问题上生成的回答流畅但推理过程不可追溯，难以判断最终答案是否基于连贯的中间推理过程，因此需要一种能够显式组织多步推理且可解释的AI系统。

Method: 开发Graph-PRefLexOR图原生推理模型，使用Group Relative Policy Optimization (GRPO)进行微调，将推理过程组织为四个显式阶段：机制探索、图构建、模式提取、假设合成，将神经语言生成与符号关系结构相连接。

Result: 在100个材料科学和力学文献的开放问题上，相比基模型提升40-65%，推理可追溯性提升最大；语义多样性比基线高约2-3倍；测试时图扩展显示额外计算主要增加长程概念重组而非语义覆盖范围。

Conclusion: 图原生强化学习为材料设计及其他科学应用中可解释的AI假设生成系统提供了一条有效路径。
...
TL;DR: 本文提出一种基于贝叶斯网络的不确定性感知Agentic RAG框架，通过传播规划器、评估器和生成器的不确定性信号来估计系统级不确定性，在HotpotQA上表现优于StrategyQA。


<details>
  <summary>Details</summary>
Motivation: 可信赖部署Agentic RAG系统需要估计多阶段推理流水线何时可能失败，但目前缺乏系统级的不确定性量化机制。

Method: 构建包含规划器、评估器和生成器的Agentic RAG框架，各阶段产生基于语义分歧和生成器自评估的不确定性信号，通过贝叶斯网络传播这些信号以估计系统级不确定性，并提供节点级故障点指示。

Result: 在StrategyQA和HotpotQA上使用GPT-3.5-Turbo和GPT-4.1-Nano评估，使用AUROC、AUARC、ECE和Brier Score指标。贝叶斯传播在HotpotQA上更有效（多跳推理累积不确定性），StrategyQA暴露了校准不足和上游信号不可靠的问题。

Conclusion: 贝叶斯不确定性传播是监控Agentic RAG系统有前景但初步的机制，需在工业领域（如海上风电维护决策支持）进一步验证。
...
TL;DR: 提出了PedNStream，一个基于LTM的开源宏观行人网络流仿真框架，支持随机链路动力学和控制器集成，用于大规模人群管理。


<details>
  <summary>Details</summary>
Motivation: 大多数开源行人仿真工具要么是微观的，要么不适用于网络规模的闭环评估，缺乏兼具计算效率和反馈控制兼容性的宏观仿真平台。

Method: 基于Link Transmission Model (LTM)扩展，引入随机链路动力学（扩散和活动引起的变异性），用基于效用的路径选择替代动态用户均衡，采用模块化框架并内置控制器接口。

Result: 合成场景验证了队列形成、回堵、拥堵消散和自适应重路由等机制；真实网络实验验证了大规模行为和与实际观测的一致性；闭环案例展示了控制器集成；运行时分析量化了可扩展性。
...
TL;DR: 本文提出了一种基于多智能体LLM的自动化管道，能从专利文献中自动提取化学反应规则，将标准分类从68类扩展到14,073类，并以97.7%的准确率分类未见反应，实现了可自我扩展的符号系统。


<details>
  <summary>Details</summary>
Motivation: 现有的计算机辅助合成规划依赖人工编码的固定规则集，无法适应长尾化学现象和新化学反应，且手动编码难以扩展。

Method: 使用多智能体LLM框架，在665,901个美国专利反应上自动分类并编写反应规则，每个规则通过验证循环在语料库中进行测试。

Result: 将标准分类从68类扩展到14,073类；轻量级指纹分类器对未见反应的分类准确率达到97.7%，与领先的商业分类器相当且分辨率更高。

Conclusion: 该方法构建了一个活的反应性数据库，提供了一条将生成模型转化为可靠、可自我扩展的符号系统的通用路径。
...
TL;DR: 本文提出OpenAgent问题形式化框架,研究LLM智能体在开放环境下的分布偏移问题,并提出了扰动增强微调策略来提升智能体鲁棒性。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在静态基准测试中表现出色,但在真实场景中面临用户查询、工具集和交互动态的持续变化,存在泛化鸿沟。

Method: 形式化OpenAgent问题设定,构建可控沙箱环境,定义感知、交互、推理、内化四层层次的细粒度环境偏移,并提出扰动增强微调(Perturbation-Augmented Fine-Tuning)干预策略。

Result: 实验表明,无论是SFT还是RL训练的智能体,在面对开放环境偏移时均存在不同程度的性能下降。
...
TL;DR: 提出了一种两步法，通过约束规划优化调度和状态依赖系统执行，解决自主实验室中多仪器硬件资源利用问题，用于金属有机框架合成。


<details>
  <summary>Details</summary>
Motivation: 自主实验室中AI代理能建议实验，但面对多仪器不同容量和吞吐量的硬件约束时，如何高效规划与执行实验调度是一个挑战。

Method: 两步法：1）使用约束规划寻找最优调度，最小化总时间同时满足硬件限制和容量；2）为每个任务建立状态依赖系统，确保最优调度的稳健执行。

Result: 该方法能够找到满足硬件约束的最优调度，并通过状态依赖系统实现稳健执行。

Conclusion: 该方法有效解决了自主实验室中多仪器硬件资源利用的调度与执行问题，适用于金属有机框架合成等场景。
...
TL;DR: 本文提出Theoria验证架构，将AI候选答案重写为可独立审计的状态转换序列，通过“变更完整性”确保每一步都有显式理由。在HLE-Verified Gold上以91.4%严格精度认证105个问题，并发现与整体LLM评判互补（Jaccard 0.14-0.36）；在对抗性毒化证明中，结构化评判比整体评判高出11.5个百分点，主要优势体现在隐藏前提和伪造引用上。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统在可信度验证上存在两难：形式化证明覆盖范围有限，而标量LLM评判评分不透明且无法事后审计。作者希望填补这一差距，构建可审计、可验证的推理信任架构。

Method: Theoria架构将候选答案重写为类型化状态转换序列，每一步都需要明确理由（引用、计算或问题给定事实），并确保相邻证明状态之间的每个差异都必须有解释（变更完整性）。通过严格的形式化审计实现可验证性。

Result: HLE-Verified Gold上认证105题，严格精度91.4%；对抗性毒化证明捕获率94.7%（vs 整体评判83.2%）；隐藏前提检测90.6%（vs 62.5%）；伪造引用检测100%（vs 90%）；GPQA Diamond上认证精度97.1%。

Conclusion: 结构化验证架构在可审计性和对抗性鲁棒性上优于整体LLM评判，尤其擅长捕捉隐藏前提和伪造引用等关键错误类型，且与整体评判具有互补性。
...
TL;DR: 本文提出了一种模块化的多层智能体架构，用于模拟气候治理过程中公民、NGO、媒体和政治决策者之间的复杂互动，结合了认知决策模型与制度行为。


<details>
  <summary>Details</summary>
Motivation: 现有气候治理的智能体模型往往将制度动态与个体行为机制割裂开来，缺乏统一框架来模拟二者在民主治理中的交互。

Method: 构建了模块化多层智能体架构，整合了(HUMAT/MOA框架的)动机驱动个体决策、基于同质性的社会影响网络，以及NGO、媒体、政治家等制度层行为模块；政治决策由专家意见、公众动员、政党立场和媒体框架等多信号聚合产生。

Result: 本文为概念性方法论文，尚未呈现实证结果，重点阐述了架构设计原则、模块结构和集成逻辑。
...
TL;DR: 提出M2Note，一种无需训练的持续演进来解决VLM多模态推理中的常见错误，通过将失败轨迹转化为可检索的记忆笔记，在推理时引导模型避免重复错误。


<details>
  <summary>Details</summary>
Motivation: VLM在多模态推理中仍存在跳过视觉检查、错误应用领域规则、幻觉等问题，现有SFT和RL方法迭代成本高且对分布变化脆弱。

Method: M2Note：训练无关的持续演进框架，将失败轨迹转化为"主题-指导"笔记；测试时通过多模态RAG检索相关笔记注入上下文；采用批量级后验证与回滚机制稳定演进；支持自我演化和跨模型演化。

Result: 在6个多模态推理基准上持续改进，跨领域和 backbone 均有效，成本低、样本高效，且与CoT提示互补。

Conclusion: M2Note通过外部化学习到可编辑记忆，实现无需权重更新的VLM持续错误纠正和能力迁移，是一种高效、稳健的多模态推理增强框架。
...
TL;DR: 本文提出SIVE框架，通过虚构城镇Montelago的120个合成角色，验证LLM驱动的生成式合成群体(GSP)的内部有效性——即合成群体对已知效价刺激的反应是否有序、可复制且符合群体结构。所有七项预注册标准在温度扫描中均通过，并发现了一个诊断性成功案例。


<details>
  <summary>Details</summary>
Motivation: 在使用GSP工具于真实人群之前，必须回答一个基本问题：它是否以有序、可复制、群体结构化的方式响应已知效价的刺激（即可控性）。本文关注的是内部效度——合成群体是否能追踪自身施加的潜在结构。

Method: 构建虚构城市Montelago，创建120个具有已知潜在结构的合成角色，暴露于从强正向到强负向的七种机构通信条件下。通过温度扫描，评估七个预注册标准：保真度、稳定性、噪声底限、特异性、敏感性和有序性。

Result: 所有七项标准在每个温度下均通过。发现一个校准失败转化为诊断成功的案例：一个设计为"弱正向"的消息被识别为功能负向，追溯发现其文本中包含未解决问题、不确定性和机构被动性；重新设计后恢复了预期排序。噪声子实验显示仪器固有噪声约为跨代理估计的一半，且在不同温度下稳定。

Conclusion: 在将GSP用于外部有效性主张之前，内部有效性验证是逻辑上必要的前提。SIVE框架提供了一种系统性的内部效度验证方法，揭示了聚合统计量所掩盖的微观动态。
